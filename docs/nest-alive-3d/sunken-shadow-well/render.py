"""Exploration render for the `sunken-shadow-well` nest concept.
...
The thesis is depth from light rather than geometry — nothing is drawn in
front of the occupant, so the whole silhouette stays readable and the sense
of "down inside the bowl" comes purely from a luminance falloff, a wall AO
arc and a bounce highlight.
"""
import os, sys, math
...
# ── AFTER: sunken-shadow-well ────────────────────────────────────────────────

_WELL_TOP_DY   = -2    # first row that still reads as fully lit
_WELL_FULL_DY  = 8     # row where the falloff reaches its floor
_WELL_BOT_DY   = 20
_WELL_MAX_A    = 115   # ~55% luminance at the bottom of the ramp
_WELL_CACHE = None


def _well_gradient(bird, bw, bh, by, cy):
    """Vertical luminance falloff, pre-masked to the occupant's silhouette.

    Masking to bird pixels matters because the multiply also lands on the sky
    and on the near weave rows below the bowl; without it the shading would
    read as a rectangular smudge instead of the bird sitting in shade.
    """
    global _WELL_CACHE
    ...
    span = float(_WELL_FULL_DY - _WELL_TOP_DY)
    gx0 = _NEST_CX - 18
    for row in range(g.get_height()):
        y = top + row
        t = min(1.0, max(0.0, (y - top) / span))
        a = int(round(_WELL_MAX_A * t))
        lum = 255 - a
        for gx in range(36):
            sx, sy = gx0 + gx - (_NEST_CX - bw // 2), y - by
            if 0 <= sx < bw and 0 <= sy < bh and bird.get_at((sx, sy)).a > 0:
                g.set_at((gx, row), (lum, lum, lum, 255))
...
def draw_slot_after(surf, cy, alive):
    ...
    if alive:
        ...
        # Depth is spent on light, not on occluders: the occupant dims towards
        # its feet as if the bowl swallowed the light, and the head stays lit.
        g, gx0, gtop = _well_gradient(bird, bw, bh, by, cy)
        surf.blit(g, (gx0, gtop), special_flags=pygame.BLEND_RGBA_MULT)

        # Inner wall of the bowl, one pixel inside the lip — the contact
        # darkening that tells the eye the wall carries on behind the rim.
        ...
        # Warm bounce off the far wall lands on whatever the topmost lit edge
        # of the occupant is, so the shoulders separate from the black hollow.
        ...
        # The thinnest possible front lip: one pixel is enough to close the
        # ellipse once the shading is already doing the depth work.
        pygame.draw.arc(surf, _NEST_TWIG_BRIGHT, rim_abs, math.pi, 2 * math.pi, 1)
    else:
        hx, hy_off, hw, hh = hollow
        pygame.draw.rect(surf, _NEST_HOLLOW_COL, (hx, cy + hy_off, hw, hh))
...
def main():
    ...
    sheet.fill((28, 30, 34))
    sheet.blit(font.render("sunken-shadow-well  -  round 1", True, (240, 235, 225)),
               (margin, y_title))

    labels = ["today / alive", "today / empty", "concept / alive", "concept / empty"]
    x0 = (W - top_w) // 2
    for i, (p, lbl) in enumerate(zip([p_before_a, p_before_e, p_after_a, p_after_e],
                                     labels)):
        x = x0 + i * (PANEL_W + gap)
    ...
    sheet.blit(small.render("AFTER  alive  (10x)", True, (255, 214, 120)),
               (x1 + zw + gap, y_bot_lbl))
