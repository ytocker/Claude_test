"""
`bandaged-breast` — hurt-parrot concept exploration (standalone, not wired in).

V5, built on battle-bloodshot (V3). Same fight, later in the story: the bird has
...
from out-shouting the beak and the lens glint as the sprite's brightest point.
...
# Field-dressing cream, not clinic white: a pad this size at full white becomes
...
def _stamp_clipped(surf, layer):
    """Composite a dressing layer only where the bird already exists, keeping the
    destination alpha. Dressings are laid out on generous quads and trimmed by
    the silhouette rather than hand-fitted to it — that way an edge that slightly
    overhangs the body follows the body's own curve instead of showing a stray
    corner floating off in the sky."""
    for x in range(surf.get_width()):
        for y in range(surf.get_height()):
            px = layer.get_at((x, y))
...
def _draw_chest_dressing(surf):
    """The primary read: a taped pad high on the breast with a red cross on it.
    The cross is the one piece of universal shorthand available at this sprite
    size — a player parses "patched up" from it before they parse anything else
...
def _build_hurt_frame(wing_angle_deg):
    ...
    # Dressings last, and the chest pad last of all — laid over the finished rake
