"""DISPOSABLE BOTTLE — single-use supermarket PET water bottle parcel.

The IDENTITY is the throwaway PET bottle read, which must stay DISTINCT from the
shipped sports squeeze bottle. Three beats carry it: a small BLUE screw CAP (not
the squeeze bottle's red one), a white/blue paper LABEL band wrapping the MIDDLE
of the body (the brand read no squeeze bottle has), and a RIBBED lower body (the
moulded PET base flutes). The body is clear-blue tinted with blue water, so the
...
def build(mode="normal") -> pygame.Surface:
    ...
    body_rad = 4                  # PET base rounds in more than the squeeze bottle
