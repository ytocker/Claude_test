...
HIGH-FIDELITY approach (WHY): the prior pass drew on a 44px work surface and
collapsed Pip to three flat masses, which read low-res / blobby at true size.
This version supersamples HARD — the whole baby is composed at 6× the output on a
132px work surface — so the parent's real detail (hooked two-tone beak, a layered
...
crisply when smoothscaled DOWN to the parcel. Output is bumped to 28px (a touch
over PARCEL_SIZE; the draw code centre-anchors and parcels may run larger) so the
added beak + eye + wing detail actually resolves on-screen without growing the
footprint much.

A baked dark OUTLINE drawn first (inflated silhouette) carries the shape on
bright DAY sky; a warm cream keyline traced inside is the NIGHT lifeline.
Everything is held in toward centre with off-edge margins so the rotozoom across
Pip's bank arc never clips the crest, beak, or tail against the surface edge.
"""
import pygame
