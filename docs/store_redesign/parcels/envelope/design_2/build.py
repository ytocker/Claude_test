...
flat object that must survive Pip's bank without collapsing into "a card". Three
things carry the read at true size: the rose flap V (the envelope cue), one
unbroken CREAM band across the lower third (the letter inside), and the glossy
red HEART seated on that band like a wax seal. The heart is value-separated by a
cream halo ring so it survives even in grayscale — the identity can't ride on
hue alone. Built on a 44px work surface then smoothscaled to 22 (same scaffold
as the mailer) with a baked dark outline for DAY sky and a warm keyline for
NIGHT.
"""
import pygame
...
# the fold is what sells "envelope" on the gameplay sprite, not just the hero.
ROSE_FLAP_SHADE = (168, 88, 108)
CREAM = (246, 233, 221)        # ~#F6E9DD — the letter band + heart halo
...
def build(mode="normal") -> pygame.Surface:
    ...
    # wax seal closing the envelope, rather than straddling the rose V. A CREAM
    # halo ring outside the dark outline value-separates it: heart-red and rose
    # collapse to the same gray, so without the light ring the heart vanishes in
    # grayscale.
