"""
5 options for a dark interior fill inside the nest cup (the "upper ellipse").
Each option varies the fill colour drawn before sticks/courses so the
bird sits inside a visibly solid dark bowl rather than floating on the panel.
"""
import os, sys, math
...
OUT_DIR = f"{ROOT}/docs/lives-display-v16/nest-interior"
os.makedirs(OUT_DIR, exist_ok=True)

# ── Nest palette (unchanged) ──────────────────────────────────────────────────
PANEL_DARK   = (12, 8, 38)
...
# ── 5 interior fill colour options ───────────────────────────────────────────
# Each is (fill_rgb, slug, label)
OPTIONS = [
    (( 65, 38, 12), "earth-brown",  "ID 1  earth brown"),
    ((  8, 12, 72), "deep-navy",    "ID 2  deep navy"),
    (( 55, 10, 55), "dark-plum",    "ID 3  dark plum"),
    (( 38, 32, 28), "warm-charcoal","ID 4  warm charcoal"),
    ((  8, 50, 38), "dark-teal",    "ID 5  dark teal"),
]

src     = parrot_mod._get_frames()[1]
verts, courses, stick_wins, rim_rect, stick_bottom, hollow, bird_h = make_params(CX, S)
bird_w  = max(1, int(src.get_width() * bird_h / src.get_height()))
bird    = pygame.transform.smoothscale(src, (bird_w, bird_h))


def make_draw_fn(interior_col):
    cx = CX

    def draw_slot(surf, cy, alive):
        rx, ry_off, rw, rh = rim_rect

        # ── NEW: fill the nest cup interior with a solid dark background ──
        # 1. Solid rect covering the full slot height (shows through course gaps)
        pygame.draw.rect(surf, interior_col, (rx, cy, rw, stick_bottom + 4))
        # 2. Filled ellipse for the arc/rim opening above the courses
        cup_y = cy + ry_off
        cup_h = rh + 2
        pygame.draw.ellipse(surf, interior_col, (rx, cup_y, rw, cup_h))

        # back rim arc (drawn over the fill)
...
for col, slug, label in OPTIONS:
    ...
    print(f"{label:22s}  bird-red={count:3d}  {'PASS' if count > 20 else 'FAIL'}")
...
hdr = "NEST INTERIOR  \xb7  DARK FILL OPTIONS  \xb7  V15"
try:
...
for idx, (label, path) in enumerate(renders):
    ...
    draw.rectangle([px, py0, px+PW-1, py0+PH-1], outline=(50, 44, 80), width=1)
    ...
    try:
        ...
        fw, fh = len(label)*5, 8
    draw.text((px + (PW-fw)//2, fy + (FTR_H-fh)//2), label, font=flbl, fill=GOLD)

SHOWCASE = f"{OUT_DIR}/nest_interior_compare.png"
