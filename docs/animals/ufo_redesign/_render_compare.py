"""Final 5-up comparison: every winning UFO concept in the real gameplay frame.
...
so the five can be compared exactly as they appear mid-play before the user picks.

Run:
    SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy python docs/animals/ufo_redesign/_render_compare.py
"""
...
CONCEPTS = [
    ("LANDER POD", "lander_pod"),
    ("BLACK TRIANGLE", "triangle"),
    ("EYE-ORB", "eye_orb"),
    ("BIO-JELLYFISH", "jelly"),
    ("RINGSHIP", "ringship"),
]
...
def load_build(folder):
    p = HERE / "concepts" / folder / "build.py"
    spec = importlib.util.spec_from_file_location(f"_cmp_{folder}", p)
...
def main():
    ...
    out = HERE / "gameplay_compare_v2.png"
