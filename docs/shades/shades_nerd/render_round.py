"""Preview sheet for NERD SPECS (shades_nerd) — self-contained, no game imports.

Left: product shot at eye_w=96 on neutral grey. Right: in-game read at eye_w=22
over a ~24px scarlet circle (Pip's head) with a small dark eye dot, shown native
and at ~7x zoom so the eye-through-clear-lens read is judged big. Headless.
...
def head_with_specs(eye_w, head_r):
    """A scarlet head circle with a dark eye dot, then the clear specs over it —
    the eye must still read through the lens."""
    pad = int(eye_w * 0.9)
    size = head_r * 2 + pad * 2
...
def main():
    ...
    sheet.fill((118, 122, 130))   # neutral grey backdrop
    sheet.blit(title.render(
        "SKYBIT — SHADES · NERD SPECS · clear lenses, chunky black horn-rims",
        True, (20, 22, 28)), (24, 14))
    ...
    # ~7x zoom of the on-Pip head so the eye-through read is judged big.
    Z = 7
