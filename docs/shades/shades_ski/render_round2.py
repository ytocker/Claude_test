"""Round-2 preview sheet for SKI GOGGLES (shades_ski) — self-contained.

Left: product shot at eye_w=96 on neutral grey. Right: the in-game read at
eye_w=22 over a ~24px scarlet head (Pip), shown native and at ~7x zoom so the
single-band strap + inward highlight are judged big against the scarlet edge.
Headless.

Run:  SDL_VIDEODRIVER=dummy python docs/shades/shades_ski/render_round2.py
"""
import os
import sys
...
def head_with_specs(eye_w, head_r):
    """Scarlet head circle with a dark eye dot, then the goggles over it — the
    goggle must sit on the head without bleeding past the scarlet edge."""
    pad = int(eye_w * 1.0)
    size = head_r * 2 + pad * 2
...
def main():
    ...
    sheet.blit(title.render(
        "SKYBIT — SHADES · SKI GOGGLES · R2 · white frame · one warm strap",
        True, (20, 22, 28)), (24, 14))
    ...
    sheet.blit(tiny.render("~7x zoom — strap reads as one band, no bleed",
                           True, (20, 22, 28)), (zrect.left, zrect.bottom + 6))
