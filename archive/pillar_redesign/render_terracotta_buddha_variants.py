"""Terracotta + Buddha pillar exploration sheet — round 3.

10 rows × 5 biome phases (day · sunrise · sunset · dusk · night). One
canonical seed per cell so the comparison is honest — different rows
are different candidates, not different RNG rolls of the same draw.

Output:
  docs/pillar_redesign/terracotta_buddha_round_3.png

Run from anywhere:
    SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy \
        python archive/pillar_redesign/render_terracotta_buddha_variants.py
"""
...
import pillar_terracotta_buddha_variants as tbv
...
# Ground accents: warriors all share the loess-dust band from Xi'an's
# Lintong; the Buddhas each get a hint of their own siting.
VARIANTS = [
    ("Terracotta General (高级军吏俑) — officer, double fish-tail crown",
    ...
    ("Standing Maitreya / Budai (彌勒) — laughing, cloth bag",
...
VARIANT_URLS = [
    ...
    "travelchinaguide.com/.../warrior-types.htm",
    ...
    "wiki/Reclining_Buddha + Yungang_Grottoes",
...
def _scene_backdrop(phase: float) -> pygame.Surface:
    """Sky + clouds + keeper-V4 mountains + ground band, no pillars."""
...
def render_tile(candidate_name: str, accent_band: tuple,
    ...
    # Two pillar pairs — primary on the right (full slot test), and one
    # smaller pair on the left so the AD can verify the figure reads at
    # a different gap size too.
    gap_y = 280
    gap_h = 170
    px = W - 90
...
def make_sheet() -> pygame.Surface:
    ...
    title = font_head.render(
        "TERRACOTTA + BUDDHA  —  R3 ·  "
        "10 candidates × day → sunrise → sunset → dusk → night",
        True, (240, 240, 240))
    ...
    for r, (label, fn_name, tag, accent) in enumerate(VARIANTS):
        ...
        for c, (pname, pval) in enumerate(PHASES):
            ...
            # Clear the cache so each cell genuinely re-renders — the
            # cache key already separates by palette but explicit wipe
            # is honest comparison.
            if hasattr(tbv, "_PILLAR_CACHE"):
                tbv._PILLAR_CACHE.clear()
...
def main() -> None:
    sheet = make_sheet()
    out = OUT / "terracotta_buddha_round_3.png"
