...
def _ground_clear(sx, half_w=10):
    lo, hi = _GROUND_PILLAR_LANE
    return not (sx + half_w > lo and sx - half_w < hi)


def _post_ok(sx, half_w=6):
    """A vertical post is allowed only at the far left (x<55) or inside the
    narrow mid dead zone — never at the bird's altitude/column or the pillar."""
    if sx + half_w < 55:
        return not (sx + half_w > _PILLAR_LANE[0] and sx - half_w < _PILLAR_LANE[1])
    lo, hi = _POST_DEADZONE
    if sx - half_w >= lo and sx + half_w <= hi:
        return True
    return False
