#!/usr/bin/env python3
"""
sun-arc  ·  flight-log arc v2  ·  arc_minimal — direction D: just the arc, no chrome.
...
def draw_overlay(ss):
    ...
    # NEWBIE PERIOD — no explicit marker; the warm rail dots carry it subtly.
    ...
    for p, rad, fn, col in ring_items:
        x, y = arc_pos(p, rad)
        if p > DEATH_PHASE:
            continue  # Past death: nothing visible — pure mystery
        if rad != R_INNER:
