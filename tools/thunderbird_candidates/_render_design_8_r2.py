"""Round-2 review sheet for thunderbird CIRCUIT RAPTOR (design_8)."""
import os
...
from tools.thunderbird_candidates.design_8 import build

OUT = "/home/user/skybit/docs/store_redesign/animal/thunderbird/design_8/round_2.png"
...
hero = ninja_render.hero_panel(build, 220, bg=(12, 26, 30))
...
# 40px truth per frame — proves the node-pulse sweeps root→tip in motion.
truth40 = []
for i in range(4):
    src = build(i, 0.0)
    w = 40
    h = int(src.get_height() * w / src.get_width())
    truth40.append(pygame.transform.scale(src, (w, h)))
...
sheet_w = max(sheet_w, 620)
sheet_h = pad + 40 + top_h + pad + truth_panel_h + pad + strip_h + pad + 150

sheet = pygame.Surface((sheet_w, sheet_h))
sheet.fill((22, 24, 30))

title = pygame.font.SysFont("dejavusans", 18, bold=True).render(
    "THUNDERBIRD  —  Design 8: CIRCUIT RAPTOR  (Round 2)", True, (217, 255, 246))
...
def caption(x, yy, text, w):
    surf = font.render(text, True, (200, 210, 220))
...
x += gp.get_width() + pad
caption(x, y, "Hero (void-dark)", hero.get_width())
sheet.blit(hero, (x, y + label_h))
y += top_h + pad

# Row 2: 40px truth chips on bright + dark backgrounds.
caption(pad, y, "40px NEAREST truth  —  build(2, 10)  on bright-day + night sky", sheet_w - pad * 2)
cy = y + label_h + 20
chip = 120
...
# Row 4: 40px per-frame truth strip — node-pulse sweep verification.
caption(pad, y, "40px NEAREST per frame  —  node-pulse current sweeps root→tip", sheet_w - pad * 2)
ty = y + label_h
tx = pad
tchip = 96
for i, t in enumerate(truth40):
    cell = pygame.Surface((tchip, tchip))
    cell.fill((150, 205, 235) if i % 2 == 0 else (10, 14, 26))
    cell.blit(t, t.get_rect(center=(tchip // 2, tchip // 2)))
    sheet.blit(cell, (tx, ty))
    lc = (40, 40, 50) if i % 2 == 0 else (200, 210, 220)
    sheet.blit(small.render(f"f{i}", True, lc), (tx + 4, ty + 2))
    tx += tchip + 10
