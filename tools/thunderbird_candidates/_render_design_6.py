...
hero = ninja_render.hero_panel(build, 220, bg=(10, 8, 20))
...
sheet_w = max(sheet_w, pad * 2 + 4 * (fw + 12))
...
def caption(x, yy, text, w):
    surf = font.render(text, True, (210, 205, 190))
...
caption(pad, y, "40px NEAREST truth  —  build(2, 10)  on bright-day + night sky",
        sheet_w - pad * 2)
cy = y + label_h + 20
chip = 120
...
sheet.blit(small.render("night", True, (200, 205, 220)), (pad + chip + pad + 6, cy + chip - 18))
sheet.blit(small.render("(40px wide, nearest)", True, (170, 175, 190)),
...
fy = y + label_h
fx = pad
for i, ff in enumerate(strip):
    cell = pygame.Surface((fw + 6, fh + 6))
    cell.fill((22, 18, 30))
    cell.blit(ff, ff.get_rect(center=((fw + 6) // 2, (fh + 6) // 2)))
    sheet.blit(cell, (fx, fy))
    sheet.blit(small.render(f"f{i}", True, (210, 205, 190)), (fx + 4, fy + 2))
    fx += fw + 12
