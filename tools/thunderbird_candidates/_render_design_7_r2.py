"""Round-2 review sheet for thunderbird BALL LIGHTNING (design_7)."""
import os
...
from tools.thunderbird_candidates.design_7 import build

OUT = "/home/user/skybit/docs/store_redesign/animal/thunderbird/design_7/round_2.png"
...
hero = ninja_render.hero_panel(build, 220, bg=(20, 14, 6))

# 40px NEAREST truth on both bright and dark chips.
truth_src = build(2, 10.0)
tw = 40
th = int(truth_src.get_height() * tw / truth_src.get_width())
truth = pygame.transform.scale(truth_src, (tw, th))  # nearest (default)
...
sheet_w = max(sheet_w, 560)  # ensure the title fits on one line
...
title = pygame.font.SysFont("dejavusans", 18, bold=True).render(
    "THUNDERBIRD  —  Design 7: BALL LIGHTNING  (Round 2)", True, (255, 243, 176))
...
def caption(x, yy, text, w):
    surf = font.render(text, True, (220, 210, 200))
...
x += gp.get_width() + pad
caption(x, y, "Hero (scorch-dark)", hero.get_width())
...
# actual 40px pixel size shown 1:1 too
...
for i, f in enumerate(strip):
    ...
    sheet.blit(small.render(f"f{i}", True, (220, 210, 200)), (fx + 4, fy + 2))
    fx += fw + 12
