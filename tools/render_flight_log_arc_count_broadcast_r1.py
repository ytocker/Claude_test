#!/usr/bin/env python3
"""
arc_count/broadcast — round 1.

Lower-thirds sports-broadcast chrome: two solid stats bands anchored at
y=468–540, leaving ~390px of open sky for the arc and parrot. Hero "18%"
floats in the open sky zone. Chyron callout near the parrot.
...
REPLACED: render_screen() chrome only — broadcast lower-thirds concept.
"""
import os
import math
import random
...
def render_screen():
    ...
    # ── Banner: lighter top bar, less intrusive since stats live at bottom ──
    ...
    # ── Hero "18%" floating in open sky, right of arc apex ──
    f28 = pygame.font.Font(FONT_PATH, 28)
    pct_s = f28.render(f"{int(DEATH_PHASE*100)}%", True, GOLD)
    pct_x, pct_y = 210, 178
    surf.blit(pct_s, (pct_x, pct_y))
    text(surf, "OF THE DAY FLOWN", 7, midleft=(pct_x, pct_y + pct_s.get_height() + 3),
    ...
    # ── Death chyron — sharp-cornered box near parrot ──
    chyr_w, chyr_h = 110, 24
    chyr_x = max(4, int(bx) - chyr_w - 12)
    chyr_y = int(by) - 14
    ...
    text(surf, f"ENDED · PILLAR {DEATH_PILLAR}", 8, midleft=(chyr_x+6, chyr_y+12),
         color=CREAM, shadow=None)

    # ── Band 1 — header strip (y=468–494, full width) ──
    B1_Y, B1_H = 468, 26
    band1 = pygame.Surface((W, B1_H), pygame.SRCALPHA)
    band1.fill((*INK, 240))
    # Left gold accent bar
    pygame.draw.rect(band1, (*GOLD, 255), (0, 0, 4, B1_H))
    surf.blit(band1, (0, B1_Y))
    alpha_line(surf, (*GOLD, 100), (0, B1_Y), (W-1, B1_Y), 1)
    text(surf, "FLIGHT LOG", 10, midleft=(12, B1_Y + B1_H//2), color=GOLD, track=3, shadow=None)
    text(surf, f"DAY {DAY_N}", 12, midright=(W-12, B1_Y + B1_H//2), color=CREAM, shadow=None)

    # ── Band 2 — stats strip (y=496–540, four cells) ──
    B2_Y, B2_H = 496, 44
    band2 = pygame.Surface((W, B2_H), pygame.SRCALPHA)
    band2.fill((*SCRIM, 250))
    # Left gold accent bar
    pygame.draw.rect(band2, (*GOLD, 255), (0, 0, 4, B2_H))
    surf.blit(band2, (0, B2_Y))

    # Four columns: 18% large | PILLAR | TIME | SEALED
    # Column dividers
    for div_x in (90, 190, 270):
        alpha_line(surf, (*GOLD, 50), (div_x, B2_Y+4), (div_x, B2_Y+B2_H-4), 1)

    # Cell 1: 18% oversized
    f22 = pygame.font.Font(FONT_PATH, 22)
    c1 = f22.render(f"{int(DEATH_PHASE*100)}%", True, GOLD)
    surf.blit(c1, (8, B2_Y + (B2_H - c1.get_height())//2))
    text(surf, "FLOWN", 6, midleft=(8, B2_Y + B2_H - 10), color=(120,116,108), track=2, shadow=None)

    # Cell 2: PILLAR
    text(surf, "PILLAR", 7, center=(140, B2_Y + 14), color=(120,116,108), track=2, shadow=None)
    text(surf, str(DEATH_PILLAR), 14, center=(140, B2_Y + 30), color=CREAM, shadow=None)

    # Cell 3: TIME
    text(surf, "TIME", 7, center=(230, B2_Y + 14), color=(120,116,108), track=2, shadow=None)
    text(surf, f"0:{TIME_ALIVE:02d}", 14, center=(230, B2_Y + 30), color=CREAM, shadow=None)

    # Cell 4: SEALED
    text(surf, "SEALED", 7, center=(305, B2_Y + 14), color=(120,116,108), track=2, shadow=None)
    text(surf, "4/4", 14, center=(305, B2_Y + 30), color=(160,155,175), shadow=None)

    # ── BACK — below bands ──
    text(surf, "← BACK", 10, center=(W//2, 575), color=(140,136,128), track=3, shadow=None)
