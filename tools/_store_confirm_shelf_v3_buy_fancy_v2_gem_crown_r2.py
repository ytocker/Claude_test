...
Output → docs/store_confirm_shelf_v3/buy-fancy-v2/gem-crown/round_2.png
"""
import os, sys, math
...
def _crown_gems(big):
    # A peak-taper crown arc — a raised, larger center gem flanked by lower,
    # smaller outer gems — so the cluster reads as a crown, not a flat row.
    # Drawn AFTER the button so it seats ON the metal lip, not behind it.
    LEG = sc.RARITY["legendary"]

    # Warm aura first so the gems appear to radiate light onto the button lip.
    sc._alpha_aura(big, m(58), m(BTN_CY) - m(25), m(24), LEG["glow"],
                   peak=60, layers=10)

    # (x, y, r): center peak raised + enlarged, outer pair widened + tapered.
    gems = [
        (m(40), m(BTN_CY) - m(22), m(7)),
        (m(58), m(BTN_CY) - m(28), m(9)),
        (m(76), m(BTN_CY) - m(22), m(7)),
    ]
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-crown (A) r2  |  AFFORDABLE / UNAFFORDABLE",
...
# PIL verification
aff = panels_data[0][2]
gem_px = aff.getpixel((BUY_CX, BTN_CY - 28))
print(f"center crown gem ({BUY_CX},{BTN_CY-28}): {gem_px}  → "
