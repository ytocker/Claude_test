"""Render the Design 4 (Lab Specimen #7) R2 review sheet: gameplay panel,
hero product-shot, and a 40px NEAREST truth-read, labelled and committed to
docs/store_redesign/costume/zombie/design_4/round_2.png."""
import os
...
from tools.zombie_candidates.design_4 import build

OUT = "docs/store_redesign/costume/zombie/design_4/round_2.png"
...
def truth_read(source):
    """Scale the hero frame down to ~40px (NEAREST), then back up 5x — the
...
title = font_big.render("DESIGN 4 — LAB SPECIMEN #7 · R2", True, (232, 236, 210))
...
for panel, w, cap in zip((gp, hero, tr), col_w, captions):
    ...
    # Backing plate for the truth-read so the transparent margin reads clean.
    if cap.startswith("40px"):
        plate = pygame.Rect(x - 6, y - 6, panel.get_width() + 12, panel.get_height() + 12)
        pygame.draw.rect(sheet, (18, 18, 24), plate, border_radius=8)
    sheet.blit(panel, (x, y))
    ct = font_sm.render(cap, True, (170, 174, 160))
