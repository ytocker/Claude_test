#!/usr/bin/env python3
"""
arc_count / MEDALLION concept — round 1.

Upper-left glowing badge anchors "18%" trophy number + event-color pips.
Stats run left-concentrated. Death callout is a borderline chip.
BACK is a small left-anchored rounded rect.
"""
import os
import math
import random
...
def render_screen():
    ...
    # Badge — upper-left sky margin (center at ~52, 42, overlaps banner hairline)
    BCX, BCY = 52, 42
    ...
    pygame.draw.circle(badge_lay, (*INK, 220), (40, 40), 36)
    pygame.draw.circle(badge_lay, (*GOLD, 200), (40, 40), 36, 2)
    # Inner ring
    pygame.draw.circle(badge_lay, (*GOLD, 80), (40, 40), 28, 1)
    surf.blit(badge_lay, (BCX-40, BCY-40))

    # "18%" inside badge
    f22 = pygame.font.Font(FONT_PATH, 22)
    hero_s = f22.render(f"{int(DEATH_PHASE*100)}%", True, GOLD)
    surf.blit(hero_s, (BCX - hero_s.get_width()//2, BCY - hero_s.get_height()//2 - 6))
    text(surf, f"PIL {DEATH_PILLAR}", 7, center=(BCX, BCY+14), color=(170, 166, 156), shadow=None)

    # Event pips on outer ring — 4 pips equally spaced, dimmed (all sealed)
    PIP_COLORS = [GEYSER_C, CLOWN_C, RAIN_C, SNOW_C]
    for j, pip_c in enumerate(PIP_COLORS):
        angle = _math.radians(45 + j * 90)
        px = int(BCX + _math.cos(angle) * 36)
        py = int(BCY + _math.sin(angle) * 36)
        pip_dim = tuple(c // 3 for c in pip_c)
        pygame.draw.circle(surf, pip_dim, (px, py), 4)
        pygame.draw.circle(surf, (60, 56, 64), (px, py), 4, 1)

    # Stats stack — left-anchored column, below badge, in open sky
    STATS_X = 18
    STATS_Y = 94
    stat_rows = [
        (f"{int(DEATH_PHASE*100)}% OF DAY", GOLD),
        (f"PILLAR {DEATH_PILLAR}", CREAM),
    ...
    # Death callout — borderline chip near parrot
    call_w, call_h = 100, 28
    call_x = int(bx) + 20
    call_y = int(by) - 36
    c_lay = pygame.Surface((call_w, call_h), pygame.SRCALPHA)
    pygame.draw.rect(c_lay, (*INK, 0), c_lay.get_rect())          # transparent fill
    pygame.draw.rect(c_lay, (*GOLD, 80), c_lay.get_rect(), 1, border_radius=3)
    pygame.draw.rect(c_lay, (200, 70, 50, 180), (0, 0, 2, call_h))  # left accent bar
    ...
    # BACK — small left-anchored rounded rect
    back_r = pygame.Rect(18, 582, 80, 24)
