...
def draw_overlay(ss):
    ...
    # AHEAD run — thin, cool, 1px, alpha=90. Reads as a ghost hint, not an achievement.
...
def render_screen():
    ...
    surf.blit(bg, (0, 0))
    # Dark veil on the unearned side (right of death position) — gradient 155→185
    vx = int(DEATH_X)
    for dx in range(14):
        a = int(155 * (1 - dx / 14))
    ...
    # Gradient veil: 155 at the seam, 185 at the right edge
    veil_w = W - vx
    for cx in range(veil_w):
        a = int(155 + 30 * (cx / max(1, veil_w - 1)))
        col_v = pygame.Surface((1, H), pygame.SRCALPHA)
        col_v.fill((6, 8, 20, a))
        surf.blit(col_v, (vx + cx, 0))
    ...
    # DAY COMPLETE — 3-layer aspirational glow visible through the deep veil
    xdc, ydc = pos_u(1.0)
