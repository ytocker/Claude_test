#!/usr/bin/env python3
"""
sun-arc  ·  flight-log arc v2  ·  arc_void round 3 — pitch black void, lone beacon at DAY COMPLETE.
...
def draw_overlay(ss):
    ...
    # NEWBIE PERIOD zone — warm arc connecting start to end, with end tick
    ...
    for p, rad, fn, col in ring_items:
        if p > DEATH_PHASE:
            continue  # Pitch black void — nothing exists past death
        x, y = arc_pos(p, rad)
        if rad != R_INNER:
...
def render_screen():
    ...
    surf.blit(bg, (0, 0))
    # Pitch black void past death — hard seam with feather only at the edge
    vx = int(DEATH_X)
    for dx in range(14):
        a = int(252 * (1 - dx / 14))
    ...
    void_surf = pygame.Surface((W - vx, H), pygame.SRCALPHA)
    void_surf.fill((4, 5, 12, 252))
    surf.blit(void_surf, (vx, 0))
    ...
    # DAY COMPLETE — massive 5-layer beacon, the only light in the void
    xdc, ydc = pos_u(1.0)
    for rad, col_g, peak in ((62, (255, 120, 30), 18), (48, (255, 155, 50), 28),
                              (36, (255, 185, 80), 44), (22, (255, 215, 120), 66),
                              (12, (255, 244, 180), 96)):
        g_dc = soft_glow(rad, col_g, peak=peak, falloff=1.7)
        surf.blit(g_dc, (int(xdc) - rad - 1, int(ydc) - rad - 1), special_flags=pygame.BLEND_ADD)
    pygame.draw.circle(surf, (255, 252, 210), (int(xdc), int(ydc)), 10)
    pygame.draw.circle(surf, (255, 255, 248), (int(xdc) - 1, int(ydc) - 1), 6)
