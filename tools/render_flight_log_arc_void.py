#!/usr/bin/env python3
"""
sun-arc  ·  flight-log arc v2  ·  arc_void — direction B: past death is pure dark.
...
Each direction script (arc_clean, arc_void, etc.) imports this layout and
overrides one dimension.
"""
import os
import math
import random
...
def draw_overlay(ss):
    ...
    # AHEAD run — thin, cool, 1px, alpha=60. Reads as a ghost hint, not an achievement.
    ahead = [PU(u_death + (1.0 - u_death) * i / 150) for i in range(151)]
    pygame.draw.lines(ss, (*COOL, 60), False, ahead, max(1, int(1.0 * k)))
    ...
    pygame.draw.polygon(ss, (*INK, 180),
                        [(x, y - 3.4 * k), (x + 2.6 * k, y), (x, y + 3.4 * k),
                         (x - 2.6 * k, y)], int(2.0 * k))
    # Extra: faint gold disc glow at DAY COMPLETE — visible through the deep veil
    x_dc, y_dc = pos_u(1.0)
    # (drawn on the non-SS surf after downscale — see render_screen)

    # NEWBIE PERIOD bracket — start and end ticks, no fill arc (void style)
    for p_nb in (0.001, NEWBIE_END_PHASE):
        ux_nb, uy_nb = radial_unit(p_nb)
        xnb, ynb = arc_pos(p_nb, R_INNER)
        pygame.draw.line(ss, (*NEWBIE_C, 190),
                         ((xnb - ux_nb * 5.0) * k, (ynb - uy_nb * 5.0) * k),
                         ((xnb + ux_nb * 5.0) * k, (ynb + uy_nb * 5.0) * k), int(2.2 * k))
    ...
    for p, rad, fn, col in ring_items:
        ...
        dim = 0.05 if p > DEATH_PHASE else 1.0
...
def render_screen():
    ...
    surf.blit(bg, (0, 0))
    # Dark veil on the unearned side (right of death position) — stronger alpha=210
    vx = int(DEATH_X)
    for dx in range(14):
        a = int(210 * (1 - dx / 14))
    ...
    # DAY COMPLETE glow through the void
    xdc, ydc = pos_u(1.0)
    g_dc = soft_glow(18, (204, 165, 74), peak=22, falloff=2.0)
    surf.blit(g_dc, (int(xdc) - 19, int(ydc) - 19), special_flags=pygame.BLEND_ADD)
    ...
    text(surf, "NEWBIE", 8, center=(int(nx), int(ny)), color=NEWBIE_C, shadow=None)
