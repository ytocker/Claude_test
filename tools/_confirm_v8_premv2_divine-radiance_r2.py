"""divine-radiance — round_2 final render.

Round_2 polish: sunburst petals brighter (244,168,46 fill), price band brightened
to pearl-white for RARE/EPIC (the band carries the numeral now as bold dark text),
tighter radial aura kill-zone. LEGENDARY rose Zone B unchanged — cross-hue works.
"""
import os
import sys
import math
...
def render_popup(tier):
    ...
    # 2 ── capped outer corona (kill-slab tighter at y=190)
    sc._alpha_aura(big, m(CX), m(DISC_CY), m(62), pal["glow"], peak=80, layers=14)
    kx0, ky0 = m(CX) - m(80), m(190)
    kw, kh = m(160), m(240) - m(190)
    ...
    sc.plain_text(big, NAMES[tier], name_f, (m(CX), m(Y_NAME)),
                  sc.lerp_color(pal["glow"], (255, 255, 255), 0.35), shadow_a=150,
                  weight=m(1.0), keyline=(6, 6, 16), kw=m(1.0))

    # 4 ── Zone A: BRIGHTER pearl-white price band (r2 fix — improved contrast)
    mat = pygame.Surface((m(106), m(34)), pygame.SRCALPHA)
    pygame.draw.rect(mat, (6, 7, 18, 220), mat.get_rect(), border_radius=m(10))
    big.blit(mat, (m(CX) - m(53), m(CHIP_CY) - m(17)))
    band = pygame.Rect(m(CX) - m(51), m(CHIP_CY) - m(14), m(102), m(28))
    sc.chip_body_stops(big, band, m(9),
                       [(0.0, (248, 248, 255)), (0.5, (220, 228, 248)),
                        (1.0, (180, 192, 228))],
                       rim_dark=(56, 64, 100), rim_bright=(255, 255, 255),
                       gloss=48)
    price = PRICES[tier]
    price_f = sc.font(16)
    ...
    sc.plain_text(big, price, price_f,
                  ...
                  (30, 34, 62), shadow_a=0, weight=m(0.7))

    # 5 ── dead-zone medallion (unchanged, works well)
    mcx, mcy = m(CX), m(297)
    pygame.draw.circle(big, (12, 13, 32), (mcx, mcy), m(15))
    pygame.draw.circle(big, CARD_RING_BRIGHT, (mcx, mcy), m(15), max(2, m(2)))
    for k in range(8):
        a = math.pi * 2 * k / 8
        x1, y1 = mcx + m(17) * math.cos(a), mcy + m(17) * math.sin(a)
        x2, y2 = mcx + m(27) * math.cos(a), mcy + m(27) * math.sin(a)
        pygame.draw.line(big, CARD_RING_BRIGHT, (x1, y1), (x2, y2), max(2, m(2)))
    sc.facet_gem(big, mcx, mcy, m(7), pal["gem"], pal["deep"])
    ...
    pygame.draw.rect(big, (6, 7, 18), shelf, width=max(1, m(1)), border_radius=m(14))

    buy = pygame.Rect(m(BUY_CX) - m(BTN_W) // 2, m(BTN_CY) - m(BTN_H) // 2,
                      m(BTN_W), m(BTN_H))
    sc.chip_body_stops(big, buy, m(BTN_RAD),
                       [(0.0, (224, 180, 96)), (1.0, (176, 120, 40))],
                       rim_dark=GOLD_A_RIM_DARK, rim_bright=GOLD_A_RIM_BRIGHT, gloss=32)
    ...
    sc._dark_chip_body(big, can, m(BTN_RAD),
                       ...
                       rim_dark=CARD_RING_DEEP, rim_bright=CARD_RING_BRIGHT, gloss=14)
    sc.plain_text(big, "CANCEL", sc.font(11), can.center, (170, 180, 214),
                  shadow_a=0, weight=m(0.7))

    # 7 ── Zone B: flank gems + ribbon (LEGENDARY rose cross-hue)
    ...
    # 8 ── STATEMENT: 16 BRIGHTER gold sunburst petals (r2: fill=244,168,46)
    ...
    for k in range(16):
        ...
        p4 = (hx + tip_r * math.cos(ang - a_tip),
              hy + tip_r * math.sin(ang - a_tip))
        pygame.draw.polygon(big, (244, 168, 46), [p1, p2, p3, p4])   # brighter in r2
        pygame.draw.polygon(big, (80, 44, 6), [p1, p2, p3, p4], width=max(2, m(2)))

    # 9 ── inner corona tight halo
    sc._alpha_aura(big, hx, hy, m(58), pal["glow"], peak=150, layers=10)
