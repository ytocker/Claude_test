#!/usr/bin/env python3
"""
lantern_reach  ·  flight-log arc concept  ·  round 1
...
# ── Skybit background (copied from render_flight_log_progress_sun_arc.py) ────
...
DEATH_X, DEATH_Y = arc_pos(DEATH_PHASE)   # ≈ (92, 279)
...
def render():
    ...
    # Dark overlay to set the cold, unlit mood (the lantern hasn't lit this yet)
    ...
    # CLOWN notch at R=169 (inward 6px)
    draw_notch(surf, 0.403, radius=169, size=3)
    # RAIN notch at R=181 (outward 6px)
    draw_notch(surf, 0.430, radius=181, size=3)
    # SNOWSTORM notch at R=175
    draw_notch(surf, 0.820, radius=175, size=3)
    ...
    # Additional secondary warm pools along the flown arc to reinforce the lit corridor
    ...
    # 5. Draw the flown arc in GOLD (p=0.0 to p=0.184) — 3px segments
    steps = 80
    for i in range(steps):
        pa = DEATH_PHASE * i       / steps
        pb = DEATH_PHASE * (i + 1) / steps
        t  = i / steps
        # Warm gold: brighter near the death point
        brightness = int(200 + 55 * t)
        col = (brightness, int(160 + 46 * t), int(60 + 32 * t))
        x0, y0 = arc_pos(pa)
        x1, y1 = arc_pos(pb)
        pygame.draw.line(surf, INK, (int(x0), int(y0)), (int(x1), int(y1)), 5)
        pygame.draw.line(surf, col, (int(x0), int(y0)), (int(x1), int(y1)), 3)

    # 6. DAY COMPLETE marker at (355, 430)
    ...
    # Dark header band
    SCRIM = (18, 14, 26)
    pygame.draw.rect(surf, SCRIM, (0, 0, W, 72))
    fade = pygame.Surface((W, 10), pygame.SRCALPHA)
    for i in range(10):
        fade.fill((*SCRIM, int(230 * (1 - i / 10) ** 1.4)), pygame.Rect(0, i, W, 1))
    surf.blit(fade, (0, 72))
    alpha_line(surf, (*GOLD, 140), (0, 72), (W - 1, 72), 1)

    # "FLIGHT LOG" header
    text(surf, "FLIGHT LOG", 22, center=(W // 2, 28), color=GOLD,
         track=4, shadow=None)
    # Subtitle
    text(surf, f"DAY {DAY_N}  ·  PILLAR {DEATH_PILLAR}  ·  0:{TIME_ALIVE:02d}", 11,
         center=(W // 2, 52), color=CREAM, shadow=None)

    # "18.4% OF THE DAY FLOWN" near top
    pct   = f"{DEATH_PHASE * 100:.1f}%"
    f_big = font(20)
    f_sml = font(10)
    w_pct  = f_big.size(pct)[0]
    w_tail = f_sml.size("  OF THE DAY FLOWN")[0]
    x0_pct = (W - (w_pct + w_tail)) / 2
    text(surf, pct, 20, midleft=(x0_pct, 90), color=GOLD,
         shadow=(0, 0, 0, 170))
    text(surf, "  OF THE DAY FLOWN", 10,
         midleft=(x0_pct + w_pct, 92), color=CREAM,
         shadow=(0, 0, 0, 160))
    alpha_line(surf, (*GOLD, 80),
               (int(x0_pct), 106),
               (int(x0_pct + w_pct + w_tail), 106), 1)

    # Small callout chip near death point
    chip_surf = pygame.Surface((84, 28), pygame.SRCALPHA)
    pygame.draw.rect(chip_surf, (20, 14, 30, 220), chip_surf.get_rect(),
                     border_radius=6)
    pygame.draw.rect(chip_surf, (*GOLD, 100), chip_surf.get_rect(),
                     width=1, border_radius=6)
    surf.blit(chip_surf, (int(dx) + 10, int(dy) - 14))
    text(surf, "LIGHT SOURCE", 8,
         midleft=(int(dx) + 16, int(dy)),
...
def main():
    surf = render()
    out = os.path.join(ROOT, "docs", "flight_log_arc", "lantern_reach", "round_1.png")
