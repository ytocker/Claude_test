#!/usr/bin/env python3
"""
arc_count/monolith round 2 — banner shrink, stat hierarchy, callout fix, BACK anchor.

Changes from round 1:
  • Banner 74 → 48 px, hairline at y=48, text centred at y=24
  • Hero "18%" at 80 pt with track=-3 (tight optical kerning)
  • "OF THE DAY FLOWN" lifted from 8 pt to 10 pt
  • Stats: DAY row deleted; values 9 pt → 15 pt CREAM; keys dimmed to (120,118,130);
    row pitch 18 → 22 px; y_stats pulled up so stat zone ends before y=220
  • Events/unsealed line: color lifted to (150,155,170), size 8 → 9 pt
  • Death callout dropped to by+14 to clear the TIME stat row (no collision)
  • DAY COMPLETE: midright=(W-18, ...) instead of W-6
  • BACK anchored to y=608; 1 px GOLD hairline at y=590 (alpha≈38, 15 %); 44 px touch band
"""
import os
import math
import random
...
def render_screen():
    ...
    # Note 2: Banner shrunk to 48 px; text centred at y=24; gold hairline at y=48
    BANNER_H = 48
    ...
    text(surf, "FLIGHT LOG", 21, midleft=(18, 24), color=GOLD, track=3, shadow=None)
    text(surf, f"DAY {DAY_N}", 21, midright=(W - 18, 24), color=GOLD, track=2, shadow=None)

    # Note 3: Hero "18%" at 80 pt with track=-3 (tight optical spacing via text() helper)
    pct_str = f"{int(DEATH_PHASE * 100)}%"
    hero_h = font(80).get_height()
    hero_top_y = BANNER_H + 8          # 56 px from top
    hero_rect = text(surf, pct_str, 80,
                     center=(W // 2, hero_top_y + hero_h // 2),
                     color=GOLD, track=-3, shadow=None)

    # Note 5: "OF THE DAY FLOWN" caption lifted from 8 pt → 10 pt
    cap_h = font(10).get_height()
    cap_top_y = hero_rect.bottom + 6
    text(surf, "OF THE DAY FLOWN", 10,
         center=(W // 2, cap_top_y + cap_h // 2),
         color=(180, 175, 165), track=3, shadow=None)

    # Note 1 & 4: Stats block — DAY row deleted; only PILLAR + TIME remain.
    #   Values: 9 pt → 15 pt CREAM.  Keys: 8 pt (120,118,130) — dimmer.
    #   Row pitch: 18 → 22 px.  y_stats pulled up so stat zone ends before y=220.
    y_stats = cap_top_y + cap_h + 8
    stat_pairs = [("PILLAR", str(DEATH_PILLAR)), ("TIME", f"0:{TIME_ALIVE:02d}")]
    for i, (k_lbl, v_lbl) in enumerate(stat_pairs):
        ys = y_stats + i * 22
        text(surf, k_lbl, 8, midright=(W // 2 - 8, ys),
             color=(120, 118, 130), track=2, shadow=None)
        text(surf, v_lbl, 15, midleft=(W // 2 + 8, ys),
             color=CREAM, track=1, shadow=None)

    # Note 5: Events / unsealed line — color lifted to (150,155,170); size 8 → 9 pt
    events_y = y_stats + len(stat_pairs) * 22 + 14
    text(surf, "4 EVENTS UNSEALED AHEAD", 9, center=(W // 2, events_y),
         color=(150, 155, 170), track=2, shadow=None)

    # Note 6: DAY COMPLETE — midright=(W-18, ...) instead of W-6
    xdc, ydc = pos_u(1.0)
    text(surf, "DAY COMPLETE", 8, midright=(W - 18, int(ydc) + 18), color=GOLD, shadow=None)

    # Note 1: Death callout — dropped to by+14 to clear stat rows (was by-40)
    call_w, call_h_box = 118, 30
    call_x = int(bx) + 18
    call_y = int(by) + 14
    call_surf = pygame.Surface((call_w, call_h_box), pygame.SRCALPHA)
    pygame.draw.rect(call_surf, (*SCRIM, 220), call_surf.get_rect())   # no border_radius
    pygame.draw.rect(call_surf, (*GOLD, 90), call_surf.get_rect(), 1)  # 1 px gold border
    surf.blit(call_surf, (call_x, call_y))
    text(surf, "ENDED HERE", 9, midleft=(call_x + 8, call_y + 10), color=GOLD, shadow=None)
    text(surf, f"PILLAR {DEATH_PILLAR}", 8,
         midleft=(call_x + 8, call_y + 22), color=(170, 165, 155), shadow=None)

    # Note 6: BACK anchored to y=608.
    #   1 px GOLD hairline at y=590 (15 % alpha ≈ 38) separates BACK from content above.
    #   44 px touch target band: y=590 … y=634 — BACK text at y=608 sits centred within it.
    alpha_line(surf, (*GOLD, 38), (0, 590), (W - 1, 590), 1)
    _touch_band = pygame.Rect(0, 590, W, 44)   # invisible sizing reference, not drawn
    text(surf, "BACK", 13, center=(W // 2, 608), color=GOLD, track=4, shadow=None)
