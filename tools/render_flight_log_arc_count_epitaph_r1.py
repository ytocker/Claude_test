#!/usr/bin/env python3
"""
arc_count / EPITAPH — round 1

Pure typographic eulogy. No chips, no connector lines, no rounded containers,
no outlines. The screen reads like a beautiful in-memoriam verse centered in
the open sky above the arc.

Background, arc, parrot, sun all carried forward from round 7 unchanged.
"""
import os
import math
import random
...
def render_screen():
    ...
    # ── VERSE BLOCK — centered in the sky, around y=96–210 ──
    CX_TEXT = W // 2  # centered on screen

    # Title
    TITLE_Y = 98
    text(surf, "THE PARROT FLEW", 11, center=(CX_TEXT, TITLE_Y), color=GOLD, track=5,
         shadow=(0, 0, 0, 120))

    # Rule 1
    RULE1_Y = TITLE_Y + 18
    RULE_W = 120
    alpha_line(surf, (*GOLD, 70), (CX_TEXT - RULE_W // 2, RULE1_Y),
               (CX_TEXT + RULE_W // 2, RULE1_Y), 1)

    # Hero "18%" — CREAM (stone-cut), large, with black incised shadow
    f36 = pygame.font.Font(FONT_PATH, 36)
    hero_s = f36.render(f"{int(DEATH_PHASE * 100)}%", True, CREAM)
    hero_x = CX_TEXT - hero_s.get_width() // 2
    hero_y = RULE1_Y + 10

    # Incised shadow: draw text in dark color slightly offset behind
    shadow_s = f36.render(f"{int(DEATH_PHASE * 100)}%", True, (10, 8, 16))
    surf.blit(shadow_s, (hero_x + 1, hero_y + 1))
    surf.blit(shadow_s, (hero_x + 1, hero_y + 2))
    surf.blit(hero_s, (hero_x, hero_y))

    # "OF THE DAY FLOWN" below hero number
    OF_Y = hero_y + hero_s.get_height() + 4
    ...
    alpha_line(surf, (*GOLD, 50), (CX_TEXT - RULE_W // 2, RULE2_Y),
               (CX_TEXT + RULE_W // 2, RULE2_Y), 1)

    # Coda
    text(surf, "4 ENCOUNTERS AWAIT", 8, center=(CX_TEXT, RULE2_Y + 12),
         color=(110, 108, 122), track=2, shadow=None)

    # ── "FELL AT PILLAR" near parrot — bare text, no container ──
    text(surf, f"FELL AT PILLAR {DEATH_PILLAR}", 8,
         midleft=(int(bx) + bw // 2 + 6, int(by)),
         color=(200, 75, 55), shadow=None)

    # ── BACK — em-dash flanked, dim, at bottom ──
    back_text = "— BACK —"
    back_lay = pygame.Surface((W, 20), pygame.SRCALPHA)
    f_back = font(10)
    bs = f_back.render(back_text, True, CREAM)
    bsx = (W - bs.get_width()) // 2
    back_lay.blit(bs, (bsx, 0))
    back_lay.set_alpha(153)  # 60% opacity
    surf.blit(back_lay, (0, 592))
