...
import pygame
pygame.init()
from tools.fly_candidates.pop_v2 import build

# Bright daytime sky swatch — a noir black fly is meant to read against the
# game's lit sky, so the truth check is on light, not dusk.
def sky(surf):
    r = surf.get_rect()
    for y in range(r.height):
        t = y / r.height
        c = (int(150 + 60 * t), int(200 + 30 * t), int(240 - 10 * t))
...
# 40px NEAREST truth strip — the real in-game read, on a lit sky-blue field.
truth = pygame.Surface((4 * 60 + 10, 70), pygame.SRCALPHA)
truth.fill((176, 214, 235))
for fi in range(4):
    fr = build(fi, 0.0)
    bb = fr.get_bounding_rect()
    fr2 = fr.subsurface(bb).copy() if (bb.width and bb.height) else fr
...
out = "/home/user/skybit/docs/store_redesign/animal/fly/pop_v2/round_1.png"
