...
abdomen, cyan halftone wings and two enormous dotted goggle eyes. Same
Lichtenstein grammar as Design 5 — flat colour blocks, uniform 2px black ink
loop on every element, hex-packed Ben-Day dot fills — but tuned to a cold
blue/lime charge instead of the warm red/yellow original, so the 40px read
carries on the ink loop plus the goggle eyes.
...
# so the downscaled outer arc stays clean — no faceted ink spikes — before it
# takes its lime halftone + ink loop. Reads as a translucent fan, not a
# patterned bowtie.
_WING_ROOT = (8, 24)


def _wing_surface():
    """One rounded pop-art fan: a single smooth cyan membrane ellipse, a lime
    halftone, two bold black veins, then a 2px ink loop."""
    ...
    # Lime halftone sits inside the cyan membrane — the wing's pop accent that
    # still recedes behind the eyes at truth scale because the dots stay sparse.
...
def build_pop_v1(wing_angle_deg):
    ...
    # 2px ink seam at the waist where blue meets lime — a divider, not a full
    # stacked-bands ladder, so the barrel silhouette still wins.
    ...
    # Notch-breaks over the blue dome's top push the body from "beetle" to
