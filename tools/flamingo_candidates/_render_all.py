...
— one labeled sheet under docs/store_redesign/animal/flamingo/.

Run: ``PYTHONPATH=. SDL_VIDEODRIVER=dummy python tools/flamingo_candidates/_render_all.py [round_tag]``
"""
...
DESIGNS = [
    ("design_1", "ROUNDED TUFT"),
    ("design_2", "LAYERED FEATHERS"),
    ("design_3", "SLEEK SWEPT"),
    ("design_4", "UPSWEPT WISPS"),
    ("design_5", "FLUFFY POMPOM"),
]

OUT_DIR = "docs/store_redesign/animal/flamingo"
...
def render_design(slug, name):
    build = importlib.import_module(
        f"tools.flamingo_candidates.{slug}").build
    ...
    sheet.fill((18, 16, 26))
    _label(sheet, f"FLAMINGO TAIL  ·  {name}  ·  {ROUND.replace('_', ' ')}",
           pad, 16, size=26)
