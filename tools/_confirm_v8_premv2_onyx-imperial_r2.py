"""onyx-imperial — round_2 final render.

Round_2 fix: Zone A luminance diff vs Zone B was only 1.5 lum points (both
near-black). Deep-violet enamel replaced with a BRIGHT GOLD chip so Zone A reads
dramatically above Zone B. Corner gems enlarged to r=m(18) for bolder WOW.
"""
...
# Enlarged corner anchors (r2: r=18 vs r=15)
...
def _setting_claw(surf, gx, gy):
    ...
    ux, uy = dx / L, dy / L
    tipx, tipy = bx + ux * m(32), by + uy * m(32)
    px, py = -uy, ux
    bw, tw = m(8) / 2, m(2.5) / 2
    quad = [
        (bx + px * bw, by + py * bw),
    ...
    pygame.draw.polygon(surf, CLAW_SEAM, quad, width=max(2, m(2)))
    pygame.draw.circle(surf, CLAW_PRONG, (int(tipx), int(tipy)), m(3))
...
def render_popup(tier):
    ...
    # 2 · claws → corner gems (r=18 in r2)
    for gx, gy in CORNERS:
        _setting_claw(big, gx, gy)
    for gx, gy in CORNERS:
        sc.facet_gem(big, m(gx), m(gy), m(18), pal["gem"], pal["deep"])

    # 3 · name
    nf = _fit_font(NAMES[tier], NAME_FS, m(POP_W) - m(40))
    sc.plain_text(big, NAMES[tier], nf, (m(CX), m(Y_NAME)), (220, 225, 240),
                  shadow_a=150, weight=m(0.9), keyline=(6, 6, 16), kw=m(1.0))

    # 4 · Zone A — BRIGHT GOLD chip (r2 fix: replaces dark violet)
    plate = pygame.Rect(0, 0, m(110), m(30))
    plate.center = (m(CX), m(CHIP_CY))
    sc.chip_body_stops(big, plate, m(9),
                       [(0.0, (255, 230, 140)), (0.45, (230, 180, 70)), (1.0, (168, 110, 24))],
                       GOLD_A_RIM_DARK, GOLD_A_RIM_BRIGHT, gloss=38)
    num = PRICES[tier]
    pf = sc.font(16)
    ...
    sc.plain_text(big, num, pf,
                  ...
                  (40, 22, 4), shadow_a=0, weight=m(0.9))

    # 5 · dead-zone lozenge (unchanged)
    diamond = [(m(130), m(282)), (m(147), m(297)), (m(130), m(312)), (m(113), m(297))]
    pygame.draw.polygon(big, (14, 14, 26), diamond)
    pygame.draw.polygon(big, CARD_RING_BRIGHT, diamond, width=max(2, m(2)))
    sc.facet_gem(big, m(130), m(297), m(8), pal["gem"], pal["deep"])
    ...
    # 7 · Zone B — flank gems + notched ribbon (teal cross-hue for LEGENDARY)
    ...
    # 8 · 2-ring aura'd hero (statement ≥60px)
    sc._alpha_aura(big, m(CX), m(DISC_CY), m(73), pal["glow"], peak=70, layers=16)
    pygame.draw.circle(big, (3, 3, 10), (m(CX), m(DISC_CY)), m(69), max(1, m(1)))
    pygame.draw.circle(big, (*CARD_RING_BRIGHT, 235), (m(CX), m(DISC_CY)), m(66), max(2, m(2)))
    pygame.draw.circle(big, (*pal["gem"], 170), (m(CX), m(DISC_CY)), m(58), max(1, m(2)))
...
def main():
    ...
    title = title_f.render("CONFIRM PURCHASE  ·  onyx-imperial  ·  round_2",
                           True, (236, 202, 116))
