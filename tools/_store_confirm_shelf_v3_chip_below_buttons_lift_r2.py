"""Chip-below gems — option B: buttons-lift, R2.

Buttons hold the shelf TOP; the price chip sits low, right above the bottom-
frame gems. Reversing the vertical stack puts the call-to-action first and reads
the price as a footnote to the buttons.

R2 addresses the art-director notes on R1:
- Shelf expanded SHELF_H 87→97 (shelf_bottom 313→323) so buttons+chip+gems stop
  pinching each other; buttons stay put at the shelf top.
- Chip lifted CHIP_CY 287→283 (chip_top=268, chip_bottom=298=gem_top): a 4px
  btn→chip breath, and the warm gem aura now grazes the chip's lower edge for a
  "gem nestled above the plate" read.
- Chip fill re-hued to a COOL indigo that is ~15-20% lighter than the shelf
  interior, plus a top sheen, so the plate reads as its own object and its cool
  value separates it from the warm amber gems below (no gold-on-gold smear).
- BUY given brightness hierarchy over CANCEL so the CTA is the dominant read
  before the price.

Layout: SHELF_Y=226, SHELF_H=97, BTN_CY=249, CHIP_CY=283, CHIP_H=30,
BOT_GEM_CY=309. Gem centers (33,309)/(167,309); BUY center (58,249); chip
center (100,283).

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/buttons-lift/round_2.png
"""
import os, sys, math
...
# Option B (chip-below): buttons at shelf TOP, chip at shelf BOTTOM near gems.
# Taller shelf (97) buys vertical room so buttons/chip/gems no longer pinch.
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 226, 174, 97
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 249, 8
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    if locked:
        ...
        # Deliberately dimmer than BUY so the CTA wins the first read.
        stops   = [(0.0, (24, 26, 58)), (1.0, (13, 15, 40))]
        lab_col = (140, 146, 190)
        sheen   = 12
    else:
        # Brightest button on the shelf — BUY is the dominant call-to-action.
        stops   = [(0.0, (48, 50, 100)), (1.0, (28, 30, 68))]
        lab_col = (220, 224, 252)
        sheen   = 28
    ...
    if locked:
        ...
        sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                     (*sc.CARD_RING_BRIGHT, 190), w=max(1, m(1.4)))
    else:
        # A thicker, fully-opaque bright rim gives BUY extra visual weight.
        sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    # Cool indigo plate, lifted ~15-20% brighter than the shelf interior so it
    # reads as its own object and its cool value separates from the warm gems.
    face_stops = ([(0.0, (46, 48, 96)), (1.0, (30, 32, 74))] if affordable
                  else [(0.0, (40, 42, 66)), (1.0, (26, 28, 48))])
    big.blit(sc.vgrad_stops(chip.w, chip.h, crad, face_stops, 255), chip.topleft)
    sc.top_sheen(big, chip, crad, m(10), peak=22)

    if affordable:
        sc.bevel_rim(big, chip, crad, sc.CARD_RING_DEEP,
                     (*sc.CARD_RING_BRIGHT, 210), w=max(1, m(1.4)))
    else:
...
def _draw_shelf(big, affordable):
    ...
    # Buttons first (shelf top), then chip (shelf bottom) so the chip nests
    # cleanly above the bottom gems as the last shelf element.
...
def _draw_bottom_gems(big, affordable):
    for gem_cx in [m(BOT_GEM_LX), m(BOT_GEM_RX)]:
        if affordable:
            sc._alpha_aura(big, gem_cx, m(BOT_GEM_CY), m(16), PAL["glow"], peak=55, layers=14)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip-below B: buttons-lift R2  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "buttons-lift", "round_2.png"))
...
# Panel-local checks (panel origin at 18,54 on the canvas)
img     = canvas
chip_px = img.getpixel((18 + 100, 54 + CHIP_CY))
btn_px  = img.getpixel((18 + BUY_CX, 54 + BTN_CY))
gem_l   = img.getpixel((18 + BOT_GEM_LX, 54 + BOT_GEM_CY))
print(f"chip:{chip_px} btn:{btn_px} gem:{gem_l}")
print(f"chip centre: {chip_px}  {'ok' if chip_px != BG else 'WARN bg'}")
print(f"BUY centre: {btn_px}  {'ok' if btn_px != BG else 'WARN bg'}")
print(f"gem left: {gem_l}  {'ok' if gem_l != BG else 'WARN bg'}")
