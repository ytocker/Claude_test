#!/usr/bin/env python3
"""
coin-tab  ·  store buy-confirmation popup v3  ·  round 1

Concept (NEW — replaces price-ribbon): the price display and the action
buttons read as ONE connected object. A folder-tab price container hangs
notched into the top edge of a unified BUY|CANCEL button bar — manila-folder
style — so content + controls form a single piece instead of two stacked
elements. BUY and CANCEL are the SAME shape sharing the bar's outer corners,
split only by a recessed bevel channel; the difference is fill + label colour.
"""
import math
import os
import sys
...
def _channel(big, cx_d, y0, y1):
    """Recessed vertical bevel channel dividing BUY from CANCEL: a dark valley
    with a bright lip on each side so the split reads as a moulded groove, not
    a painted line — the two halves are one piece, notched apart."""
    lip = max(1, sc.m(1))
    val = max(2, sc.m(2.5))
    off = sc.m(1.8)
...
def _tab_face(big, tab, top_stops, r_top, border_col):
    ...
    # matte top highlight so the tab lifts a touch off the bar without going glossy
...
def _patched_draw_confirm(self, surf):
    ...
    BAR_L = CX - BAR_W // 2               # 16
    BUY_CX = BAR_L + BAR_W // 4           # 58 — centre of left half
    CAN_CX = BAR_L + (BAR_W * 3) // 4     # 142 — centre of right half
    BTN_CY = (BTN_TOP + BTN_BOT) // 2     # 270

    TAB_W, TAB_H, TAB_TOP, TAB_RAD = 96, 42, 210, 8
    TAB_CY = 231                          # coin+numeral baseline on tab face
    HIT_H = 40
    ...
    if affordable:
        ...
        # BUY cell goes inert cool-slate; CANCEL half is unchanged slate
    ...
    _channel(big, m(CX), m(BTN_TOP) + m(3), m(BTN_BOT) - m(3))
    ...
    # folder-tab hanging into the bar's top edge
    tab = pygame.Rect(m(CX - TAB_W // 2), m(TAB_TOP), m(TAB_W), m(TAB_H))
    sc.drop_shadow(big, tab, m(TAB_RAD), blur=m(4), alpha=120, dy=m(2))
    if affordable:
...
hdr = store_mod._font(16, True).render(
    "coin-tab R1 · FOLDER-TAB PRICE + ONE-PIECE BUY|CANCEL BAR",
    True, (220, 190, 100))
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_popup_v3", "coin-tab", "round_1.png")
