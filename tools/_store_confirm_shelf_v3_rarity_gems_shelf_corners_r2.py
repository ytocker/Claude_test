"""Round 2 — concept A "shelf-corners" (rarity-gems, store confirm popup v3).

Two facet gems pinned in the shelf's TOP rounded corners, clear of the BUY/CANCEL
buttons that occupy the bottom strip. AFFORDABLE shows the epic-rarity hue with a
fuller aura (peak=40, r=12); UNAFFORDABLE swaps to dead slate + half aura so the
decoration reads "locked" without shouting. Base card/shelf/chip/button furniture is
copied from the lock showcase so this concept sits in its real context.

Output -> docs/store_confirm_shelf_v3/rarity-gems/shelf-corners/round_2.png
"""
import os, sys, math
...
# Shelf top-corner gem positions: 9 px inside the shelf's top edge (SHELF_Y=235)
# so they sit flush with the shelf's rounded-corner inset, well clear of the
# BUY/CANCEL buttons that begin at y=287.
CORNER_GEM_CY  = 244   # SHELF_Y + 9
CORNER_GEM_LX  = 22    # left corner
CORNER_GEM_RX  = 178   # right corner
CORNER_GEM_R   = 9
CORNER_AURA_R  = 12    # wider halo for readability at this exposed position
CORNER_AURA_PK = 40    # brighter peak so the jewel corner reads from a glance
...
def _draw_corner_gems(big, affordable):
    # Concept A: two gems pinned in the shelf's TOP rounded corners — 9 px below the
    # shelf top edge — so they anchor the plaque without colliding with the BUY/CANCEL
    # buttons at the bottom strip. Aura wells up first so the halo sits behind the
    # facet body, never washing over it.
    if affordable:
        base, deep = PAL["gem"], PAL["deep"]
        glow, peak = PAL["glow"], CORNER_AURA_PK
    else:
        base, deep = MUTED_BASE, MUTED_DEEP
        glow, peak = (90, 92, 110), 14

    for cx in (CORNER_GEM_LX, CORNER_GEM_RX):
        sc._alpha_aura(big, m(cx), m(CORNER_GEM_CY), m(CORNER_AURA_R), glow,
                       peak=peak, layers=12)
        sc.facet_gem(big, m(cx), m(CORNER_GEM_CY), m(CORNER_GEM_R), base, deep)
...
def render_popup(name, affordable):
    ...
    # Concept-A gems sit on the shelf furniture, drawn BEFORE the hero disc so the
    # hero aura/glass layers stay the topmost focal element.
    _draw_corner_gems(big, affordable)
...
CANVAS_H = MARGIN + HDR_H + POP_H + MARGIN         # 412
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2 - 4), "A — shelf-corners",
          fill=CREAM, font=fnt_hdr, anchor="mm")

PANELS = [("AFFORDABLE", True), ("UNAFFORDABLE", False)]
panel_top = MARGIN + HDR_H

for ci, (state_lab, affordable) in enumerate(PANELS):
    px = MARGIN + ci * (POP_W + GAP)
    surf  = render_popup("TEMPEST", affordable)
    raw   = pygame.image.tostring(surf, "RGB")
    panel = Image.frombytes("RGB", (POP_W, POP_H), raw)
    canvas.paste(panel, (px, panel_top))

    # ID badge — dark pill top-left, cream "A" with lavender outline
    bx, by = px + 4, panel_top + 4
    bw = fnt_bdg.getlength("A") + 10
    draw.rounded_rectangle([bx, by, bx + bw, by + 16], radius=5,
                           fill=(24, 22, 38), outline=LAVEND, width=1)
    tx, ty = bx + bw / 2, by + 8
    for ox, oy in ((-1, 0), (1, 0), (0, -1), (0, 1)):
        draw.text((tx + ox, ty + oy), "A", fill=LAVEND, font=fnt_bdg, anchor="mm")
    draw.text((tx, ty), "A", fill=CREAM, font=fnt_bdg, anchor="mm")

    # state caption, bottom-centred inside the panel margin
    draw.text((px + POP_W // 2, panel_top + POP_H - 10), state_lab,
              fill=(160, 176, 214) if affordable else (150, 150, 162),
              font=fnt_lab, anchor="mm")

OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "rarity-gems",
                   "shelf-corners", "round_2.png")
...
# ── PIL verification ──────────────────────────────────────────────────────────
# Top-corner gems at (22,244) and (178,244) must be non-background.
# Old bottom-corner positions (27,314) and (173,314) must NOT be gem pixels
# — they fall inside button/shelf-bottom territory now.
BG = (8, 8, 20)
surf = render_popup("TEMPEST", True)
raw  = pygame.image.tostring(surf, "RGB")
aff  = Image.frombytes("RGB", (POP_W, POP_H), raw)

print("-- top-corner gem positions (must be non-bg) --")
for (x, y) in [(CORNER_GEM_LX, CORNER_GEM_CY), (CORNER_GEM_RX, CORNER_GEM_CY)]:
    p = aff.getpixel((x, y))
    status = "ok non-bg" if p != BG else "WARN bg"
    print(f"  affordable gem ({x},{y}): {p}  {status}")

print("-- old bottom-corner positions (must NOT be gem) --")
for (x, y) in [(27, 314), (173, 314)]:
    p = aff.getpixel((x, y))
    print(f"  old pos ({x},{y}): {p}  (button/bg territory)")
