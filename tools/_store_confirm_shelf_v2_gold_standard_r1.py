"""Round 1 — shelf concept 'gold-standard' (store_confirm_shelf_v2 series).

The minimal change: both BUY and CANCEL take the SAME golden card-body bevel
(CARD_RING_DEEP → CARD_RING_BRIGHT) instead of the current cool-purple bevel,
and the cream coin chip trades its flat brown outline for the same golden
bevel rim. Button fills (indigo) and chip face (amber) are untouched — the
whole shelf now reads as one gilded family with the card frame around it.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE. Verify with PIL, never by viewing.
Output → docs/store_confirm_shelf_v2/gold-standard/round_1.png
"""
import os, sys, math
...
def _btn(big, btn_rect, rad, label, locked=False):
    # Fills stay exactly as the current shelf; only the RIM turns gold so the
    # buttons join the card's gilded family instead of ringing themselves in
    # a competing cool-purple bevel.
    if locked:
    ...
    # Same golden bevel as the card body — the one line that defines the concept.
    sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                 (*sc.CARD_RING_BRIGHT, 200), w=max(1, m(1.6)))
...
def _draw_chip(big, cx, cy, price, affordable):
    # Matte amber display pill; the plain brown outline is swapped for the same
    # golden bevel rim used everywhere else so the chip is gilded, not fenced.
    ...
    if affordable:
        sc.bevel_rim(big, chip, crad, sc.CARD_RING_DEEP,
                     (*sc.CARD_RING_BRIGHT, 180), w=max(1, m(1.4)))
    else:
        # Grey bevel keeps the same rim SILHOUETTE while the chip is inert.
...
def _draw_shelf(big, affordable):
    # Inset recessed tray carried over unchanged from the current shelf — the
    # only edits in this concept live on the chip + buttons that sit in it.
    ...
    _btn(big, can, brad, "CANCEL", locked=False)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gold-standard  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v2", "gold-standard", "round_1.png")
