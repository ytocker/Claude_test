"""BUY-button differentiation: coin-medallion-tab (D).
...
Output → docs/store_confirm_shelf_v3/buy-fancy/coin-medallion-tab/round_1.png
"""
import os, sys, math
...
def _buy_btn(big, btn_rect, rad, locked):
    ...
    sc.plain_text(big, "BUY", sc.font(14),
                  (btn_rect.centerx + m(8), btn_rect.centery), (210, 205, 240),
                  shadow_a=110, weight=m(0.8), keyline=(8, 6, 20), kw=m(0.9))
    # Coin at x=24 (left edge 24-10=14) clears the shelf wall at x=13 by 1px.
    sc.coin_glyph(big, m(24), m(302), m(10))
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "coin-medallion-tab (D) r1  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   ...
                   "coin-medallion-tab", "round_1.png")
...
print(f"BUY center ({BUY_CX},{BTN_CY}): {buy_px}  → should be indigo-ish")
print(f"CANCEL center ({CAN_CX},{BTN_CY}): {can_px}")
pip_px = aff.getpixel((m(24) // 2, BTN_CY))
print(f"coin pip ({m(24)//2},{BTN_CY}): {pip_px}  → "
      f"{'OK non-bg' if pip_px != (8, 8, 20) else 'WARN bg'}")
