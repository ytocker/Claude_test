#!/usr/bin/env python3
"""
arc_count/codex round 2 — navigator's logbook, polish pass.

Changes from round 1:
1. Scrim card (10,10,18) @ alpha 205 behind hero+table block (12,78,336,220)
2. Values right-aligned to x=W-24 instead of fixed mid-column
3. Value font 13pt; key color (130,126,118) 8pt; ROW_H=26
4. Hero "18%" in GOLD; labels 10pt, baseline-aligned to numeral baseline
5. PILLAR row: key "FELL AT PILLAR", value red (220,80,60), accent bar 3px
6. DAY row removed; 0:47 removed from banner; banner 74→56px, text at y=28
7. DAY COMPLETE: midright=(W-18,…), track=2, soft shadow 1px offset (6,8,14)
"""
import os
import math
import random
...
def draw_ghost_night(surf):
    """The night the run never reached: faint stars and an unrisen moon."""
...
def draw_overlay(ss):
    ...
    # AHEAD run — thin, cool, ~35% alpha, no glow, no keyline.
    ...
    # NEWBIE PERIOD zone (p=0.0–0.20): warm arc on R_INNER
    ...
    # Event glyphs at true angular positions
    ring_items = [
...
def trail_bloom(surf):
    """Warm haze under the flown run only."""
    u_death = ease(DEATH_PHASE)
    for i in range(30):
        t = i / 29
...
def draw_sun(surf, ss):
    """Pale and hazed: the sun is in the veiled sky now."""
    x, y = pos_u(sun_u())
...
def draw_macaw(surf):
    """Single-frame macaw canted along the tangent, ink-keylined."""
    tilt = arc_tangent_deg(DEATH_PHASE)
...
def render_screen():
    ...
    # ── Banner (y=0–56) — shrunk from 74px; text centred at y=28 (note 6) ──
    BANNER_H = 56
    pygame.draw.rect(surf, SCRIM, (0, 0, W, BANNER_H))
    fade = pygame.Surface((W, 10), pygame.SRCALPHA)
    for i in range(10):
        fade.fill((*SCRIM, int(238 * (1 - i / 10) ** 1.4)), pygame.Rect(0, i, W, 1))
    surf.blit(fade, (0, BANNER_H))
    # Hairline at y=56 (note 6)
    alpha_line(surf, (*GOLD, 150), (0, BANNER_H), (W - 1, BANNER_H), 1)
    text(surf, "FLIGHT LOG", 15, midleft=(18, 28), color=GOLD, track=3, shadow=None)
    text(surf, f"DAY {DAY_N}", 15, midright=(W - 18, 28), color=CREAM, shadow=None)
    # "0:47" removed from banner (note 6)

    # ── Scrim card behind hero+table block (note 1) — drawn BEFORE text ──────
    card_surf = pygame.Surface((336, 220), pygame.SRCALPHA)
    card_surf.fill((10, 10, 18, 205))
    surf.blit(card_surf, (12, 78))

    # ── Hero zone — "18%" in GOLD, 64pt (note 4) ─────────────────────────────
    f64 = pygame.font.Font(FONT_PATH, 64)
    hero_surf = f64.render(f"{int(DEATH_PHASE * 100)}%", True, GOLD)
    HERO_X = 20
    surf.blit(hero_surf, (HERO_X, 80))

    # Label lines: 10pt, baseline-aligned to numeral baseline (note 4)
    hero_baseline_y = 80 + f64.get_ascent()
    f10_h = font(10).get_height()
    f10_asc = font(10).get_ascent()
    # Position "FLOWN" so its baseline sits at the numeral's baseline
    flown_mid_y = int(hero_baseline_y - f10_asc + f10_h / 2)
    oftd_mid_y = flown_mid_y - f10_h - 2
    label_x = HERO_X + hero_surf.get_width() + 10
    text(surf, "OF THE DAY", 10, midleft=(label_x, oftd_mid_y),
         color=(150, 145, 135), track=2, shadow=None)
    text(surf, "FLOWN", 10, midleft=(label_x, flown_mid_y),
         color=(150, 145, 135), track=2, shadow=None)

    # Gold separator rule ON TOP of scrim card (note 1)
    RULE_Y = 82 + hero_surf.get_height() + 8
    alpha_line(surf, (*GOLD, 80), (18, RULE_Y), (W - 18, RULE_Y), 1)

    # ── Log table (below RULE_Y) ─────────────────────────────────────────────
    TABLE_Y = RULE_Y + 10
    TABLE_X_KEY = 24
    TABLE_X_VAL = W - 24      # right-aligned flush (note 2)
    ROW_H = 26                 # increased from 20 (note 3)
    KEY_COLOR = (130, 126, 118)
    PILLAR_RED = (220, 80, 60)

    rows = [
        # DAY row removed (note 6)
        ("FELL AT PILLAR", str(DEATH_PILLAR)),   # renamed + danger signal (note 5)
        ("TIME ALIVE",     f"0:{TIME_ALIVE:02d}"),
        ("EVENTS SEALED",  "4 / 4"),
        ("PHASE",          "EARLY DAY"),
    ]
    for i, (k, v) in enumerate(rows):
        ry = TABLE_Y + i * ROW_H
        # Row separator
        alpha_line(surf, (80, 78, 85, 60), (18, ry - 3), (W - 18, ry - 3), 1)
        # Red 3px accent bar on FELL AT PILLAR (note 5)
        if k == "FELL AT PILLAR":
            pygame.draw.rect(surf, PILLAR_RED, (18, ry - 3, 3, ROW_H))
        mid_y = ry + ROW_H // 2
        text(surf, k, 8, midleft=(TABLE_X_KEY, mid_y),
             color=KEY_COLOR, track=1, shadow=None)
        # Values: 13pt, right-aligned; FELL AT PILLAR in danger red (notes 2,3,5)
        val_color = PILLAR_RED if k == "FELL AT PILLAR" else CREAM
        text(surf, v, 13, midright=(TABLE_X_VAL, mid_y),
             color=val_color, shadow=None)
    # Closing separator
    ry_end = TABLE_Y + len(rows) * ROW_H
    alpha_line(surf, (80, 78, 85, 60), (18, ry_end - 3), (W - 18, ry_end - 3), 1)

    # ── DAY COMPLETE label — midright=(W-18,…), track=2, shadow (note 7) ────
    xdc, ydc = pos_u(1.0)
    dc_x = W - 18
    dc_y = int(ydc) + 18
    # Soft shadow: dim copy 1px offset in (6,8,14)
    text(surf, "DAY COMPLETE", 8, midright=(dc_x + 1, dc_y + 1),
         color=(6, 8, 14), shadow=None, track=2)
    text(surf, "DAY COMPLETE", 8, midright=(dc_x, dc_y),
         color=GOLD, shadow=None, track=2)

    # ── BACK — slim bordered text button ─────────────────────────────────────
    back_r = pygame.Rect(0, 0, 100, 28)
    back_r.center = (W // 2, 597)
    back_surf = pygame.Surface(back_r.size, pygame.SRCALPHA)
    pygame.draw.rect(back_surf, (*SCRIM, 200), back_surf.get_rect(), border_radius=4)
    pygame.draw.rect(back_surf, (*GOLD, 70), back_surf.get_rect(), 1, border_radius=4)
    surf.blit(back_surf, back_r.topleft)
    text(surf, "BACK", 11, center=back_r.center, color=CREAM, track=3, shadow=None)
