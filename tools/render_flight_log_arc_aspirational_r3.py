#!/usr/bin/env python3
"""
sun-arc  ·  flight-log arc v2  ·  arc_aspirational round 3 — sunrise rays at DAY COMPLETE, 5-layer golden bloom.
...
Direction E changes:
  - DAY COMPLETE gets a full sun treatment — a glowing disc visible through
    the dark veil; the right terminal becomes an aspirational target.
  - Larger gold diamond at DAY COMPLETE for aspirational prominence.
  - "NEWBIE PERIOD" full label with bracket.
  - "DISCOVER WHAT'S AHEAD" aspirational hint at bottom.
"""
import os
import math
import random
...
def draw_overlay(ss):
    ...
    # DAY COMPLETE — larger gold diamond for aspirational prominence
    x, y = PU(1.0)
    ...
    # Sunrise rays radiating from DAY COMPLETE terminus
    for ray_i in range(8):
        ra = math.radians(ray_i * 45 + 22)
        rdx, rdy = math.cos(ra), math.sin(ra)
        r_start = 7.0 * k
        r_end = 32.0 * k
        pygame.draw.line(ss, (255, 215, 80, 170),
                         (int(x + rdx * r_start), int(y + rdy * r_start)),
                         (int(x + rdx * r_end), int(y + rdy * r_end)),
                         max(1, int(2.0 * k)))
    for ray_i in range(4):
        ra = math.radians(ray_i * 90)
        rdx, rdy = math.cos(ra), math.sin(ra)
        r_start = 9.0 * k
        r_end = 44.0 * k
        pygame.draw.line(ss, (255, 195, 60, 110),
                         (int(x + rdx * r_start), int(y + rdy * r_start)),
                         (int(x + rdx * r_end), int(y + rdy * r_end)),
                         max(1, int(1.4 * k)))
    ...
    for p, rad, fn, col in ring_items:
        ...
        if p > DEATH_PHASE:
            ink_sil = (46, 42, 56)
            stamp_glyph(ss, fn, x * k, y * k, 5.2 * k, ink_sil, ink_pad=int(1.4 * k))
        else:
...
def render_screen():
    ...
    # DAY COMPLETE — 5-layer sunrise glow at the right terminus
    xdc, ydc = pos_u(1.0)
    for rad, col_g, peak in ((54, (255, 140, 30), 16), (40, (255, 168, 55), 26),
                              (28, (255, 196, 80), 42), (18, (255, 220, 120), 62),
                              (10, (255, 244, 180), 90)):
        g_dc = soft_glow(rad, col_g, peak=peak, falloff=1.8)
        surf.blit(g_dc, (int(xdc) - rad - 1, int(ydc) - rad - 1), special_flags=pygame.BLEND_ADD)
    pygame.draw.circle(surf, (255, 252, 200), (int(xdc), int(ydc)), 9)
    pygame.draw.circle(surf, (255, 255, 240), (int(xdc) - 1, int(ydc) - 1), 5)
    # DAY COMPLETE label
    text(surf, "DAY COMPLETE", 8, midright=(W - 6, int(ydc) - 20), color=GOLD, shadow=None)
    ...
    # NEWBIE PERIOD label — full name with bracket
