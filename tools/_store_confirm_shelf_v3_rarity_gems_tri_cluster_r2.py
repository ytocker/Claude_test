"""Concept E — tri-cluster — Round 2 for the store confirm popup rarity-gems v3 line.

Fixes from art-director critique:
- Cluster raised to cy=321 (was 323) so the 1-px button-bevel overlap disappears;
  center radius trimmed from r=7 to r=6 for 3px headroom below the button row.
- Flank gem base deepened to (200,155,60) / deep (110,80,20) — saturated amber
  that reads as visible gold facet body rather than a near-white sparkle.
- Flank auras match the new body colour so the haloes don't contradict the gems.
- Center aura radius trimmed to m(12) to stay proportional with the smaller r=6 gem.

Output -> docs/store_confirm_shelf_v3/rarity-gems/tri-cluster/round_2.png
"""
import os, sys, math
...
# Saturated amber for gold flank gems — visible facet body colour, not a sparkle.
FLANK_BASE = (200, 155, 60)
FLANK_DEEP = (110,  80, 20)
...
def _draw_tri_cluster(big, affordable):
    # Three gems threaded on a hairline below the button row: rarity centerstone
    # (violet) flanked by saturated-amber pip gems. Auras render first so the
    # haloes stay behind the facet bodies; hairline bridges all three; gems cap.
    # Raised to cy=321 so the cluster clears the button bevel by ~3px.
    if affordable:
        c_base, c_deep, c_glow = PAL["gem"], PAL["deep"], PAL["glow"]
        f_base, f_deep, f_glow = FLANK_BASE, FLANK_DEEP, FLANK_BASE
        c_peak, f_peak         = 26, 16
    ...
    # Center aura radius trimmed to m(12) to stay proportional with r=6 gem.
    sc._alpha_aura(big, m(100), m(321), m(12), c_glow, peak=c_peak, layers=12)
    sc._alpha_aura(big, m(72),  m(321), m(9),  f_glow, peak=f_peak, layers=8)
    sc._alpha_aura(big, m(128), m(321), m(9),  f_glow, peak=f_peak, layers=8)

    pygame.draw.line(big, hair_col,
                     (m(67), m(321)), (m(133), m(321)), max(1, m(1)))

    # Center r=6 (was 7) gives 3px headroom above; flanks r=5 unchanged.
    sc.facet_gem(big, m(100), m(321), m(6), c_base, c_deep)
    sc.facet_gem(big, m(72),  m(321), m(5), f_base, f_deep)
    sc.facet_gem(big, m(128), m(321), m(5), f_base, f_deep)
...
draw.text((CANVAS_W // 2, HDR_H // 2), "E — tri-cluster  [R2]",
...
# ── PIL verification ──────────────────────────────────────────────────────────
# Gem centres must be non-background; flank color must show gold character (R>G>B).
BG = (8, 8, 20)
aff = _panel(True)
print("\n=== E tri-cluster gem-centre checks (affordable, R2) ===")
for (x, y) in [(100, 321), (72, 321), (128, 321)]:
    p = aff.getpixel((x, y))
    ok = "ok non-bg" if p != BG else "WARN bg"
    print(f"  ({x:>3},{y}): {p}  {ok}")

flank_px = aff.getpixel((72, 321))
r, g, b = flank_px[0], flank_px[1], flank_px[2]
gold_char = "gold R>G>B" if r > g > b else "WARN not gold"
print(f"\n  flank (72,321): R={r} G={g} B={b}  -> {gold_char}")

muted = _panel(False).getpixel((100, 321))
print(f"  center hue — affordable {aff.getpixel((100, 321))} (epic violet: B>R)"
