"""Shelf-redesign: gem-cradle-tab v3 (progressive lower, delta=10 (chip_bottom=307, gem_top=298 -> 9px overlap)).
...
v3 — delta=10 (chip_bottom=307, gem_top=298 -> 9px overlap):
  SHELF_Y=238, SHELF_H=90  # shelf_bottom=328
  BTN_CY=262, BTN_H=30
  CHIP_CY=294    # chip_bottom=307; gem_top=298 -> 9px overlap
  BOT_GEM_CY=309            # fixed

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output -> docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-tab/v3/round_1.png
"""
import os, sys, math
...
# v3 — progressive lower (delta=10 (chip_bottom=307, gem_top=298 -> 9px overlap)).
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 238, 174, 90  # SHELF_H = 328 - SHELF_Y; shelf_bottom=328
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 262, 8
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-tab v3  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-tab", "v3", "round_1.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

img   = Image.open(OUT)
btn   = img.getpixel((18 + 58,  54 + 262))
chip  = img.getpixel((18 + 100, 54 + 294))
gem_l = img.getpixel((18 + 33,  54 + 309))
print(f"v3  size:{img.size}  btn:{btn}  chip:{chip}  gem_l:{gem_l}")
