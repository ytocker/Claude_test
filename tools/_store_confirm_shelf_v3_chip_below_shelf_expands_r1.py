"""Chip-below gems — option E: shelf-expands (R1).

Variant of the shelf-expands take with the price chip repositioned BELOW the
Buy/Cancel buttons: buttons rise to the shelf top, chip drops toward the shelf
bottom just above the straddling frame gems. Shelf geometry and the seat-halo
gem settings are inherited unchanged from the bottom-gems R2 script.
  - Buttons near shelf top: BTN_CY=246.
  - Chip near shelf bottom: CHIP_CY=284 (above gem straddle at cy=316).
  - SHELF_Y=221, SHELF_H=101 (bottom stays at 322), BOT_GEM_CY=316 unchanged.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/shelf-expands/round_1.png
"""
import os, sys, math
...
# Chip below buttons: buttons hug the shelf top, chip drops toward the bottom
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip-below E: shelf-expands  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "shelf-expands", "round_1.png"))
...
BG  = (8, 8, 20)
aff = panels_data[0][2]
chip_px = aff.getpixel((100, 284))
btn_px  = aff.getpixel((58, 246))
