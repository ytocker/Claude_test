...
— one labeled sheet under docs/store_redesign/animal/pufferfish/.

Run: ``PYTHONPATH=. SDL_VIDEODRIVER=dummy python tools/pufferfish_candidates/_render_all.py [round_tag]``
"""
...
DESIGNS = [
    ("design_1", "FINNED PUFFER"),
    ("design_2", "BALLOON SPINES"),
    ("design_3", "FUGU TORPEDO"),
    ("design_4", "AQUA PUFFER"),
    ("design_5", "DERPY BEAK"),
]

OUT_DIR = "docs/store_redesign/animal/pufferfish"
...
def render_design(slug, name):
    build = importlib.import_module(
        f"tools.pufferfish_candidates.{slug}").build
    ...
    sheet.fill((18, 16, 26))
    _label(sheet, f"PUFFERFISH  ·  {name}  ·  {ROUND.replace('_', ' ')}",
           pad, 16, size=26)
    ...
    _label(sheet, "FLAP (inflate)", col3 + 4, y0 + 250, size=14)
