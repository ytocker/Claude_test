"""Compose the SHADOWSTRIKE review sheet (hero + gameplay + 40px truth read).
...
OUT = "docs/store_redesign/costume/ninja/design_1/round_3.png"
...
def _truth_read(box):
    ...
    # Render the bird alone (transparent) at native res, then crop to content.
    frame = build(ninja_render.FRAME_IDX, ninja_render.TILT)
    bb = frame.get_bounding_rect()
    frame = frame.subsurface(bb).copy()
    # Scale longest side to 40px with NEAREST (the brutal downscale).
    sw, sh = frame.get_size()
    scale = 40.0 / max(sw, sh)
    small = pygame.transform.scale(
        frame, (max(1, int(sw * scale)), max(1, int(sh * scale))))
    # Composite the tiny bird on day + night swatches, magnify 3x NEAREST.
...
def main():
    box = 360                       # panel height for all three columns
    gw = int(box * 9 / 16)          # gameplay keeps the 360x640 portrait aspect
