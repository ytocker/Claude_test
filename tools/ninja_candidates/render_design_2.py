"""Compose the design_2 (CRIMSON FANG) review sheet: hero shot + in-gameplay
panel + a NEAREST-neighbour 40px "truth read" magnified 3x. Headless only.
"""
import os
...
OUT = "docs/store_redesign/costume/ninja/design_2/round_1.png"
TITLE = "CRIMSON FANG — Blood Assassin Kunoichi  (ninja design_2)"
...
def _font(sz, bold=False):
    f = pygame.font.SysFont("arial,dejavusans", sz, bold=bold)
    return f
...
def main():
    ...
    _label(sheet, "crimson re-plumaged macaw · crossed-kunai X · streaming "
                  "charcoal scarf · steel brow plate", 22, 42, sz=13, col=SUB)
    ...
    # In-gameplay panel. The harness crops a window of int(H*0.78)=499px tall
    # out of the 360x640 scene, so the panel must stay portrait (w/h <= 360/499)
    # or the crop runs past the scene edge.
    gw, gh = 180, 250
    ...
    tx = gx + gw + 24
    panel_w = W - tx - 22
    # Backing chip so the dark cloth separates from the sheet.
