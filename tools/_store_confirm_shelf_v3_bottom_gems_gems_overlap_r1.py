"""Bottom-frame gems — option D: gems-overlap.

No layout change — shelf and buttons stay at original positions.
Gems render BEFORE the shelf/chip/buttons so buttons draw on top.
"Embedded jewel" aesthetic: gems partially visible at outer button corners.

SHELF_Y=235, CHIP_CY=258, BTN_CY=302 (all original, unchanged).
Gems at cy=309 overlap BTN bottom (287→317) — intentional inset-jewel effect.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/bottom-gems/gems-overlap/round_1.png
"""
import os, sys, math
...
# Option D: NO layout change — gems overlap buttons intentionally
...
def render_popup(name, affordable):
    ...
    # Bottom gems drawn BEFORE shelf so buttons overlay them — "embedded jewel" look
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Bottom gems D: gems-overlap  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "bottom-gems", "gems-overlap", "round_1.png"))
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

# Note: gem centers at 309 overlap with buttons at 287→317 — gem pixel will be button color
BG  = (8, 8, 20)
aff = panels_data[0][2]
for lbl, gx, gy in [("left", BOT_GEM_LX, BOT_GEM_CY), ("right", BOT_GEM_RX, BOT_GEM_CY)]:
    p = aff.getpixel((gx, gy))
    print(f"  gem {lbl} ({gx},{gy}): {p}  {'ok (gem hidden by button)' if p != BG else 'WARN bg'}")
p = aff.getpixel((BUY_CX, BTN_CY))
print(f"  BUY ({BUY_CX},{BTN_CY}): {p}  {'ok' if p != BG else 'WARN bg'}")
# Verify gem aura is visible just outside button edge (x=18 is inside gem but outside button at x=20)
gem_edge_p = aff.getpixel((18, BOT_GEM_CY))
print(f"  gem aura edge (18,{BOT_GEM_CY}): {gem_edge_p}  {'ok' if gem_edge_p != BG else 'WARN bg'}")
