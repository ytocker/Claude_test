"""Shelf-redesign concept 1: gem-cradle-chip (R1).

The narrow price chip drops all the way down to cy=309 — the same level as the
bottom gems — so it sits *between* them like a jewel in a setting. The shelf
above becomes a compact button-only tray; the flanking gems form a matched
triad around the chip.

  SHELF_Y=210, SHELF_H=72   # shelf_bottom=282  (button tray only)
  BTN_CY=234, BTN_H=30      # compact button row
  CHIP_CY=309, CHIP_H=28    # capsule sitting at gem level, between the gems
Bottom gems at cy=309 flank the chip.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-chip/round_1.png
"""
import os, sys, math
...
# Chip cradled between the bottom gems: compact button-only tray on top, the
# capsule dropped to gem level so the triad reads as a single set piece.
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 210, 174, 72   # shelf_bottom=282
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 234, 8
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD           = 88, 28, 14    # rounder capsule, narrower
CHIP_CY                            = 309           # at gem level, between the gems
...
def _draw_shelf(big, affordable):
    ...
    # Compact button tray on top; chip is dropped to gem level below the shelf.
    ...
    # Chip drawn BEFORE the bottom gems so the gem auras render on top of the
    # chip's outer edges, seating the capsule between the flanking gems.
...
def render_popup(name, affordable):
    ...
    # Shelf draws the compact tray, buttons, AND the cradled chip at cy=309.
    _draw_shelf(big, affordable)

    # Bottom frame gems drawn AFTER shelf so their auras seat over the chip edges.
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-chip  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-chip", "round_1.png"))
...
# PIL verification
img   = Image.open(OUT)
chip  = img.getpixel((18 + 100, 54 + 309))   # chip centre at gem level
btn   = img.getpixel((18 + 58,  54 + 234))   # BUY centre
gem_l = img.getpixel((18 + 33,  54 + 309))   # left bottom gem
print(f"size:{img.size}  chip:{chip}  btn:{btn}  gem_l:{gem_l}")
