"""Chip-below layout — option D: shelf-notch (R1).

Same shelf-notch construction as bottom-gems D (circular bezel holes punched
into the shelf so gems read as stones "set into" the frame), but the price
chip is repositioned BELOW the Buy/Cancel buttons. Buttons now sit near the
shelf top; the chip sits near the shelf bottom, just in front of the notch
gems so each stone peeks out from behind the chip at the shelf edge.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/shelf-notch/round_1.png
"""
import os, sys, math
...
# Chip repositioned BELOW the buttons: buttons ride near the shelf top, the
# chip drops to the shelf bottom just above/over the seated notch gems.
...
def _draw_shelf(big, affordable):
    ...
    # Punch circular bezel holes so gems read as set into the frame, not on it.
    # notch_mask: white keeps the shelf, transparent erases it down to the hole.
    ...
    # Buttons first (near shelf top), then the price chip below them so the
    # chip layers in front of the shelf surface at the bottom edge.
...
def render_popup(name, affordable):
    ...
    # Gems drawn after the shelf so they fill the punched bezel holes and read
    # as stones seated into the frame rather than floating above it.
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip below D: shelf-notch  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "shelf-notch", "round_1.png"))
...
notch   = img.getpixel((18 + 19,  54 + 309))  # edge of notch — should be dark card interior
print(f"chip: {chip_px}  btn: {btn_px}  gem: {gem_l}  notch_edge: {notch}")
