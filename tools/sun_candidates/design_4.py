...
def build_kawaii(wing_angle_deg):
    ...
    # Soft rounded bobble corona (gentle scallops, flare a touch on shine).
    _stub_ring(surf, cx, cy, r - 2, 5 + int(sh * 1), 11, rb, rt)

    # Plump pastel disc.
    _radial_body(surf, cx, cy, r, core, mid, edge)

    # Big-eyed adorable face with round blush + a tiny smile.
    _sun_face(surf, cx, cy, eye_dx=6, eye_r=5, iris=(96, 56, 24),
