"""Chip-below buttons — option A: full-raise (R2).

R2 addresses the R1 art-director critique:
  1. Kill hollow midriff: BTN_H 30→34, BTN_CY 231→238 (btn_top=221, bottom=255).
  2. Shelf embraces gems: SHELF_H 79→93 (shelf_bottom 303, 5px below gem_top≈298).
  3. Chip breathing room: CHIP_CY 271→275 (chip 260→290 within taller shelf).
  4. Chip reads as its own plate: fill ~18% lighter than shelf interior + a
     bright inner sheen so it lifts off the dark indigo shelf gradient.
  5. BUY re-established as primary CTA: brighter fill stops, larger glyph, and
     a touch more sheen than CANCEL, which stays subdued.

  SHELF_Y=210, SHELF_H=93   # shelf_bottom=303
  BTN_CY=238, BTN_H=34      # btn_top=221, btn_bottom=255
  CHIP_CY=275, CHIP_H=30    # chip_top=260, chip_bottom=290
Bottom gems at cy=309 — UNCHANGED.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/full-raise/round_2.png
"""
import os, sys, math
...
# Option A: raise shelf + all contents; chip now BELOW buttons.
# R2: taller shelf embraces gem tops; buttons grown + lowered to close the gap.
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 210, 174, 93   # shelf_bottom=303
BTN_W, BTN_H, BTN_RAD              = 76, 34, 9
BTN_CY, BTN_GAP                    = 238, 8            # btn 221→255 (11px from shelf top)
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD          = 112, 30, 8
CHIP_CY                           = 275               # chip 260→290, 13px lip to shelf bottom
...
def _draw_shelf(big, affordable):
    ...
    # Chip below buttons: draw buttons first (near shelf top), then chip (near bottom)
    ...
    brad = m(BTN_RAD)
    _btn(big, buy, brad, "BUY",    15, locked=not affordable)   # primary: larger glyph
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip-below A: full-raise (R2)  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "full-raise", "round_2.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

# PIL verification: chip below buttons, BUY primary, bottom gems intact
img  = Image.open(OUT)
w, h = img.size
chip_px = img.getpixel((18 + 100, 54 + CHIP_CY))   # chip centre
btn_px  = img.getpixel((18 + 58,  54 + BTN_CY))     # BUY centre
