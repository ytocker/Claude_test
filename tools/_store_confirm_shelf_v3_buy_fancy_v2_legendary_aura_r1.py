"""buy-fancy-v2 concept E — legendary-aura.

A massive amber glow bloom fills the shelf area around BUY (affordable only).
The BUY button is rendered in the canonical Ramp-A gold, and five small facet
gems orbit it like satellite stars. CANCEL stays a plain deep-card button. On
the unaffordable panel BUY reverts to a locked slate + padlock with NO glow and
NO satellites — the legendary treatment is a reward for being able to afford it.
...
Output → docs/store_confirm_shelf_v3/buy-fancy-v2/legendary-aura/round_1.png
"""
import os, sys, math
...
def _draw_shelf(big, affordable):
    ...
    # Step 1 — massive legendary glow bloom behind the BUY area, laid down
    # BEFORE the chip and buttons so it reads as light welling up from under the
    # button rather than a sticker on top. Affordable only: the bloom is the
    # "you can claim this" reward tell.
    if affordable:
        sc.soft_glow(big, m(BUY_CX), m(BTN_CY), m(54),
                     sc.RARITY["legendary"]["glow"], peak_alpha=160)
        sc._alpha_aura(big, m(BUY_CX), m(BTN_CY), m(30),
                       sc.RARITY["legendary"]["gem"], peak=120, layers=12)
    ...
    # Step 5 — five satellite facet gems orbiting BUY like stars (affordable
    # only), starting at 12 o'clock and stepping 72° so they ring the button.
    if affordable:
        SAT_R = m(7)      # satellite gem radius
        ORBIT = m(24)     # distance from BUY centre
        for k in range(5):
            angle = 2 * math.pi * k / 5 - math.pi / 2   # start at top
            sx = m(BUY_CX) + int(ORBIT * math.cos(angle))
            sy = m(BTN_CY) + int(ORBIT * math.sin(angle))
            sc.facet_gem(big, sx, sy, SAT_R,
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "legendary-aura (E) r1  |  AFFORDABLE / UNAFFORDABLE",
...
print(f"BUY center ({BUY_CX},{BTN_CY}): {buy_px}  "
      f"→ {'OK warm gold' if buy_px[0] >= 200 else 'WARN'}")
glow_px = aff.getpixel((BUY_CX, 280))
print(f"glow sample ({BUY_CX},280): {glow_px}  "
      f"→ {'OK non-bg' if glow_px != (8,8,20) else 'WARN'}")
sat_px = aff.getpixel((BUY_CX, 278))
print(f"top satellite (~{BUY_CX},278): {sat_px}  "
      f"→ {'OK non-bg' if sat_px != (8,8,20) else 'WARN'}")
