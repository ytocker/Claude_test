"""Chip-below gems — option E: shelf-expands (R2).

R2 tightens the expanded shelf so it stops pooling empty interior span:
  - SHELF_H drops 101→90 (shelf_bottom 322→311); SHELF_Y=221 unchanged.
  - Even vertical cascade: BTN_CY=243 (btn_top 7px below shelf top),
    CHIP_CY=279 (6px below the buttons), gems at 316 (11px below the chip).
  - Chip gets a distinct pale plate fill so it reads as a chip, not shelf.
  - Gem seat halos removed — gems straddle the shelf rim cleanly, no smudge.
  - Shelf top_sheen toned down and pulled up into the lip so it clears the
    button tops and never competes with the hero disc glow above.
  - BUY sits above CANCEL in hierarchy: larger, brighter face and label.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/shelf-expands/round_2.png
"""
import os, sys, math
...
# Option E R2: shelf top unchanged, bottom pulled up to 311 so the interior
# no longer pools empty space between the buttons and the chip.
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 221, 174, 90
# Even cascade down the shelf: buttons hug the top, chip below, gems straddle.
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 243, 8
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    if locked:
        ...
        # BUY reads primary: brighter face + stronger sheen than CANCEL.
        stops   = [(0.0, (44, 46, 96)), (1.0, (26, 28, 64))]
        lab_col = (214, 219, 250)
        sheen   = 26
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    # Chip fill sits pale against the darker shelf interior so it reads as a
    # distinct plate even before its number is drawn.
    face_stops = ([(0.0, (50, 52, 96)), (1.0, (40, 42, 80))] if affordable
                  else [(0.0, (46, 48, 72)), (1.0, (36, 38, 58))])
...
def _draw_shelf(big, affordable):
    ...
    # Seat halos removed — the dark rings read as smudges; the gems now straddle
    # the shelf rim on their own, which is a cleaner setting.

    # Toned, lifted sheen: shorter band so its bright crown stays inside the
    # top lip and clears the button tops instead of competing with the disc.
    sc.top_sheen(shelf, shelf.get_rect(), 0, m(8), peak=30)
    ...
    # Buttons draw before the chip: buttons hug the shelf top, chip sits below
    ...
    brad = m(BTN_RAD)
    # BUY runs a touch larger than CANCEL to sit above it in the hierarchy.
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip-below E: shelf-expands R2  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "shelf-expands", "round_2.png"))
...
BG  = (8, 8, 20)
aff = panels_data[0][2]
chip_px = aff.getpixel((100, 279))
btn_px  = aff.getpixel((58, 243))
gem_l   = aff.getpixel((33, 316))
print(f"chip:{chip_px} btn:{btn_px} gem:{gem_l}")
for lbl, px in [("chip", chip_px), ("btn", btn_px), ("gem", gem_l)]:
    print(f"  {lbl}: {px}  {'ok' if px != BG else 'WARN bg'}")
