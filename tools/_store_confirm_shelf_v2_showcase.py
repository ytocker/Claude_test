"""Phase 5 showcase: store_confirm_shelf_v2

Panels (left → right):
  0  BEFORE  — current live _draw_confirm
  A  gold-standard   round_2.png  left (affordable) panel
  B  gold-warm-buy   round_2.png  left (affordable) panel
  C  gold-hierarchy  round_2.png  left (affordable) panel
  D  gold-mono       round_2.png  left (affordable) panel
  E  gold-airy       round_2.png  left (affordable) panel

Output: docs/store_confirm_shelf_v2/showcase.png
"""
import sys, os
...
# Sheet: MARGIN=18, HDR_H=28, GAP_HDR=8 → panel_y=54
# Left panel: (18, 54, 218, 394) = 200×340

SLUGS = [
    ("A", "gold-standard",  "GOLD STANDARD"),
    ("B", "gold-warm-buy",  "GOLD WARM BUY"),
    ("C", "gold-hierarchy", "GOLD HIERARCHY"),
    ("D", "gold-mono",      "GOLD MONO"),
    ("E", "gold-airy",      "GOLD AIRY"),
]

BASE = "docs/store_confirm_shelf_v2"
...
hx = CANVAS_W // 2
hy = MARGIN + HDR_H // 2
draw.text((hx, hy - 8), "STORE CONFIRM — GOLD BEVEL REDESIGN",
...
OUT = "docs/store_confirm_shelf_v2/showcase.png"
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}×{CANVAS_H})")

# PIL verification: each panel must be non-background at its BUY button center
# In 1x coords: BUY_CX=58, BTN_CY=302. In 200×355 panel (scaled from 200×340):
# y_scale = 355/340 → BTN_CY_scaled = round(302 * 355/340) ≈ 315
BUY_CX_1X = 58
BTN_CY_355 = round(302 * 355 / 340)
print(f"\n=== Showcase PIL verification (BUY button center per panel) ===")
verify_img = Image.open(OUT)
panels_ok = True
for i, (badge, slug, label) in enumerate([("0","BEFORE","BEFORE")] + [(b,s,l) for b,s,l in SLUGS]):
