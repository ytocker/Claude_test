"""BUY-fancy exploration: glass-cabochon (C).

BUY reads as a polished cabochon gem lens: deep sapphire fill, a
dramatically tall glass sheen, and a secondary inner blue bevel ring for
...
Output → docs/store_confirm_shelf_v3/buy-fancy/glass-cabochon/round_1.png
"""
import os, sys, math
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    ...
    # BUY — deep sapphire cabochon lens. Draw order: shadow, fill, tall sheen,
    # outer gold bevel, inner blue bevel, then label — so the double rim reads
    # as glass depth and the sheen bows over the top like a polished dome.
    stops   = [(0.0, (88, 100, 168)), (1.0, (44, 56, 108))]
    lab_col = (224, 230, 250)
    ...
    sc.bevel_rim(big, buy_inner, max(2, rad - m(2)), (30, 36, 72, 160),
                 (150, 165, 220, 150), w=max(1, m(1.2)))
    sc.plain_text(big, label, sc.font(font_px), btn_rect.center, lab_col,
                  shadow_a=110, weight=m(0.8), keyline=(8, 6, 20), kw=m(0.9))
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "glass-cabochon (C) r1  |  AFFORDABLE / UNAFFORDABLE",
...
print(f"BUY center {buy_px}, CANCEL center {can_px}")
print(f"BUY blue-indigo check (B>R,G): {'OK' if buy_px[2] > buy_px[0] and buy_px[2] > buy_px[1] else 'WARN'}")
