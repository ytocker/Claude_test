#!/usr/bin/env python3
"""
scale-to-fit  ·  store buy-confirmation popup v4  ·  long-name overflow  ·  R1

The control/baseline concept for the long-name overflow problem: the item
name shrinks uniformly (font size stepped down −0.5 from 30, floor 9) until it
fits inside a 168px safe width, then renders as a single centred line at
Y_NAME=168. No layout change — the "invisible fix" the other concepts must beat.

Everything else in the draw function is unchanged from shelf-chip R2.
"""
import math
import os
import sys
...
# Font size actually used per panel — captured by the draw fn for the footer.
_used_sz = {}
...
def _draw_confirm(self, surf):
    ...
    # scale-to-fit: shrink font until name fits safe_w
    safe_w = m(168)
    sz = NAME_FS  # start at 30
    while sz > 9:
        f = sc.font(sz)
        if sc._glyph_base(name, f, 0).get_width() <= safe_w:
            break
        sz -= 0.5
...
# Header
hdr_txt = "SCALE-TO-FIT · Long-name overflow R1"
if fhdr:
...
if fsub:
    zt = fsub.render("PANEL 2 NAME ZONE · 2× (y 130–210)", True, DIM)
...
for i, nm in enumerate(NAMES):
    ...
    if fsub:
        l2 = fsub.render(f"(sz {sz_str})", True, DIM)
