"""hybrid-2 Z1 BUY text refinement — match the CANCEL text treatment.

CANCEL sets the reference: font 14, pale letters, soft shadow, dark keyline.
Three panels: the round_2 Z1 dark-ink text as rendered, a literal match using
CANCEL's exact pale-lavender colour, and a warm-neutral match keeping
CANCEL's size/treatment with cream letters that hold contrast on gold.
"""
import os
import sys
...
# (label, buy_text_col, buy_font_px)
VARIANTS = [
    ("T1 · dark ink (r2)", GOLD_A_NUM, 16),
    ("T2 · CANCEL exact", (150, 155, 200), 14),
    ("T3 · CANCEL style, cream", (255, 248, 220), 14),
]


def make_buttons_fn(text_col, fpx):
    def buttons(ov):
        rad = m(12)
        for cx, is_cancel in ((BUY_CX, False), (CAN_CX, True)):
            ...
            if is_cancel:
                ...
                lab_c, pk, lbl, fp = (150, 155, 200), 14, "CANCEL", 14
                rd, rb = CARD_RING_DEEP, CARD_RING_BRIGHT
            else:
                s, lab_c, pk, lbl, fp = Z1, text_col, 28, "BUY", fpx
            ...
            plain_text(ov, lbl, font(fp), r.center, lab_c,
...
def main():
    ...
    try:
        panels = []
        for label, tc, fpx in VARIANTS:
            h2.overlay_buttons = make_buttons_fn(tc, fpx)
        ...
        idr.text((MARGIN, 18),
                 "hybrid-2 Z1 BUY text refinement · match CANCEL treatment · EPIC",
                 fill=(236, 214, 160))
