"""Shelf-redesign: gem-cradle-tab w1 (large downward shift, Δ20).
...
w1 — Δ20:
  SHELF_Y=248, SHELF_H=80  # shelf_bottom=328 (pinned)
  BTN_CY=270, BTN_H=26
  CHIP_CY=302, CHIP_H=26    # chip_bottom=315; bottom gem center (309) sits inside chip zone
  BOT_GEM_CY=309            # fixed

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-tab/w1/round_1.png
"""
import os, sys, math
...
# w1 — Δ20 downward shift; shelf bottom pinned at card bottom (328).
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 248, 174, 80  # SHELF_H = 328 - SHELF_Y
BTN_W, BTN_H, BTN_RAD              = 76, 26, 9
BTN_CY, BTN_GAP                    = 270, 8
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD           = 88, 26, 8
CHIP_CY                            = 302   # chip_bottom=315; bottom gem center (309) sits inside chip zone
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-tab w1 | AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-tab", "w1", "round_1.png"))
...
print(f"w1  size:{img.size}  btn:{btn}  chip:{chip}  gem_l:{gem_l}")
