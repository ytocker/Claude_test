#!/usr/bin/env python3
"""
horiz-squeeze  ·  store buy-confirmation popup v4  ·  long-name overflow  ·  R1
...
Technique ladder:
  1. Negative tracking. Reduce inter-glyph spacing (the `tracking` param already
     threaded through plain_text/_glyph_base) one device-px at a time until the
     tracked master fits <=168px, clamped at -20 device px so glyphs never kiss.
     The keyline is stamped from the SAME tracked master, so it condenses in
     lockstep with the letters — no floating outline.
  2. smoothscale fallback. Only if tracking alone can't reach the safe width does
     the name get rendered to a temp surface at the -20 clamp and squeezed
     horizontally to exactly 168px. Height (cap-height) is untouched, so the
     silhouette weight holds and the keyline scales with the glyphs.
...
TRACK_MIN = -20          # device-px clamp: below this glyphs start to collide
...
def _condense(name, name_font):
    """Return (tracking, use_scale) for `name`. tracking is device px (<=0);
    use_scale flags that even the -20 clamp is still too wide, so the caller must
    fall back to a horizontal smoothscale."""
    safe_w = m(SAFE_LOG)
    tracking = 0
    while tracking > TRACK_MIN:
        if sc._glyph_base(name, name_font, tracking).get_width() <= safe_w:
            break
        tracking -= 1
    glyph_w = sc._glyph_base(name, name_font, tracking).get_width()
    return tracking, glyph_w > safe_w


def _regime_label(tracking, use_scale):
    if use_scale:
...
def _draw_name(big, name, cx_ss, y_ss):
    ...
    safe_w = m(SAFE_LOG)
    tracking, use_scale = _condense(name, name_font)
    ...
    if not use_scale:
        ...
        # Fallback: render at the tracking clamp to a temp surface, then squeeze
        # width-only to the safe zone so cap-height (silhouette weight) holds.
        glyph_w = sc._glyph_base(name, name_font, TRACK_MIN).get_width()
        fh = name_font.get_height()
        pad = m(6)
        temp = pygame.Surface((glyph_w + pad * 2, fh + m(12)), pygame.SRCALPHA)
        sc.plain_text(temp, name, name_font,
                      (temp.get_width() // 2, temp.get_height() // 2),
                      (250, 248, 240), shadow_a=160, tracking=TRACK_MIN,
                      weight=m(0.9), keyline=(6, 6, 16), kw=m(1.0))
        squeezed = pygame.transform.smoothscale(temp, (safe_w, temp.get_height()))
        big.blit(squeezed, squeezed.get_rect(center=(cx_ss, y_ss)))

    return _regime_label(tracking, use_scale)
...
# Header
h = fhdr.render("HORIZ-SQUEEZE · Long-name overflow R1", True, GOLD)
...
# Zoom strip: 2x of Panel 1's name zone (logical y=140..195, full width).
