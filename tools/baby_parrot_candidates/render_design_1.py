"""Compose the HATCHLING (design_1) round_1 review sheet: a hero product shot +
an in-gameplay panel + a 40px NEAREST truth read split day | night across three
poses, so the eggshell-cap read is judged at the size it actually ships at.
...
from tools.baby_parrot_candidates.design_1 import build

OUT = "docs/store_redesign/parrot/baby_parrot/design_1/round_1.png"
TITLE = "DESIGN 1 — HATCHLING  (cracked eggshell cap + shard over lens + damp down-wisps · rare)"
...
def _truth_read(box):
    """The 40px-in-motion test: NEAREST-shrink the gameplay frame to a ~40px bird
    then magnify 3x so the real on-screen read is judged. Three poses over a
    day | night split."""
...
def main():
    ...
    for x, name in zip(xs, captions):
        _label(sheet, name, x, y + box + 4, size=13, color=(190, 194, 210))
