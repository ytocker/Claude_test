"""Round 1 — shelf concept 'boutique-receipt' (store_confirm_shelf_v3 series).

A deliberate pattern-break: a cream/parchment shelf sits UNDER the dark indigo
card, with dark-chocolate CTAs on top. The light-under-dark tension is bridged
so no raw edge ever meets cream directly — a 4px indigo bleed at the shelf's top
edge connects it up into the card body, and a brown lip line seats the buttons'
gold bevels against chocolate/mocha, never against the cream field.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE. Verify with PIL, never by viewing.
Output -> docs/store_confirm_shelf_v3/boutique-receipt/round_1.png
"""
import os, sys
...
def _btn(big, rect, rad, label, fill_stops, label_col, font_size,
    ...
    if gold_bevel:
        ...
        # never contacts the cream field beneath.
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    hair = pygame.Surface((num_rect.w, max(1, m(1))), pygame.SRCALPHA)
    hair.fill((*hair_col, 140))   # ~55% alpha
...
def _draw_shelf(big, affordable):
    ...
    # Indigo bleed: a 4px strip fading indigo->transparent so the cream shelf
    # visually connects up into the dark card body instead of hard-cutting.
    bleed_h = m(4)
    bleed = pygame.Surface((sw, bleed_h), pygame.SRCALPHA)
    for y in range(bleed_h):
        a = int(255 * (1 - y / max(1, bleed_h)))
        pygame.draw.line(bleed, (*INDIGO_BLEED, a), (0, y), (sw - 1, y))
    ...
    # CANCEL is milky mocha — blends toward the parchment so it recedes behind
    # BUY. Same gold bevel in both states.
    _btn(big, can, brad, "CANCEL",
         [(0.0, (129, 103, 79)), (1.0, (102, 82, 61))],
         (172, 162, 144), 13, gold_bevel=True, keyline=None, sheen=12)
...
draw.text((CANVAS_W // 2, 30),
          "boutique-receipt r1  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "boutique-receipt", "round_1.png")
