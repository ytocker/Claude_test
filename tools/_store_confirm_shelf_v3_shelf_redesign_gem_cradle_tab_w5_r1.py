"""Shelf-redesign: gem-cradle-tab w5 (large downward shift, Δ40).
...
w5 — Δ40:
  SHELF_Y=268, SHELF_H=60  # shelf_bottom=328 (pinned)
  BTN_CY=284, BTN_H=22
  CHIP_CY=314, CHIP_H=26    # most compressed shelf; chip nearly flush with card bottom
  BOT_GEM_CY=309            # fixed

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-tab/w5/round_1.png
"""
import os, sys, math
...
# w5 — Δ40 downward shift; shelf bottom pinned at card bottom (328).
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 268, 174, 60  # SHELF_H = 328 - SHELF_Y
BTN_W, BTN_H, BTN_RAD              = 76, 22, 9
BTN_CY, BTN_GAP                    = 284, 8
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD           = 88, 26, 8
CHIP_CY                            = 314   # most compressed shelf; chip nearly flush with card bottom
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-tab w5 | AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-tab", "w5", "round_1.png"))
...
print(f"w5  size:{img.size}  btn:{btn}  chip:{chip}  gem_l:{gem_l}")
