"""Round-1 review render of the `flight_logbook` Flight Log concept.
...
Offline tool — writes docs/flight_log_screen/flight_logbook/round_1.png.
...
def _ruled_lines(surf):
    ...
    for i in range(RULE_N):
        ...
        pygame.draw.line(lay, (*RULE_BLUE, 50), (MARGIN_X - PAGE.x - 6, y),
...
def _margin_notes(surf, run):
    ...
    for phase, lines in PHASE_MARKS:
        ...
        tick.fill((*GOLD_MUTED, 110))
        surf.blit(tick, (MARGIN_X - 4, int(y)))

    # Where this run actually stopped — the one gold mark in the margin.
    y = int(PHASE_TOP + (PHASE_BOT - PHASE_TOP) * run["phase"])
    lay = pygame.Surface((14, 14), pygame.SRCALPHA)
    pygame.draw.polygon(lay, (*GOLD, 235), [(0, 1), (7, 6), (0, 11)])
    surf.blit(lay, (MARGIN_X - 9, y - 6))
    bar = pygame.Surface((9, 2), pygame.SRCALPHA)
    bar.fill((*GOLD, 220))
    surf.blit(bar, (MARGIN_X, y - 1))
...
def _draw_route(surf, pts):
    lay = pygame.Surface((W, H), pygame.SRCALPHA)
    pygame.draw.lines(lay, (0, 0, 12, 120), False,
                      [(x + 1, y + 2) for x, y in pts], 2)
    pygame.draw.lines(lay, (*INK, 232), False, pts, 2)
    surf.blit(lay, (0, 0))
    sx, sy = pts[0]
    pygame.draw.circle(surf, GOLD, (int(sx), int(sy)), 3)
    pygame.draw.circle(surf, (0, 0, 12), (int(sx), int(sy)), 3, 1)


# ── doodles (all ≥2px stroke so they read as pen, not hairline) ──────────────

def _d_geyser(surf, cx, cy, c):
    col = [(cx + 2.6 * math.sin(i * 0.9), cy + 10 - i * 2.4) for i in range(7)]
    pygame.draw.lines(surf, c, False, col, 2)
    tip = col[-1]
    for dx, dy in ((-7, -7), (0, -10), (7, -7)):
        pygame.draw.line(surf, c, tip, (tip[0] + dx, tip[1] + dy), 2)


def _d_coins(surf, cx, cy, c):
    for i in range(5):
        a = math.radians(208 + i * 31)
        px = cx + 15 * math.cos(a)
        py = cy + 11 + 15 * math.sin(a)
        pygame.draw.circle(surf, c, (int(px), int(py)), 3, 2)
...
def _d_snow(surf, cx, cy, c):
    for ox, oy, r in ((-4, -2, 6), (8, 6, 4)):
        for ang in (0, 60, 120):
            dx = math.cos(math.radians(ang)) * r
            dy = math.sin(math.radians(ang)) * r
            pygame.draw.line(surf, c, (cx + ox - dx, cy + oy - dy),
                             (cx + ox + dx, cy + oy + dy), 2)
...
def _hero(surf, run, cy):
    ...
    _hand_ellipse(surf, nr.centerx, cy, nr.w / 2 + 22, 54, run["seed"], GOLD)
...
def render_screen(run):
    ...
    tab = _arc_table(pts)
    _draw_route(surf, pts)
    _place_doodles(surf, run, pts, tab)
    _death_mark(surf, pts, tab)
...
def build_sheet(screens):
    ...
    sheet.fill(BG)
    _caps(sheet, "FLIGHT LOGBOOK  ·  ROUND 1", 12, 20, 16, GOLD, tracking=3)
