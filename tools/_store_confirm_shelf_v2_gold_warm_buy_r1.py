"""Round 1 — shelf concept 'gold-warm-buy'.

Both action buttons wear the card's single golden bevel. BUY warms toward a
dark amber fill so it harmonizes with its gold rim (a warm block that pulls the
eye); CANCEL keeps a cool indigo fill under the same gold rim. A compact,
understated price chip sits above, in the same warm-gold material family. The
whole tension is warm BUY vs. cool CANCEL under one shared metal.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE. Verify with PIL, never by viewing.
Output → docs/store_confirm_shelf_v2/gold-warm-buy/round_1.png
"""
import sys, os, math
...
def _draw_chip(big, affordable):
    ...
    if affordable:
        face_stops = [(0.0, (228, 214, 168)), (1.0, (196, 180, 132))]
    else:
...
def _draw_shelf(big, affordable):
    ...
    # CANCEL: cool indigo under the shared gold rim.
    can_stops = [(0.0, (84, 78, 126)), (1.0, (50, 46, 82))]
    _btn(big, can_r, "CANCEL", can_stops, (220, 210, 240), 24,
         (14, 12, 26), 200)

    # BUY: warm dark amber that harmonizes with its gold rim.
    if affordable:
        buy_stops = [(0.0, (72, 62, 42)), (1.0, (48, 40, 24))]
...
OUT = "docs/store_confirm_shelf_v2/gold-warm-buy/round_1.png"
