"""Before/after figure for the hero-halo smoothing on the checkpoint-5 design.

BEFORE is the stock stepped aura (concentric circles readable as rings);
AFTER swaps in the analytic per-pixel gradient (same colour, same strength).
Gold system, EPIC, full popup with a 2x zoom of the halo zone below.

Output: colorways/halo_smooth_v1.png
"""
import os
import sys
...
def main():
    ...
    try:
        variants = (("BEFORE · stepped circles", False),
                    ("AFTER · smooth gradient", True))
        ZX0, ZY0, ZX1, ZY1 = 14, 20, 246, 250
        zoom_w, zoom_h = (ZX1 - ZX0) * 2, (ZY1 - ZY0) * 2
        MARGIN, HEAD, GAP = 24, 52, 20
        cell_w = max(POP_W, zoom_w)
        strip_w = MARGIN * 2 + len(variants) * (cell_w + GAP) - GAP
        strip_h = HEAD + POP_H + 8 + zoom_h + 34
        strip = Image.new("RGB", (strip_w, strip_h), (10, 9, 20))
        idr = ImageDraw.Draw(strip)
        idr.text((MARGIN, 16),
                 "hero halo smoothing · checkpoint 5 · gold · EPIC · zoom 2x",
                 fill=(236, 214, 160))
        for i, (label, smooth) in enumerate(variants):
            h2._DRAW_FN[0] = oc._patched_draw(pal["ring"], smooth_halo=smooth)
