"""Review sheet for DISCO DESIGN 4 — THE SELECTOR (funky-soul DJ), round 1.
...
``SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy python -m tools.disco_candidates.render_4_r1``.
"""
...
title = _font(26, True).render(
    "D4 THE SELECTOR — R1  ·  funky-soul DJ  ·  in gameplay", True, _GOLD_PALE)
sheet.blit(title, title.get_rect(midtop=(sheet_w // 2, 24)))
sub = _font(15, True).render(
    "vinyl record at wing · foam DJ headphones + ear-cup · amber aviators · satin collar + blazer · platform clogs",
    True, (170, 162, 190))
...
out = os.path.join("docs", "store_redesign", "costume", "disco", "design_4", "round_1.png")
