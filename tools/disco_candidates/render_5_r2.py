"""Round-2 review sheet for DISCO Design 5 — MIRRORBALL.

Renders the revised candidate over the real gameplay biome (the deliverable
read) plus a clean hero shot and a 4x zoom of every flap frame so the big-facet
tiles, great-circle seams, chrome vest, spotlight wedges, and tail light-streak
can all be judged at both zoom and 40px. Preview only — writes under docs/, not
...
OUT = "docs/store_redesign/costume/disco/design_5/round_2.png"
...
def main():
    ...
    sheet.blit(title.render("D5 MIRRORBALL — R2", True, (240, 244, 252)), (GAP, 20))
    ...
    for f in range(4):
        ...
        small_read = pygame.transform.smoothscale(
            crop, (40, int(40 * crop.get_height() / crop.get_width())))
