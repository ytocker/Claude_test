...
T_EDGE   = ( 60,  36,  20)
# A true keyline, not the body paint dimmed. T_EDGE is H24 S0.50 against a
# board body of H26 S0.49 - the same pigment 38 luma darker, which is why no
# silhouette on this screen ever exceeded a third of a value range.
KEYLINE  = ( 22,  14,  10)
IRON     = ( 62,  56,  60)
IRON_HI  = (132, 128, 134)
ROPE     = (198, 166, 106)
# Was (128,96,52) = luma 101 against a day sky of 106: the rope lost its outer
# contour entirely in daylight and read as a hair. This holds at both poles.
ROPE_D   = ( 74,  55,  30)
...
# The enamel face of the START board - the "colour of the start button". The
# shipped scarlet is H0.0 at chroma 0.725 against a gold of H43.6 / 0.690:
# delta-H 44 and a chroma ratio of 0.95x, so it fails both arms of the
# separation gate and reads as a hotter version of the timber it sits in.
# START's board is entirely below y468, on the constant ink plate, so it is
# never adjacent to the blue sky - the face colour is genuinely free.
CTA_FACES = {
    "control":    ((240,  55,  55), (148,  20,  20)),
    "jade":       ((120, 196, 164), ( 34,  96,  78)),
    "kingfisher": (( 72, 176, 212), ( 18,  88, 124)),
    "lapis":      (( 86, 118, 208), ( 24,  44, 110)),
}
CTA = os.environ.get("CTA", "control")
BEST_SCORE = int(os.environ.get("BEST", "47"))
CTA_TOP, CTA_BOT = CTA_FACES[CTA]
...
def _board_points(w, h, chamfer=5, notch=5):
    ...
    # w-1 / h-1, not w / h: the last valid index on a (w,h) surface.
    xr, yb = w - 1, h - 1
    return [
        (chamfer, 0), (xr - chamfer, 0), (xr, chamfer),
        (xr - notch, h * 0.5), (xr, yb - chamfer), (xr - chamfer, yb),
        (chamfer, yb), (0, yb - chamfer), (notch, h * 0.5), (0, chamfer),
    ]
...
_T_MID_L = 0.299 * T_MID[0] + 0.587 * T_MID[1] + 0.114 * T_MID[2]


def _timber_at(target):
    """The board ramp for a body of `target` luma, on the timber hue.

    Exposure is authored as a luma target rather than a 0-1 dial because the
    ladder has to be read against measured backdrops - open sky at L106 by day
    and L20 at night - and a dial gives no way to aim at those.
    """
    def scaled(luma):
        k = luma / _T_MID_L
        return tuple(min(255, max(0, int(round(c * k)))) for c in T_MID)
    return scaled(target * 1.28), scaled(target * 0.70)


def timber_board(w, h, seed=0, chamfer=5, notch=5, plain=False, exposure=96):
    ...
    `exposure` is how much light this board catches. Every board used to share
    one T_LIT->T_DARK ramp, so the three signs and the post were identical in
    value - the chain had no ladder at all, which is most of what read as
    ripe. Exposure ladders them instead: nearest the cloud is brightest, the
    post is deepest and reads as structure rather than signage.
    """
    rnd = random.Random(seed)
    w, h = int(w), int(h)
    lit, dark = _timber_at(exposure)
    body = pygame.Surface((w, h), pygame.SRCALPHA)
    _grad_fill(body, (0, 0, w, h), lit, dark)

    for _ in range(max(3, h // 5)):
        gy = rnd.uniform(h * 0.12, h * 0.9)
        col = dark if rnd.random() < 0.6 else _mix(dark, KEYLINE, 0.5)
        pts = []
    ...
    # Two-step contour: a lit crown on the top arris carries the silhouette
    # against the L20 night sky, the dark keyline carries it against the L106
    # day sky. Neither alone holds at both poles.
    crown = _mix(lit, (255, 226, 178), 0.42)
    pygame.draw.line(body, crown, (chamfer, 1), (w - chamfer, 1), 2)
    pygame.draw.line(body, _mix(crown, (255, 236, 196), 0.5),
                     (chamfer + 2, 0), (w - chamfer - 2, 0), 1)
    pygame.draw.line(body, KEYLINE, (chamfer, h - 1), (w - chamfer, h - 1), 1)

    if plain:
        pygame.draw.rect(body, KEYLINE, (0, 0, w, h), 1)
        return body
    ...
    # A width-1 stroke rasterises differently from the polygon fill along the
    # chamfer and notch diagonals, so the mask edge fell outside the stroke and
    # the left edge exposed raw body colour on half its rows. Stroke wide
    # enough to cover any disagreement, then re-clip so the silhouette is
    # unchanged and the keyline is its outermost pixels.
    pygame.draw.polygon(body, KEYLINE, pts, 3)
    body.blit(mask, (0, 0), special_flags=pygame.BLEND_RGBA_MIN)
    return body
...
def draw_signchain(surf):
    ...
    # Exposure falls with distance from the cloud the light comes off.
    rows = [("STORE", "coin", (112 + dx, 386), -3.0, bw, bh, 1.00),
            ("TOP 10", "trophy", (118 + dx, 446), 2.4, bw, bh, 0.72),
            ("SETTINGS", "gear", (110 + dx, 506), -1.6, bw, bh, 0.48)]
    ...
    rects = {}
    for label, kind, (cx, cy), ang, w, h, expo in rows:
        rad = math.radians(-ang)
        for sgn, apt in zip((-1, 1), anchors):
            ox = sgn * (w * 0.36)
            hx = cx + ox * math.cos(rad)
        ...
        if label == "START":
            ...
            board = timber_board(w, h, seed=sum(map(ord, label)) % 997,
                                 chamfer=CHAMFER, notch=NOTCH, exposure=expo)
            if kind == "coin":
            ...
            _hud._tracked_label(board, label, (LABEL_CX, h // 2 + 1), 17,
                                color=KEYLINE, track=2, alpha=150)
            _hud._tracked_label(board, label, (LABEL_CX, h // 2 - 1), 17,
                                color=GOLD_PALE, track=2, alpha=250)
...
def _best_tag(surf, cx, cy, best):
    ...
    score is absent from the shipped menu entirely. A small tag on the post
    surfaces it without disturbing anything above."""
    w, h = 104, 24
    tag = timber_board(w, h, seed=5, chamfer=4, notch=0, exposure=88)
...
def draw_start_B(surf, tails):
    ...
    # The post is the deepest member and the only one with no lit crown: it is
    # structure, while every board above it is signage.
    post = timber_board(24, 120, seed=7, plain=True, exposure=70)
    surf.blit(post, (264, 494))
    brace = timber_board(74, 12, seed=11, plain=True, exposure=70)
    brace = pygame.transform.rotozoom(brace, 38, 1.0)
    surf.blit(brace, brace.get_rect(center=(236, 566)).topleft)
    # The old contact shadow was cast at (264,588,24,22), which is ON the post
    # rather than under it - and there is no floor down there to receive it. It
    # only ever subtracted ~20 luma from the piece with the least margin.
    soft_shadow(surf, rect, "raised")
    board = timber_board(rect.width, rect.height, seed=21, chamfer=9, notch=0,
                         exposure=110)
    ...
    _best_tag(surf, 276, 610, BEST_SCORE)
    return rect
