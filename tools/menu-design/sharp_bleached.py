...
# ── Iron / rope palette ─────────────────────────────────────────────────────
# The dark-timber ramp this fork inherited is gone: nothing on the menu is
# painted in it any more. T_EDGE survives only as the seating shadow under
# nail heads and iron rings.
T_EDGE   = ( 60,  36,  20)
IRON     = ( 62,  56,  60)
IRON_HI  = (132, 128, 134)
ROPE     = (198, 166, 106)
# The rope's dark strand used to be L102 — a dead match for the veiled day sky
# at L106, so in daylight the rope lost its outer contour and read as a hair.
# L58 clears 35 against both poles, which is what the two-step rule demands.
ROPE_D   = ( 76,  55,  28)

# ── Limewash ladder ─────────────────────────────────────────────────────────
# The figure/ground inversion. Every earlier pass painted dark furniture onto
# a sky that swings L106 (day) → L20 (night), so the boards fought the night
# phase. Light bodies invert that: the composition GAINS contrast as the sky
# falls. Values are chosen so every body clears delta 35 against BOTH poles —
# which rules out the whole band L71..L140 (too close to the day sky) and the
# band below L55 (too close to the night sky).
#
# Hue stays on the shipped structural axis (ambient.strut H34, metal_hi H36,
# block H40): weathered structure in this game is desaturated WARM, never blue.
# Saturation sits at HSV 0.17-0.19 — the top of the intended band rather than
# the middle of it, because at these lightnesses every step further toward
# neutral trades weathered timber for moulded plastic.
LW_START    = (206, 192, 170)   # L194 — the CTA carcass tops the ladder
LW_STORE    = (192, 178, 157)   # L180
LW_TOP10    = (180, 166, 146)   # L168
LW_SETTINGS = (170, 157, 137)   # L159 — the ladder's floor, set so that even
                                # after the gradient and the weather band the
                                # darkest body row still clears 141

# The pile. It was first painted as a dark tarred timber, on the assumption
# that its background was the L106 day sky like everything else. Measuring the
# veiled backdrop under each piece killed that: only the STORE board actually
# hangs against open sky. From the TOP 10 rung downward the ground is the
# mountain silhouette, which sits at L36-47 in day and L20-26 at night — so a
# dark pile at L57 cleared the nominal test on paper while quietly vanishing
# into the hillside behind it (delta 20). Limewashed, it clears both the
# nominal day sky (delta 40) and its real local ground (delta 109).
PILE_TOP = (166, 152, 130)      # L154
PILE_BOT = (158, 144, 122)      # L146
# Texture inside a narrow value window has to be bought without luma. These
# two are ~0.2 luma apart and far apart in hue, so the grain reads as damp
# streaking and salt bloom while the value stays pinned.
PILE_WARM = (172, 146, 116)     # L150
PILE_COOL = (146, 154, 140)     # L150

# A limewashed board against a pale day sky is the direction's real risk: two
# light shapes with nothing between them. The keyline is what stops it going
# chalky, so it is deliberately heavy — 2px, and dark enough to be read as a
# shadowed sawn edge rather than a hairline.
KEYLINE  = ( 28,  21,  14)      # L22

# Ink. The letterforms are cut into the board, not applied to it, so the type
# is the DARKEST thing on the sign and its highlight is a single pixel of
# reflected light on the lower wall of the groove.
INK      = ( 72,  48,  20)      # L52  — paint-filled cut
INK_DEEP = ( 40,  26,  10)      # L29  — shadowed upper wall of the groove
INK_MID  = (104,  74,  36)      # L79  — scorched shoulder inside the cut
WEAR     = (118,  84,  48)      # L90  — brown showing through worn limewash

# START keeps the rust axis (it is the system-wide primary-CTA treatment, not
# a menu-local choice) but with the range widened top→bottom, per the shared
# brief. Overridden locally; game/hud.py is untouched.
RUST_TOP = (238, 72, 34)
RUST_BOT = ( 86, 16, 10)
...
def _wash_streaks(body, w, h, rnd, base):
    """Brush marks. Limewash is scrubbed on with a stiff brush and never
    covers evenly, so the wash itself has to have direction — without this
    a flat light body reads as moulded plastic, not painted timber."""
    grain = _shift(base, -30)
    pale = _shift(base, 16)
    for _ in range(max(4, h // 4)):
        gy = rnd.uniform(h * 0.10, h * 0.92)
        light = rnd.random() < 0.38
        col = pale if light else grain
        a = rnd.randint(38, 74) if light else rnd.randint(52, 96)
        x0 = rnd.randint(0, max(1, w // 3))
        x1 = rnd.randint(w - w // 3, w)
        line = pygame.Surface((w, 3), pygame.SRCALPHA)
        pts = []
        for gx in range(x0, x1 + 6, 6):
            pts.append((gx, 1 + math.sin(gx * 0.05 + gy) * 0.8))
        if len(pts) > 1:
            pygame.draw.lines(line, (*col, a), False, pts, 1)
        body.blit(line, (0, int(gy) - 1))


def _rub_through(body, w, h, rnd, base):
    """Wear points. Where hands, rope and weather touch a limewashed board the
    wash thins and the brown underneath comes back — that is the difference
    between 'worn' and 'freshly painted'. Concentrated at the ends and the
    lower edge, which is where a hung sign actually gets handled and rained on."""
    for _ in range(rnd.randint(3, 5)):
        edge = rnd.random() < 0.7
        cx = (rnd.uniform(2, w * 0.18) if rnd.random() < 0.5
              else rnd.uniform(w * 0.82, w - 2)) if edge else rnd.uniform(0, w)
        cy = rnd.uniform(h * 0.15, h * 0.9)
        rw = rnd.uniform(5, 13)
        patch = pygame.Surface((int(rw * 2) + 4, 7), pygame.SRCALPHA)
        for k in range(4):
            a = rnd.randint(18, 44)
            yy = rnd.randint(1, 5)
            ln = rnd.uniform(rw * 0.5, rw)
            pygame.draw.line(patch, (*WEAR, a),
                             (rw - ln + 2, yy), (rw + ln + 2, yy), 1)
        body.blit(patch, (int(cx - rw) - 2, int(cy) - 3))


def limewash_board(w, h, base, seed=0, chamfer=5, notch=5, plain=False):
    """One limewashed board. Light warm-neutral body, brush-direction wash,
    rub-through at the wear points, and a heavy dark keyline — the keyline is
    load-bearing here, because a pale board on a pale day sky has nothing else
    holding its edge."""
    rnd = random.Random(seed)
    w, h = int(w), int(h)
    body = pygame.Surface((w, h), pygame.SRCALPHA)
    # Shallow gradient only: ±6 keeps even the darkest row of the palest board
    # clear of the day sky, which a conventional lit-from-above ramp would not.
    _grad_fill(body, (0, 0, w, h), _shift(base, 6), _shift(base, -6))

    _wash_streaks(body, w, h, rnd, base)
    _rub_through(body, w, h, rnd, base)

    for _ in range(1 if w < 90 else 2):
        kx = rnd.uniform(w * 0.15, w * 0.85)
        ky = rnd.uniform(h * 0.3, h * 0.7)
        kr = rnd.uniform(2.0, 3.2)
        pygame.draw.ellipse(body, _shift(base, -78),
                            (kx - kr, ky - kr * 0.72, kr * 2, kr * 1.45))
        pygame.draw.ellipse(body, _shift(base, -40),
                            (kx - kr * 1.9, ky - kr * 1.3, kr * 3.8, kr * 2.6), 1)

    # Weathering band along the bottom edge — rain runs down and sits there.
    band = pygame.Surface((w, 5), pygame.SRCALPHA)
    for y in range(5):
        band.fill((0, 0, 0, 0))
        pygame.draw.line(band, (*_shift(base, -34), int(10 + y * 8)),
                         (0, y), (w, y))
        body.blit(band, (0, h - 5 + y - 4))

    # Two-step contour, inverted for a light body: the 1px crown is a real
    # limewash catch on the top arris, the 2px keyline below is what separates
    # the board from a bright sky. Crown is capped near L206 so the woodwork
    # never out-shouts the type.
    crown = _shift(base, int(min(26, max(8, 206 - _luma(base)))))
    pygame.draw.line(body, crown, (chamfer, 1), (w - chamfer, 1), 1)
    pygame.draw.line(body, _shift(base, -34), (chamfer, h - 2), (w - chamfer, h - 2), 1)

    if plain:
        pygame.draw.rect(body, KEYLINE, (0, 0, w, h), 2)
        return body
    ...
    pygame.draw.polygon(body, KEYLINE, pts, 2)
    return body


def pile_board(w, h, seed=0):
    """Post and brace: the same limewash family as the signs, a step below the
    darkest of them so the CTA's plinth never out-values the CTA."""
    rnd = random.Random(seed)
    w, h = int(w), int(h)
    body = pygame.Surface((w, h), pygame.SRCALPHA)
    _grad_fill(body, (0, 0, w, h), PILE_TOP, PILE_BOT)
    for _ in range(max(3, h // 6)):
        gy = rnd.uniform(1, h - 2)
        pygame.draw.line(body, rnd.choice((PILE_WARM, PILE_COOL)),
                         (rnd.randint(0, w // 3), gy),
                         (rnd.randint(w - w // 3, w), gy), 1)
    # Lit arris on top, so the pile still models as a round-ish timber rather
    # than a flat bar despite its 8-luma internal range.
    pygame.draw.line(body, (200, 188, 166), (2, 1), (w - 3, 1), 1)
    pygame.draw.rect(body, KEYLINE, (0, 0, w, h), 2)
    return body


def timber_board(w, h, seed=0, chamfer=5, notch=5, plain=False):
    """Compatibility shim for the legacy call sites (perch beam, variants A/C)
    that this direction does not repaint."""
    return pile_board(w, h, seed)


def _engrave(src):
    """Turn a rendered icon into a cut one: map its own luminance onto the ink
    ramp so internal detail survives as tonal engraving instead of collapsing
    into a solid silhouette, and return a matching light stamp for the lower
    wall of the groove. Small tiles (~26px), so the per-pixel pass is cheap."""
    w, h = src.get_size()
    ...
    for y in range(h):
        for x in range(w):
            ...
            if a < 24:
                continue
            t = min(1.0, max(0.0, _luma((r, g, b)) / 255.0)) ** 0.85
            cut.set_at((x, y), (int(INK_DEEP[0] + (INK_MID[0] - INK_DEEP[0]) * t),
                                int(INK_DEEP[1] + (INK_MID[1] - INK_DEEP[1]) * t),
                                int(INK_DEEP[2] + (INK_MID[2] - INK_DEEP[2]) * t),
                                a))
            lip.set_at((x, y), (*GOLD_PALE, int(a * 0.55)))
    return cut, lip


def engraved_icon(board, kind, cx, cy, r=12):
    """The board icons are cut, not applied. Gold pictograms sat at L192 on a
    limewash body at L151–L194 — invisible — and gold has exactly one job in
    this composition now, which is the START rim."""
    box = r * 2 + 8
    scratch = pygame.Surface((box, box), pygame.SRCALPHA)
    if kind == "coin":
        _hud._coin_icon(scratch, box // 2, box // 2, r)
    elif kind == "trophy":
        _hud._draw_trophy(scratch, box // 2, box // 2, r - 2)
    else:
        _hud._draw_gear(scratch, box // 2, box // 2, r)
    cut, lip = _engrave(scratch)
    board.blit(lip, (cx - box // 2, cy - box // 2 + 1))
    board.blit(cut, (cx - box // 2, cy - box // 2))


def engraved_label(board, text, center, size, track=2):
    """A three-pass groove: shadowed upper wall, lit lower lip, paint-filled
    cut on top. This is the whole direction in one detail — the type has to
    read as removed material, or a light board is just a light rectangle."""
    cx, cy = center
    _hud._tracked_label(board, text, (cx, cy - 1), size,
                        color=INK_DEEP, track=track, alpha=95)
    _hud._tracked_label(board, text, (cx, cy + 1), size,
                        color=GOLD_PALE, track=track, alpha=185)
    _hud._tracked_label(board, text, (cx, cy), size,
                        color=INK, track=track, alpha=255)
...
def draw_signchain(surf):
    ...
    # Each rung is a step darker than the one above it. Three identical bodies
    # at 60px pitch is most of what read as repetitive; a 15-luma ladder gives
    # the chain a direction of travel down toward the CTA without moving a
    # single rect.
    rows = [("STORE", "coin", (112 + dx, 386), -3.0, bw, bh, LW_STORE),
            ("TOP 10", "trophy", (118 + dx, 446), 2.4, bw, bh, LW_TOP10),
            ("SETTINGS", "gear", (110 + dx, 506), -1.6, bw, bh, LW_SETTINGS)]
    ...
    rects = {}
    for label, kind, (cx, cy), ang, w, h, base in rows:
        rad = math.radians(-ang)
        for sgn, apt in zip((-1, 1), anchors):
            ox = sgn * (w * 0.36)
            hx = cx + ox * math.cos(rad)
        ...
        board = limewash_board(w, h, base, seed=hash(label) % 997,
                               chamfer=CHAMFER, notch=NOTCH)
        engraved_icon(board, kind, 30, h // 2, 12)
        engraved_label(board, label, (LABEL_CX, h // 2), 17)
        # Two forged nails per board, set clear of the V-notch at h*0.5. They
        # are the only cold-grey note on a warm body, which is what keeps the
        # end margins from reading as empty.
        nail(board, 10, int(h * 0.30), 3)
        nail(board, w - 10, int(h * 0.30), 3)
...
def _enamel(board, w, h, fw, fh, size, word="START"):
    """The rust enamel plate, recessed into the limewash carcass.

    Two things change versus the shipped pill. The gradient's internal range
    widens (top L117 → bottom L36, up from 48 luma) so the plate has real
    modelling instead of a flat wash. And the plate no longer touches sky at
    all — the carcass surrounds it — which is what actually cures the measured
    flaw where the pill's bottom third at L47 mushed into a night sky at L20.
    """
    face = pygame.Rect(0, 0, fw, fh); face.center = (w // 2, h // 2)
    _grad_fill(board, (face.x, face.y, face.width, face.height),
               RUST_TOP, RUST_BOT)
    frost = pygame.Surface((face.width, face.height // 2), pygame.SRCALPHA)
    frost.fill((255, 255, 255, 22)); board.blit(frost, face.topleft)

    # Recess walls: the cut's upper/left wall is in shadow, its lower/right
    # wall catches the same light the boards do. Without this the plate reads
    # as a sticker laid on the timber rather than let into it.
    rim = face.inflate(6, 6)
    pygame.draw.lines(board, KEYLINE, False,
                      [(rim.left, rim.bottom), (rim.left, rim.top),
                       (rim.right, rim.top)], 2)
    pygame.draw.lines(board, (226, 214, 194), False,
                      [(rim.left + 1, rim.bottom), (rim.right, rim.bottom),
                       (rim.right, rim.top + 1)], 2)

    # Gold's one remaining job in the composition: the CTA rim. 3px, so it
    # holds the plate's edge at both poles on its own.
    pygame.draw.rect(board, GOLD_BRIGHT, face, 3)
    pygame.draw.rect(board, KEYLINE, face.inflate(2, 2), 1)
    pygame.draw.line(board, (*GOLD_PALE, 170), (face.left + 9, face.top + 4),
                     (face.right - 9, face.top + 4), 1)

    f = _hud._font(size, True)
    img = f.render(word, True, (255, 244, 222))
    r = img.get_rect(center=(face.centerx, face.centery - 1))
    out = f.render(word, True, (74, 12, 8)); out.set_alpha(200)
    for ox, oy in ((-1, 0), (1, 0), (0, -1), (0, 1), (1, 1)):
        board.blit(out, (r.x + ox, r.y + oy))
    board.blit(img, r.topleft)
    return face
...
def draw_start_B(surf, tails):
    ...
    post = pile_board(24, 120, seed=7)
    surf.blit(post, (264, 494))
    brace = pile_board(74, 12, seed=11)
    brace = pygame.transform.rotozoom(brace, 38, 1.0)
    surf.blit(brace, brace.get_rect(center=(236, 566)).topleft)
    # No contact shadow at the pile's foot: there is no floor down there for
    # one to fall on, and the rect it was drawn in landed ON the post, taking
    # 20 luma off the only piece with no margin to give.
    soft_shadow(surf, rect, "raised")
    board = limewash_board(rect.width, rect.height, LW_START, seed=21,
                           chamfer=9, notch=0)
    face = _enamel(board, rect.width, rect.height, 110, 46, 28)
    # Double chevron under the word, cut rather than gilded — the extra height
    # carries the emphasis the narrower board gives up, and keeping it in ink
    # leaves the rim as the only gold in the frame.
    for oy in (14, 21):
        cy2 = face.bottom + oy - 6
        pts = [(rect.width // 2 - 11, cy2 - 4), (rect.width // 2, cy2),
               (rect.width // 2 + 11, cy2 - 4)]
        pygame.draw.lines(board, (*GOLD_PALE, 170), False,
                          [(px, py + 1) for px, py in pts], 2)
        pygame.draw.lines(board, INK, False, pts, 2)
    surf.blit(board, rect.topleft)
    under_shade(surf, rect, height=6, alpha=42)
