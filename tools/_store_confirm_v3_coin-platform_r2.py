#!/usr/bin/env python3
"""
coin-platform  ·  store buy-confirmation popup v3  ·  round 2

R2 changes vs R1:
- Price numeral flipped to dark espresso (52,28,4) on cream for ~10:1 contrast;
  subtle light keyline suggests a deboss catch-light on the lower edge.
- Numeral y-nudged +3 logical px so coin disc and digit baseline share the same
  visual resting line on the plinth face.
- Unaffordable plinth stays cream with a +10 blue tint (locks are signalled by
  grey coin + grey numeral + locked BUY — not by darkening the slab itself).
- Unaffordable numeral lifted to (150,155,172) for ≥3:1 contrast on cream.
- CANCEL font nudged to 12 so BUY-13 / CANCEL-12 read as a matched pill pair.
"""
import os
import sys
...
# Espresso brown used for both the dark price numeral (affordable state) and
# the dark BUY label — keeps the two dark-on-gold/cream treatments in family.
ESPRESSO = (52, 28, 4)


def _plinth_top_edge(big, pl, plr, color=(252, 244, 222)):
...
def _price_on_plinth(big, cx_d, cy_d, price, affordable):
    """Coin glyph + numeral baseline-aligned together on the plinth face.

    The numeral y is nudged +3 logical px below coin centre so cap-height and
    coin equator share a common visual resting line on the slab — digit glyphs
    have no descenders, so their optical centre sits higher than the coin's."""
    txt = f"{price:,}"
    coin_r = sc.m(14)
    coin_d = coin_r * 2
    gap = sc.m(6)
    num_font = sc.font(22)
    # Width pass: colour doesn't matter for size
    ...
    # Digit cap-height optical centre sits ~3 px above coin equator; nudging
    # the numeral down aligns both objects to the same visual baseline.
    num_cy = cy_d + sc.m(3)
    ...
    if affordable:
        ...
        # Dark espresso on cream — ~10:1 contrast. Light keyline at narrow kw
        # gives a subtle deboss catch-light on the lower edge without lightening
        # the glyph body.
        sc.plain_text(big, txt, num_font, (num_cx, num_cy), ESPRESSO,
                      shadow_a=0, weight=sc.m(1.0),
                      keyline=(255, 240, 200), kw=sc.m(0.4))
    else:
        _contact_shadow_ellipse(big, coin_cx, cy_d + sc.m(1),
        ...
        # Lifted to (150,155,172) for ≥3:1 contrast against the cream plinth
        # face — sufficient to read "locked" without competing with the coin.
        sc.plain_text(big, txt, num_font, (num_cx, num_cy), (150, 155, 172),
...
def _patched_draw_confirm(self, surf):
    ...
    if affordable:
        ...
        # Unaffordable plinth stays cream — the locked state is signalled by
        # the grey coin, lifted-but-greyed numeral, and disabled BUY pill.
        # A +10 blue tint on the cream cools the slab slightly to feel inert
        # without reading as a dark/slate surface.
        face = sc.vgrad_stops(pl.w, pl.h, plr,
                              [(0.0, (235, 220, 185)), (1.0, (205, 190, 155))],
                              255, gamma=1.02)
        big.blit(face, pl.topleft)
        # Slightly cooled top-edge catch-light matches the tinted cream.
        _plinth_top_edge(big, pl, plr, color=(252, 244, 228))
        pygame.draw.rect(big, (140, 132, 108), pl, width=max(1, m(1)),
                         border_radius=plr)
    ...
    if affordable:
        ...
        sc.plain_text(big, "BUY", sc.font(13), buy_r.center, ESPRESSO,
    ...
    # Font nudged to 12 so the pill pair (BUY-13 / CANCEL-12) reads as siblings
    # rather than a primary + secondary at mismatched sizes.
    ...
    sc.plain_text(big, "CANCEL", sc.font(12), can_r.center, (130, 124, 148),
                  shadow_a=0, weight=m(0.6))
...
hdr = store_mod._font(16, True).render(
    "coin-platform R2 · DARK NUMERAL + CREAM-IN-BOTH-STATES",
    True, (232, 214, 160))
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_popup_v3", "coin-platform", "round_2.png")
