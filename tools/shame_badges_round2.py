"""Wall of Shame badge exploration sheet (round 2).

Round 1 explored five frame TREATMENTS; the art-director picked V5 (Tarnished
Gem-Frame) merged with V1's recessed engraved channel, then flagged that tier
read by COUNTED cues that vanish small. Round 2 commits to that merged badge and
re-encodes tier the way it must read at ~70px:

  1. Whole-shape VALUE first — bronze = darkest-warm, silver = mid-cool,
     gold-tarnished = lightest-warm. They must separate in grayscale.
  2. ONE bold backing cue — the base gem enlarged to ~radius/3.2 with a brighter
     top facet. Never a counted cue (rings/rivets/pips are gone).

This headless render proves the badge at real pocket scale: the merged badge
across all three tiers (with a grayscale strip proving value separation), a
populated 3-column wall mixing earned tiers + locked-with-progress, the V4
drooping-ribbon reserved as a single PINNED hero, and one large DETAIL view that
keeps the crack + heavy patina the grid drops. Procedural-only; reuses the
Store's "Obsidian & Gold" primitives (esp. the faceted gem) so it sits in the
real furniture. Writes the combined PNG only.
"""
...
# ── Tarnished palette ────────────────────────────────────────────────────────
# A shame badge is the desaturated INVERSE of a gold medal. Tier now reads by
# WHOLE-SHAPE VALUE first, so each triplet is tuned to a distinct grayscale band
# that survives desaturation (proven by the strip on the sheet):
#   bronze = DARKEST-warm, silver = MID-cool, gold-tarnished = LIGHTEST-warm.
# `gem` is deliberately the brightest member of each tier so the enlarged base
# gem (the single bold cue) pops against its own disc.
_TARNISH = {
    ...
    "silver": {  # mid band, cool — leans grey-blue so it can't read as a metal-warm
        "rim":  (158, 162, 172), "face": (120, 124, 136),
        "deep": (60, 64, 76),    "gem":  (196, 202, 214),
        "patina": (96, 116, 120),
    },
    "gold": {  # lightest band — dull-but-pale brass (NOT wall-of-fame gold)
        "rim":  (190, 166, 104), "face": (158, 134, 80),
        "deep": (84, 68, 34),    "gem":  (236, 214, 150),
        "patina": (118, 120, 86),
    },
}
...
# Carried from round 1, but every call now passes a HIGH-CONTRAST ink (near the
# disc's darkest) + a brighter dim fill so the glyph — the joke — is the
# strongest contrast inside the disc (art-director must-fix #5).
...
def pinned_badge(surf, cx, cy, r, tier, kind):
    ...
    pygame.draw.line(surf, lerp_color(t["rim"], WHITE, 0.3),
                     (cx, ry + 2), (cx, cy - r), 1)
    for fx in range(-9, 10, 3):              # frayed top edge
        pygame.draw.line(surf, ink, (cx + fx, ry), (cx + fx + 1, ry - 4), 1)
...
def _grayscale_luma(col):
    """Rec.601 luma of an RGB triplet — used to PROVE the three tiers separate by
    value in grayscale (the squint test the round-1 sheet failed)."""
    r, g, b = col[:3]
...
def main():
    ...
    _gradient_text(sheet, "WALL OF SHAME  —  MERGED BADGE (ROUND 2)",
    ...
    _panel(sheet, lp)
    _gradient_text(sheet, "TIER = WHOLE-SHAPE VALUE", _font(14, True),
    ...
    for tn, tk in tiers:
        ...
        # value note: the face luma of the tier (proof it's banded)
        lum = int(_grayscale_luma(_TARNISH[tk]["face"]))
        ln = _font(9, True).render(f"face luma {lum}", True, (150, 142, 150))
        sheet.blit(ln, ln.get_rect(midleft=(bx + 52, row_y + 12)))
        row_y += 96

    # grayscale strip — render the three discs, desaturate, prove separation
    strip_y = row_y + 2
    sn = _font(11, True).render("GRAYSCALE — tiers stay separable",
                                True, (190, 180, 170))
    sheet.blit(sn, sn.get_rect(center=(lp.centerx, strip_y)))
    gs = pygame.Surface((lp.w - 60, 64), pygame.SRCALPHA)
    for j, (_, tk) in enumerate(tiers):
        gcx = 50 + j * 100
    ...
    dn = _font(11, True).render("DETAIL VIEW — crack + patina reserved here",
                                True, (190, 180, 170))
    dn_y = strip_y + 14 + 64 + 22
    sheet.blit(dn, dn.get_rect(center=(lp.centerx, dn_y)))
    detail_cy = dn_y + 86
    badge(sheet, lp.centerx, detail_cy, 60, "gold", "scrooge", False, detail=True)
    dl = _font(10, True).render("THE SCROOGE", True, _GOLD_PALE)
    sheet.blit(dl, dl.get_rect(center=(lp.centerx, detail_cy + 82)))
    ...
    # next badge (art-director must-fix #2). Row height is chosen per-ROW from
    # whether any badge in that row is locked.
