"""Shelf-redesign concept 1: gem-cradle-chip (R2 — elegant revision).

R1 was clumsy because:
  - 27px dead zone between shelf_bottom=282 and chip/gems at cy=309
  - Indigo chip read as a second shelf (same colour family, no contrast)
  - coin_r=15 too large inside 28px chip height

R2 fixes:
  1. Shelf extended to card bottom (shelf_bottom=328) — one unified action panel.
  2. Gold/amber chip (PRICE_STOPS) — immediately recognisable as price.
  3. Chip at cy=280, gems at cy=309: gem auras graze chip bottom for the cradle.
  4. 3-stop shelf gradient: lighter at top (button zone) → darker at bottom (gem zone).

  SHELF_Y=215, SHELF_H=113  # shelf_bottom=328 = card_bottom
  BTN_CY=242, BTN_H=32      # btn_top=226, btn_bottom=258
  CHIP_CY=280, CHIP_H=30    # chip_top=265, chip_bottom=295 (3px gap above gem tops)
  BOT_GEM_CY=309            # fixed; auras reach up to ~y=293, grazing chip bottom

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-chip/round_2.png
"""
import os, sys, math
...
# Unified action panel stretching to card bottom.
# Buttons near top → gold chip mid → gem triad at bottom edge.
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 215, 174, 113  # shelf_bottom=328=card_bottom
BTN_W, BTN_H, BTN_RAD              = 76, 32, 9
BTN_CY, BTN_GAP                    = 242, 8              # btn_top=226, btn_bottom=258
BUY_CX = CX - (BTN_W + BTN_GAP) // 2                    # 58
CAN_CX = CX + (BTN_W + BTN_GAP) // 2                    # 142
CHIP_W, CHIP_H, CHIP_RAD           = 88, 30, 10
CHIP_CY                            = 280                 # chip_top=265, chip_bottom=295
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    sc.drop_shadow(big, chip, crad, blur=m(4), alpha=110, dy=m(2))

    if affordable:
        # Gold/amber — immediately reads as "price" and contrasts strongly with indigo shelf.
        big.blit(sc.vgrad_stops(chip.w, chip.h, crad, sc.PRICE_STOPS, 255), chip.topleft)
        sc.top_sheen(big, chip, crad, m(10), peak=40)
        sc.bevel_rim(big, chip, crad, sc.PRICE_RIM_DARK,
                     (*sc.PRICE_RIM_BRIGHT, 230), w=max(1, m(1.6)))
    else:
        face_stops = [(0.0, (62, 64, 80)), (1.0, (44, 46, 62))]
        big.blit(sc.vgrad_stops(chip.w, chip.h, crad, face_stops, 255), chip.topleft)
        sc.top_sheen(big, chip, crad, m(10), peak=16)
    ...
    if affordable:
        ...
        num_col = sc.GOLD_A_NUM   # dark brown — readable on amber
    else:
        pygame.draw.circle(big, (120, 122, 138), (coin_cx, cy), coin_r)
        pygame.draw.circle(big, (88, 90, 108), (coin_cx, cy), coin_r,
                           width=max(1, m(1)))
        num_col = (150, 154, 162)
...
def _draw_shelf(big, affordable):
    ...
    # 3-stop gradient: lighter at top (button zone) → darker at bottom (gem foundation).
    if affordable:
        shelf_stops = [(0.0, (34, 36, 72)), (0.5, (22, 24, 54)), (1.0, (12, 14, 36))]
    else:
        shelf_stops = [(0.0, (32, 34, 56)), (0.5, (22, 22, 44)), (1.0, (12, 12, 30))]
    ...
    brad = m(BTN_RAD)
    _btn(big, buy, brad, "BUY",    15, locked=not affordable)
    _btn(big, can, brad, "CANCEL", 13, is_cancel=True)

    # Gold chip inside the unified shelf panel.
    _draw_chip(big, m(CX), m(CHIP_CY), PRICE, affordable)


def _draw_bottom_gems(big, affordable):
    # Drawn after shelf — gem auras graze chip_bottom=295, creating the cradle effect.
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-chip R2  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-chip", "round_2.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

img   = Image.open(OUT)
btn   = img.getpixel((18 + 58,  54 + 242))   # BUY center
chip  = img.getpixel((18 + 100, 54 + 280))   # chip center
gem_l = img.getpixel((18 + 33,  54 + 309))   # left bottom gem
shelf = img.getpixel((18 + 100, 54 + 315))   # shelf bottom strip
print(f"size:{img.size}  btn:{btn}  chip:{chip}  gem_l:{gem_l}  shelf:{shelf}")
