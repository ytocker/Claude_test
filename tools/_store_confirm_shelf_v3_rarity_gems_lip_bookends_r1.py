"""Concept B — lip-bookends (round 1) for the v3 confirm-popup rarity gems.

Two facet_gem decorations seat on the shelf's top lip (SHELF_Y=235), framing the
price chip from above. cy=241 lets each gem straddle the shelf's upper edge — half
above, half below — which is the intended bookend read. Shown AFFORDABLE (epic
rarity hue + full aura) beside UNAFFORDABLE (muted slate + half-strength aura).

Base render is copied from _store_confirm_shelf_gems_lock_showcase.py; only the
gem placement differs. Output -> docs/store_confirm_shelf_v3/rarity-gems/lip-bookends/round_1.png
"""
import os, sys, math
...
def _draw_gems(big, affordable):
    # Concept B — lip-bookends. Two rarity gems seat on the shelf's top lip,
    # bookending the price chip from above. cy=241 straddles the shelf edge so
    # each gem reads as physically perched on the lip. Aura first so the halo
    # wells up behind the facet body rather than washing over it.
    if affordable:
        base, deep = PAL["gem"], PAL["deep"]
        glow, peak = PAL["glow"], 22
    else:
        base, deep = (80, 82, 100), (50, 52, 66)
        glow, peak = (90, 92, 110), 11
    for cx in (27, 173):
        sc._alpha_aura(big, m(cx), m(241), m(11), glow, peak=peak, layers=10)
        sc.facet_gem(big, m(cx), m(241), m(8), base, deep)
...
draw.text((CANVAS_W // 2, HDR_H // 2), "B — lip-bookends",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   ...
                   "lip-bookends", "round_1.png")
OUT = os.path.abspath(OUT)
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

# ── PIL verification (affordable panel — both bookend gem centres) ─────────────
_aff = render_popup("TEMPEST", True)
_im  = Image.frombytes("RGB", (POP_W, POP_H), pygame.image.tostring(_aff, "RGB"))
print("\n=== lip-bookend gem-centre checks (affordable) ===")
for (x, y) in [(27, 241), (173, 241)]:
    p = _im.getpixel((x, y))
