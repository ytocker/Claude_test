"""ONE comparison figure for the WATER BOTTLE parcel redesign (v2).

Renders the shipped WATER BOTTLE (original squeeze bottle) + the 5 design
candidates, each carried below Pip mid-flight over a real gameplay scene (DAY on
top, NIGHT below), side by side and labelled. Pure capture — production art
untouched.

    SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy python tools/render_water_bottle_compare.py
"""
...
from game.parcel_designs import water_bottle as original


def _load_build(n):
    p = ROOT / "docs" / "store_redesign" / "parcels" / "water_bottle" / "v2" / f"design_{n}" / "build.py"
    spec = importlib.util.spec_from_file_location(f"wb_design_{n}", p)
...
NAMES = ["SQUEEZE BOTTLE (shipped)", "DISPOSABLE BOTTLE", "INSULATED TUMBLER",
         "HIKER CANTEEN", "COCONUT DRINK", "GALLON JUG"]
...
def main():
    ...
    sheet.blit(big.render("WATER BOTTLE parcel — shipped vs 5 redesigns "
    ...
    out = ROOT / "docs" / "store_redesign" / "parcels" / "water_bottle" / "v2" / "final_comparison.png"
