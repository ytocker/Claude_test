"""Round-1 review-sheet renderer for all 5 penguin redesign candidates.

For each design it composites: a 4-frame flap filmstrip, a day gameplay panel,
a night gameplay panel, a clean hero product shot, and a 40px NEAREST "truth
read" — into one labeled sheet under docs/store_redesign/animal/penguin/.

Run headless from repo root:
``SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy python tools/penguin_candidates/_render_all.py``
"""
...
def _night_gameplay_panel(source, w, h, *, frame_idx=2, tilt=10.0):
    """Same as nr.gameplay_panel but over the night biome phase, to check that
    glow/contrast survives a dark sky (matters for AURORA KING especially)."""
    scene = pygame.Surface((GW, GH))
    palette = biome.palette_for_phase(0.64)   # deep night
...
def _filmstrip(build, cell=80):
    """4-frame flap filmstrip on a flat dark panel."""
    ...
    for i in range(4):
        ...
        pygame.draw.rect(strip, (60, 56, 74),
                         (i * cell, 0, cell, cell), 1)
    return strip


def _truth_read(build):
    """40px NEAREST downscale — the thumbnail legibility bar, on a swatch."""
    ...
    small = pygame.transform.scale(
        fr, (max(1, int(sw * sc)), max(1, int(sh * sc))))
    # show it at native 40px AND a 2x NEAREST blow-up so we can see what's lost
...
def render_design(slug, name):
    build = importlib.import_module(
        f"tools.penguin_candidates.{slug}").build
    ...
    sheet_h = 60 + max(440, 80 + pad + 120 + pad + 120) + pad
    ...
    _label(sheet, "FLAP", col3 + 4, y0 + 250, size=14)
