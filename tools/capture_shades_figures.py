"""Two combined review figures for the SHADES store category.

  1. store_overview.png  — the real store SHADES tab, every page side by side
                           (pixel-faithful App._render captures, composed).
  2. gameplay_items.png  — each shades item on Pip mid-flight over a real biome
                           scene, in one labeled grid.

Run headless from repo root:
``SDL_VIDEODRIVER=dummy python tools/capture_shades_figures.py``
"""
...
from game.store import StoreScene, _GROUP_TAB
from game.hud import _font, _GOLD_PALE, _GOLD_DEEP
from game.draw import get_sky_surface_biome, draw_mountains, draw_ground, draw_cloud
from game.entities import Pipe
from game.config import W as GW, H as GH, GROUND_Y

OUT_DIR = os.path.join(_ROOT, "docs", "store_redesign", "shades")
os.makedirs(OUT_DIR, exist_ok=True)

SHADES = store_catalog.ids_of_group("shades")
_SHADES_TAB = _GROUP_TAB["shades"]
GAME_FRAME_IDX = 2
GAME_TILT = 10.0
...
def build_store_overview() -> str:
    ...
    sheet.fill((18, 16, 28))
    title = _font(30, True).render("SHADES — STORE TAB (all pages)", True, _GOLD_PALE)
...
# ── Figure 2: each shades item on Pip in gameplay ────────────────────────────
...
def build_gameplay_items() -> str:
    ...
    sheet = pygame.Surface((sheet_w, sheet_h))
    sheet.fill((18, 16, 28))
    title = _font(30, True).render("SHADES — ON PIP IN GAMEPLAY", True, _GOLD_PALE)
    sheet.blit(title, title.get_rect(midtop=(sheet_w // 2, 22)))
    label_font, cost_font = _font(16, True), _font(13, True)
    for idx, sid in enumerate(SHADES):
