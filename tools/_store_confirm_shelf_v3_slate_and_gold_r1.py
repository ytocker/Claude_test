"""Round 1 — shelf concept 'slate-and-gold' (store_confirm_shelf_v3 series).
...
becomes the single hero of warmth against the cool slate. CANCEL recedes to a
muted bronze; the price chip is a warm off-white ledger tile, never pure cream.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE. Verify with PIL, never by viewing.
Output -> docs/store_confirm_shelf_v3/slate-and-gold/round_1.png
"""
import os, sys, math
...
# Gold bevel shared by BUY + CANCEL — the warm rim reads gilded on both, so the
# button pair feels like one struck-metal control cluster.
GOLD_DEEP   = (58, 48, 22)
...
def _btn(big, rect, rad, label, fill_stops, lab_col, lab_fs, affordable):
    ...
    if affordable:
        sc.bevel_rim(big, rect, rad, GOLD_DEEP, (*GOLD_BRIGHT, 230), w=max(1, m(2.0)))
    else:
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    # never competes with the gold BUY button for "press me". A gold hairline
    # under the numeral echoes the button rim without turning the chip into a CTA.
    chip = pygame.Rect(0, 0, m(CHIP_W), m(CHIP_H)); chip.center = (cx, cy)
    crad = m(CHIP_RAD)
    sc.drop_shadow(big, chip, crad, blur=m(3), alpha=90, dy=m(2))

    if affordable:
        face_stops = [(0.0, (224, 214, 190)), (1.0, (198, 188, 162))]
    else:
...
def _draw_shelf(big, affordable):
    ...
    brad = m(BTN_RAD)
    # BUY IS the gold: full hero gold-fill + espresso ink.
    _btn(big, buy, brad, "BUY",
         [(0.0, (242, 200, 96)), (1.0, (206, 158, 54))], (38, 28, 8), 14, affordable)
    # CANCEL recedes to muted bronze + low-contrast parchment label.
    _btn(big, can, brad, "CANCEL",
         [(0.0, (184, 154, 84)), (1.0, (154, 120, 48))], (150, 132, 96), 13, affordable)
...
draw.text((CANVAS_W // 2, 30),
          "slate-and-gold r1  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "slate-and-gold", "round_1.png")
...
print(f"BUY rim @ ({BUY_CX},{BTN_CY - BTN_H//2 + 1}): {buy_rim}  "
      f"-> warm gold R>G>B: {'OK' if buy_rim[0] > buy_rim[2] else 'check'}")
