"""Final comparison figure for the basketball costume redesign exploration.

Seven columns — the ORIGINAL sports basketball design plus the five explored
designs — each Pip mid-flight over the same real gameplay biome scene.

Run headless from repo root:
    SDL_VIDEODRIVER=dummy python tools/render_basketball_compare.py
"""
...
def _source(spec):
    if spec == "orig":
        return importlib.import_module("tools.sports_candidates.design_2").build
    return importlib.import_module(f"tools.basketball_candidates.{spec}").build
