"""raised-pedestal (A): BUY differentiated by physical mass + lift.

BUY is a chunky forward-sitting lozenge (90x34, indigo, gold bevel, drop
shadow); CANCEL is a smaller flat tab (64x28, dimmer). Dominance reads from
size + shadow depth, not colour — so the pair stays inside the card's indigo
family. Locked BUY drops to slate + padlock with a contact pass only (a wide
soft shadow is invisible on the indigo shelf).

Shelf, chip, card body, gems, name, banner, disc/aura/thumb: unchanged from
the c-orig-bg base.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444x412
Output -> docs/store_confirm_shelf_v3/buy-fancy/raised-pedestal/round_1.png
"""
import os, sys, math
...
def _buy_button(big, locked):
    # Chunky lozenge that sits forward — the mass + drop shadow are what carry
    # the "primary action" read, so colour can stay in the card's indigo.
    buy = pygame.Rect(0, 0, m(90), m(34))
    buy.center = (m(64), m(302))
    rad = m(BTN_RAD)
    ...
    sc.drop_shadow(big, buy, rad, blur=m(3), alpha=150, dy=m(2))
    big.blit(sc.vgrad_stops(buy.w, buy.h, rad,
             [(0.0, (84, 78, 126)), (1.0, (50, 46, 82))], 255), buy.topleft)
    sc.top_sheen(big, buy, rad, m(12), peak=22)
...
def _cancel_button(big):
    # Smaller flat tab — always available, so it never locks.
    can = pygame.Rect(0, 0, m(64), m(28))
    can.center = (m(149), m(302))
    rad = m(BTN_RAD)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "raised-pedestal r1  |  AFFORDABLE / UNAFFORDABLE",
...
print(f"BUY center {aff.getpixel((64, 302))}, CANCEL center {aff.getpixel((149, 302))}")
uaf = panels_data[1][2]
print(f"locked BUY center (unaffordable) {uaf.getpixel((64, 302))}")
