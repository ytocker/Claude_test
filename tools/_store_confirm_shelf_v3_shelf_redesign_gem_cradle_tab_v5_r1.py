"""Shelf-redesign: gem-cradle-tab v5 (progressive lower, delta=18 (slim buttons H=26; chip_bottom=313, 15px overlap; most compressed)).

Variation of gem-cradle-tab opt-A: the entire action block (shelf top border,
buy/cancel buttons, price chip) steps progressively lower. Shelf bottom stays
fixed at card_bottom=328; bottom gems stay fixed at BOT_GEM_CY=309.

v5 — delta=18 (slim buttons H=26; chip_bottom=313, 15px overlap; most compressed):
  SHELF_Y=246, SHELF_H=82  # shelf_bottom=328
  BTN_CY=268, BTN_H=26
  CHIP_CY=300    # chip_bottom=313; 15px overlap; most compressed
  BOT_GEM_CY=309            # fixed

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output -> docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-tab/v5/round_1.png
"""
import os, sys, math
...
# v5 — progressive lower (delta=18 (slim buttons H=26; chip_bottom=313, 15px overlap; most compressed)).
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 246, 174, 82  # SHELF_H = 328 - SHELF_Y; shelf_bottom=328
BTN_W, BTN_H, BTN_RAD              = 76, 26, 9
BTN_CY, BTN_GAP                    = 268, 8
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD           = 88, 26, 8
CHIP_CY                            = 300   # chip_bottom=313; 15px overlap; most compressed
BOT_GEM_R, BOT_GEM_CY, BOT_GEM_LX, BOT_GEM_RX = 11, 309, 33, 167
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    if affordable:
        ...
        sc.plain_text(big, txt, num_font, (num_cx, cy + m(1)), (236, 240, 232),
                      shadow_a=0, weight=m(0.7))
        # hair underline on price number
        num_h    = num_font.size(txt)[1]
        baseline = (cy + m(1) - num_h // 2) + num_font.get_ascent()
        hair_h   = m(2)
        hair_y   = baseline + m(3)
        ...
        sc.plain_text(big, txt, num_font, (num_cx, cy + m(1)), (140, 144, 152),
                      shadow_a=0, weight=m(0.7))
...
def _draw_shelf(big, affordable):
    ...
    # Price tab inside the unified shelf panel.
    _draw_chip(big, m(CX), m(CHIP_CY), PRICE, affordable)


def _draw_bottom_gems(big, affordable):
    # Drawn after shelf — gems draw ON TOP of any chip/shelf overlap.
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-tab v5  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-tab", "v5", "round_1.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

img   = Image.open(OUT)
btn   = img.getpixel((18 + 58,  54 + 268))
chip  = img.getpixel((18 + 100, 54 + 300))
gem_l = img.getpixel((18 + 33,  54 + 309))
print(f"v5  size:{img.size}  btn:{btn}  chip:{chip}  gem_l:{gem_l}")
