"""Locked + I1 design with the popup's rarity banner loyal to the card.

The popup's stock tier banner is a notched-hex construction; the item card
...
def main():
    ...
    try:
        ...
        panels.append(("LOCKED + I1",
                       "stock notched-hex rarity banner",
                       render_pop(), render_card(patched_card),
                       render_category(patched_card)))

        set_candidate_column()
        panels.append(("LOCKED + I1 + CARD RIBBON",
                       "rarity banner redrawn with the card's own lozenge "
                       "ribbon build, scaled to popup size",
                       render_pop(banner=ribbon_banner),
        ...
        idr.text((MARGIN, 16),
                 "popup rarity banner loyal to the card's lozenge ribbon · "
