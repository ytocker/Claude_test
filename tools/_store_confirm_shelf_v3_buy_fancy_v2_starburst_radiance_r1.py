...
def _draw_rays(big):
    # Rays live on their own SRCALPHA layer so overlapping violet/gold edges
    # composite once, cleanly, before the bloom rides on top additively.
    ray_surf = pygame.Surface((m(POP_W), m(POP_H)), pygame.SRCALPHA)
    N_RAYS = 20
    for i in range(N_RAYS):
        angle = 2 * _math.pi * i / N_RAYS
        if i % 2 == 0:
            ray_col = (*sc.CARD_RING_BRIGHT, 180)          # gold
        else:
            ray_col = (*sc.RARITY["epic"]["gem"], 120)     # violet
        ...
        b2 = (base_cx - spread * _math.cos(perp_angle),
              base_cy - spread * _math.sin(perp_angle))
        pygame.draw.polygon(ray_surf, ray_col, [tip, b1, b2])

    big.blit(ray_surf, (0, 0))
...
def _draw_shelf(big, affordable):
    ...
    if affordable:
        _draw_rays(big)
        sc.soft_glow(big, m(BUY_CX), m(BTN_CY), m(32), sc.CARD_RING_BRIGHT,
                     peak_alpha=140)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "starburst-radiance (B) r1  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   ...
                   "starburst-radiance", "round_1.png")
