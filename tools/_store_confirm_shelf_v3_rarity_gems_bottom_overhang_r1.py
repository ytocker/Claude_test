"""Concept D — bottom-overhang — rarity-gem shelf placement, round 1.

Two facet gems straddle the card's bottom edge (mirroring the hero disc that
overhangs the top), joined by a gold sag-arc that bows downward between them.
The pair reads as a jewelled clasp closing the card. Shown AFFORDABLE (epic
rarity hue + full aura + bright gold arc) and UNAFFORDABLE (dead-slate gems,
half-strength aura, dull tarnished arc) so the price gate stays legible.

Everything but the bottom-edge gem placement is byte-identical to the C** base
(card body, hero gems, name, banner, indigo shelf, price chip, BUY/CANCEL row,
hero disc). Output -> docs/store_confirm_shelf_v3/rarity-gems/bottom-overhang/round_1.png
"""
import os, sys, math
...
def _draw_bottom_overhang_gems(big, affordable):
    ...
    # disc's overhang at the top. Draw order: both auras well up first, then the
    # gold sag-arc bows downward between the centres, then the facet gems cap the
    # arc ends. Muted state kills the hue and halves the halo so the lock reads.
    if affordable:
    ...
    for cx in (60, 140):
        sc._alpha_aura(big, m(cx), m(330), m(14), aura_col, peak=peak, layers=13)

    # math.pi -> 2*math.pi traces the bottom half of the ellipse, so the arc sags
    # downward like a slack chain slung between the two gem mounts.
    ...
    for cx in (60, 140):
        sc.facet_gem(big, m(cx), m(330), m(10), base, deep)
...
draw.text((CANVAS_W // 2, HDR_H // 2), "D — bottom-overhang",
