#!/usr/bin/env python3
"""
arc_count / MEDALLION concept — round 2.

Changes from round 1:
 1. Badge center moved to BCY=112, fully in the open sky below the banner hairline.
 2. Event pips: full color at 60% alpha, r=3, confined to upper 180° of ring, ink notch behind each.
 3. Badge carries ONE number ("18%") centred at true BCY; "PIL 25" label removed.
 4. Inner ring redesigned as a progress arc (dim-gold full track + 65° bright-gold sweep).
 5. Stats stack trimmed to 2 rows; left edge at x=16; BACK right edge ≤ x=88.
 6. Death callout: 62×26 chip, repositioned above-left of parrot, gold leader line, gold border only.
"""
import os
import math
import random
...
def render_screen():
    ...
    text(surf, "FLIGHT LOG", 13, center=(W//2, 28), color=GOLD, track=3, shadow=None)
    text(surf, f"DAY {DAY_N}  ·  0:{TIME_ALIVE:02d}", 10, center=(W//2, 52), color=(150, 146, 138), shadow=None)

    # NOTE 1: Badge center moved to BCY=112 — fully in open sky, ~38px below banner hairline
    BCX, BCY = 52, 112
    R_BADGE = 36   # outer ring radius on the badge surface

    # Glow behind badge
    g_badge = soft_glow(34, GOLD, peak=35, falloff=2.2)
    surf.blit(g_badge, (BCX-35, BCY-35), special_flags=pygame.BLEND_ADD)

    # Outer ring (dark fill + gold border)
    badge_lay = pygame.Surface((80, 80), pygame.SRCALPHA)
    pygame.draw.circle(badge_lay, (*INK, 220), (40, 40), R_BADGE)
    pygame.draw.circle(badge_lay, (*GOLD, 200), (40, 40), R_BADGE, 2)

    # NOTE 4: Inner ring redesigned as a progress arc
    #   (a) Full 360° arc in dim gold at ~40% alpha as the track
    #   (b) Bright GOLD arc covering 18% of circumference (65° from 12 o'clock, clockwise)
    _r_prog = 28
    # Track: draw a dot every 1° for a smooth continuous ring
    for _deg in range(0, 360):
        _ang = math.radians(_deg)
        _px = int(40 + _r_prog * math.cos(_ang))
        _py = int(40 + _r_prog * math.sin(_ang))
        pygame.draw.circle(badge_lay, (*GOLD, 100), (_px, _py), 1)
    # Progress arc: 65° clockwise from 12 o'clock (top = -90° in screen coords)
    for _deg in range(0, 66):
        _ang = math.radians(_deg - 90)   # -90° = 12 o'clock; increasing = clockwise on screen
        _px = int(40 + _r_prog * math.cos(_ang))
        _py = int(40 + _r_prog * math.sin(_ang))
        pygame.draw.circle(badge_lay, (*GOLD, 255), (_px, _py), 1)

    surf.blit(badge_lay, (BCX-40, BCY-40))

    # NOTE 3: ONE number inside badge — "18%" centred at true BCY (no -6 offset)
    f22 = pygame.font.Font(FONT_PATH, 22)
    hero_s = f22.render(f"{int(DEATH_PHASE*100)}%", True, GOLD)
    surf.blit(hero_s, (BCX - hero_s.get_width()//2, BCY - hero_s.get_height()//2))
    # "PIL 25" label removed per note 3

    # NOTE 2: Event pips — upper 180° of ring (angles 180°–360°), full color at 60% alpha,
    #          r=3, with ink notch (r=4) punched behind each pip
    PIP_COLORS = [GEYSER_C, CLOWN_C, RAIN_C, SNOW_C]
    # Spread 4 pips evenly inside the upper semicircle [190°, 350°]
    _pip_angles_deg = [190, 243, 297, 350]
    for j, pip_c in enumerate(PIP_COLORS):
        _pip_ang = math.radians(_pip_angles_deg[j])
        _px = int(BCX + math.cos(_pip_ang) * R_BADGE)
        _py = int(BCY + math.sin(_pip_ang) * R_BADGE)
        # Ink notch behind pip (r=4, opaque INK)
        pygame.draw.circle(surf, INK, (_px, _py), 4)
        # Pip at full event color, 60% alpha, r=3
        _pip_lay = pygame.Surface((10, 10), pygame.SRCALPHA)
        pygame.draw.circle(_pip_lay, (*pip_c, 153), (5, 5), 3)
        surf.blit(_pip_lay, (_px - 5, _py - 5))

    # NOTE 5: Stats stack trimmed to 2 rows; left edge at x=16; below badge bottom
    STATS_X = 16
    STATS_Y = BCY + R_BADGE + 12   # 112 + 36 + 12 = 160
    stat_rows = [
        (f"0:{TIME_ALIVE:02d} ALIVE", CREAM),
        ("4 EVENTS SEALED", (140, 138, 158)),
    ]
    for i, (label, col) in enumerate(stat_rows):
        text(surf, label, 9, midleft=(STATS_X, STATS_Y + i * 18), color=col, shadow=None)

    # NOTE 6: Death callout — 62×26 chip, repositioned into clear sky above-left of parrot
    call_w, call_h = 62, 26
    call_x = max(8, int(bx) - 70)
    call_y = int(by) - 40
    # 1px gold leader line from callout center-right to bird position
    alpha_line(surf, (*GOLD, int(255 * 0.7)),
               (call_x + call_w, call_y + call_h // 2),
               (int(bx), int(by)), 1)
    # Chip: no red accent bar — GOLD border only, 1px at 70% alpha
    c_lay = pygame.Surface((call_w, call_h), pygame.SRCALPHA)
    pygame.draw.rect(c_lay, (*INK, 180), c_lay.get_rect(), border_radius=3)   # dark fill
    pygame.draw.rect(c_lay, (*GOLD, int(255 * 0.7)), c_lay.get_rect(), 1, border_radius=3)
    surf.blit(c_lay, (call_x, call_y))
    text(surf, "ENDED HERE", 8, midleft=(call_x+6, call_y+10), color=GOLD, shadow=None)
    text(surf, f"PIL {DEATH_PILLAR}", 7, midleft=(call_x+6, call_y+21), color=(160, 156, 148), shadow=None)

    # NOTE 5: BACK — width 70 so right edge = 18+70 = 88 ≤ x=88
    back_r = pygame.Rect(18, 582, 70, 24)
    b_lay = pygame.Surface(back_r.size, pygame.SRCALPHA)
    pygame.draw.rect(b_lay, (*SCRIM, 200), b_lay.get_rect(), border_radius=5)
    pygame.draw.rect(b_lay, (*GOLD, 65), b_lay.get_rect(), 1, border_radius=5)
    surf.blit(b_lay, back_r.topleft)
    text(surf, "BACK", 9, center=back_r.center, color=CREAM, track=2, shadow=None)

    return surf, (bx, by), sun_xy


OUT_SLUG = "arc_count/medallion"
OUT_FILE = "round_2.png"


def main():
    screen, bird_xy, sun_xy = render_screen()
    out = os.path.join(ROOT, "docs", "flight_log_arc_v2", OUT_SLUG, OUT_FILE)
