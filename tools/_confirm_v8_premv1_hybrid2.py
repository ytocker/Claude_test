"""hybrid-2 · confirm_purchase_v8 · premium-v1 · user-directed hybrid, swapped

Same graft set as hybrid-1 but with the price/rarity positions swapped: the
bullion price bar sits below the item name (CHIP_CY=247) and the rarity banner
sits in the shelf flanked by the bottom gems (Y_BANNER=402) — which is the
live `_draw_confirm` layout, so the base renders unpatched.

Grafts:
  · obsidian-forge (#3): bullion price bar at its full 168 px width — nothing
    flanks it in the card body, so no narrowing is needed
...
def render_popup(tier):
    ...
    _confirm_tier_banner(ov, CX, 402, 140, 23, tier, pal)
...
def main():
    ...
    try:
        ...
        idr.text((MARGIN, 18), "hybrid-2 · premium-v1 · hybrid-1 with price/rarity swapped",
                 fill=(236, 214, 160))
        for i, tier in enumerate(TIERS):
            pop = render_popup(tier)
        ...
        out_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                               "docs", "confirm_purchase_v8", "premium-v1", "hybrid-2")
