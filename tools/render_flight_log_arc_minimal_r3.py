#!/usr/bin/env python3
"""
arc_minimal — direction D round 3: skeleton trace, pure arc with no event markers.
"""
import os
import math
import random
...
def draw_overlay(ss):
    ...
    for p, rad, fn, col in ring_items:
        continue  # Skeleton trace — no event glyphs at all
        x, y = arc_pos(p, rad)
        if rad != R_INNER:
    ...
    for pass_ink in (True, False):
        for i in range(steps):
            ...
            if pass_ink:
                ...
                col = lerp_color((220, 200, 160), (255, 250, 230), t ** 0.75)
                pygame.draw.line(ss, (*col, 255), PU(u0), PU(u1), int(core * k))
    for i in range(steps):
        u0 = u_death * i / steps
        u1 = u_death * (i + 1) / steps
        t = i / steps
        pygame.draw.line(ss, (255, 255, 248, int(90 + 90 * t)), PU(u0), PU(u1),
                         max(1, int(0.8 * k)))
...
def trail_bloom(surf):
    return  # Skeleton trace — no bloom, let the pure arc speak
