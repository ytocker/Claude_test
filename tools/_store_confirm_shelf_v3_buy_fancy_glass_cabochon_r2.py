"""BUY-fancy exploration: glass-cabochon (C) — round 2.

BUY reads as a polished cabochon gem lens: a TRUE sapphire fill (cooler,
clearly blue vs the card's purple-indigo body), a crisp white-hot specular
crown pinned to the very top edge, and a bright inner blue bevel ring for
lens depth over the outer gold rim. CANCEL keeps a modest single-bevel
treatment so BUY is the clear primary. Locked state is the base padlock
slate, unchanged.

R2 fixes over R1:
  * Sapphire fill dropped ~26 R / lifted B so the hue separates from the
    indigo card — no longer reads as "brightened indigo."
  * Added a tight specular crown (white-hot line) at the top edge for a
    glassy dome highlight instead of a soft sheen gradient alone.
  * Inner bevel brightened + thickened so it reads as a real lens rim.

Shelf background: original _draw_confirm indigo (verbatim from base).

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/buy-fancy/glass-cabochon/round_2.png
"""
import os, sys, math
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    if locked:
        # Base padlock slate — unchanged so the affordability read is stable.
    ...
    if is_cancel:
        # Modest single-bevel supporting action — BUY stays the primary read.
        stops   = [(0.0, (26, 28, 64)), (1.0, (14, 16, 44))]
        lab_col = (168, 162, 200)
        sc.drop_shadow(big, btn_rect, rad, blur=m(3), alpha=100, dy=m(2))
        big.blit(sc.vgrad_stops(btn_rect.w, btn_rect.h, rad, stops, 255),
                 btn_rect.topleft)
        sc.top_sheen(big, btn_rect, rad, m(12), peak=14)
        sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                     (*sc.CARD_RING_BRIGHT, 230), w=max(1, m(2.2)))
        sc.plain_text(big, label, sc.font(font_px), btn_rect.center, lab_col,
                      shadow_a=110, weight=m(0.8), keyline=(8, 6, 20), kw=m(0.9))
        return

    # BUY — TRUE sapphire cabochon lens. Draw order: shadow, fill, tall sheen,
    # white-hot specular crown, outer gold bevel, inner blue bevel, then label.
    # The cooler fill separates the hue from the card's purple-indigo, and the
    # crown pins a glassy dome highlight to the very top edge.
    stops   = [(0.0, (62, 82, 172)), (1.0, (32, 50, 126))]
    lab_col = (224, 230, 250)
    sc.drop_shadow(big, btn_rect, rad, blur=m(3), alpha=150, dy=m(2))
    big.blit(sc.vgrad_stops(btn_rect.w, btn_rect.h, rad, stops, 255),
             btn_rect.topleft)
    sc.top_sheen(big, btn_rect, rad, m(18), peak=60)

    # Crisp specular crown — a tight white-hot line at the very top edge that
    # falls off fast, reading as the polished cabochon's specular highlight.
    spec_h = m(5)
    spec_rect = pygame.Rect(btn_rect.left + m(8), btn_rect.top + m(2),
                            btn_rect.width - m(16), spec_h)
    spec_surf = pygame.Surface((spec_rect.w, spec_h), pygame.SRCALPHA)
    for yy in range(spec_h):
        a = int(180 * (1 - yy / max(1, spec_h - 1)) ** 0.5)
        pygame.draw.line(spec_surf, (255, 255, 255, a), (0, yy), (spec_rect.w - 1, yy))
    big.blit(spec_surf, spec_rect.topleft)

    sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                 (*sc.CARD_RING_BRIGHT, 230), w=max(1, m(2.0)))
    buy_inner = btn_rect.inflate(-m(3), -m(3))
    # Brighter, thicker blue inner ring — reads as a lens rim, not noise.
    sc.bevel_rim(big, buy_inner, max(2, rad - m(2)), (20, 28, 80, 180),
                 (180, 200, 255, 200), w=max(1, m(1.8)))
    sc.plain_text(big, label, sc.font(font_px), btn_rect.center, lab_col,
                  shadow_a=110, weight=m(0.8), keyline=(8, 6, 20), kw=m(0.9))
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "glass-cabochon (C) r2  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "buy-fancy", "glass-cabochon",
                   "round_2.png")
...
sheen_px = aff.getpixel((BUY_CX, 292))
crown_px = aff.getpixel((BUY_CX, 290))
can_px  = aff.getpixel((CAN_CX, BTN_CY))
print(f"BUY center     y={BTN_CY}: {buy_px}")
print(f"BUY top-sheen  y=292: {sheen_px}")
print(f"BUY spec crown y=290: {crown_px}")
print(f"CANCEL center  y={BTN_CY}: {can_px}")
print(f"BUY true-sapphire check (B>R): {'OK' if buy_px[2] > buy_px[0] else 'WARN'}")
