"""
store_confirm_popup_v4 — name + banner Y-position options.

Renders 4 side-by-side panels (affordable state) at different Y_NAME / Y_BANNER
values so the user can pick how far down they want the item name and rarity
banner to sit relative to the cabochon disc above and the shelf tray below.
"""
import math
import os
import sys
...
def _build_draw_fn(y_name, y_banner):
    """Return a _draw_confirm replacement with the given name/banner Y offsets."""
    ...
    def _draw(self, surf):
        ...
        # Corner gems, name, banner — Y_NAME/Y_BANNER come from closure
...
# Header
hdr_txt = "SHELF-CHIP V4 · NAME + BANNER POSITION OPTIONS"
if fhdr:
...
for i, (label, y_name, y_banner) in enumerate(OPTIONS):
    ...
    # Footer label (multi-line)
    cx    = x0 + PANEL_W // 2
    foot_y = panel_top + PANEL_H + 6
    lines  = label.split("\n")
    if flbl and len(lines) >= 2:
        l1 = flbl.render(lines[0], True, CYAN)
        l2 = fsub.render(lines[1], True, DIM) if fsub else flbl.render(lines[1], True, DIM)
        canvas.blit(l1, l1.get_rect(midtop=(cx, foot_y)))
        canvas.blit(l2, l2.get_rect(midtop=(cx, foot_y + 16)))

OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_popup_v4", "name-position-options.png")
