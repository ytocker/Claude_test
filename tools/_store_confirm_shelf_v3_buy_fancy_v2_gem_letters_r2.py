"""buy-fancy-v2 concept D — GEM-LETTERS.

Three facet gems replace the BUY word: one gem per letter (B, U, Y). The button
face is near-black indigo so the legendary-gold gems ARE the label, each haloed
by a feathered aura and captioned by a tiny letter subscript. A luxury two-bead
gold regalia border rings the button. Everything else is inherited unchanged
from the c-orig-bg hybrid shelf so the exploration isolates the BUY treatment.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/buy-fancy-v2/gem-letters/round_1.png
"""
import os, sys, math
...
def _buy_gem_letters(big, btn_rect, rad):
    # The gems ARE the label: a near-black indigo face so the legendary-gold
    # brilliants read as the hero, ringed by a two-bead gold regalia border.
    ...
    sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                 (*sc.CARD_RING_BRIGHT, 200), w=max(1, m(2.0)))
    sc._draw_regalia_frame(big, btn_rect, rad)

    gem_r  = m(10)
    gem_cx = [m(44), m(58), m(72)]     # B / U / Y within the 76px button
    gem_cy = m(BTN_CY)
    lgd    = sc.RARITY["legendary"]

    # Auras first so each gem sits on its own halo, not on its neighbour's.
    for gx in gem_cx:
        sc._alpha_aura(big, gx, gem_cy, gem_r + m(4), lgd["glow"],
                       peak=100, layers=8)
    for gx in gem_cx:
        sc.facet_gem(big, gx, gem_cy, gem_r, lgd["gem"], lgd["deep"])

    # Tiny subscripts so the B/U/Y read stays legible at 1×.
    for gx, letter in zip(gem_cx, ["B", "U", "Y"]):
        sc.plain_text(big, letter, sc.font(7),
                      (gx, gem_cy + gem_r + m(5)),
                      (200, 195, 230), shadow_a=0, weight=m(0.5))
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-letters (D) r1  |  AFFORDABLE / UNAFFORDABLE",
...
# PIL verification
aff = panels_data[0][2]
for label, (x, y) in [("center gem (58,302)", (58, 302)),
                      ("left gem  (44,302)", (44, 302)),
                      ("right gem (72,302)", (72, 302))]:
    px_ = aff.getpixel((x, y))
    lit = px_[0] > 90 and px_[0] >= px_[2]
    print(f"{label}: {px_}  → {'OK gem-gold' if lit else 'WARN not gold'}")
can_px = aff.getpixel((CAN_CX, BTN_CY))
print(f"CANCEL center ({CAN_CX},302): {can_px}  → "
