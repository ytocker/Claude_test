"""Concept D — bottom-overhang — rarity-gem shelf placement, round 2.

Fixes from art-director critique: gem centres raised to cy=326 so the ⅓-overhang
below the card bottom (y=328) reads cleanly and the aura (r=14) lands exactly at
y=340 without canvas-edge clip. Arc sag deepened — rect height m(14) instead of
m(8) — so the downward bow is visible (~7px below gem centres). Arc line weight
raised to max(1, m(1.4)) for legibility. Arc angles stay math.pi → 2*math.pi
(bottom half of ellipse = sags downward between the gem mounts).

Output -> docs/store_confirm_shelf_v3/rarity-gems/bottom-overhang/round_2.png
"""
import os, sys, math
...
def _draw_bottom_overhang_gems(big, affordable):
    # Two rarity gems straddle the card's bottom edge (y=328), mirroring the hero
    # disc's overhang at the top. At cy=326 the gem (r=10) spans y=316→336,
    # placing exactly ⅓ below the card bottom while the aura (r=14) lands at
    # y=340 — the canvas bottom — with no clip. Draw order: auras first, then the
    # gold sag-arc bows downward between the centres, then the facet gems cap ends.
    if affordable:
        base, deep = PAL["gem"], PAL["deep"]
        aura_col, peak = PAL["glow"], 28
        arc_col = sc.CARD_RING_BRIGHT
    else:
        base, deep = MUTED_BASE, MUTED_DEEP
        aura_col, peak = (90, 92, 110), 14
        arc_col = MUTED_ARC

    for cx in (60, 140):
        sc._alpha_aura(big, m(cx), m(326), m(14), aura_col, peak=peak, layers=13)

    # Bottom half of the ellipse (math.pi → 2*math.pi) sags downward like a slack
    # chain. rect height m(14) bows the midpoint ~7px below the gem centres so the
    # arc is legible without obscuring the gem faces. Thicker stroke aids contrast.
    arc_rect = pygame.Rect(m(60), m(322), m(80), m(14))
    pygame.draw.arc(big, arc_col, arc_rect, math.pi, 2 * math.pi,
                    max(1, m(1.4)))

    for cx in (60, 140):
        sc.facet_gem(big, m(cx), m(326), m(10), base, deep)
...
def _draw_shelf(big, affordable):
    ...
    # Bottom-overhang gems sit on top of the shelf furniture (chip + button row).
    _draw_bottom_overhang_gems(big, affordable)
...
def render_popup(name, affordable):
    ...
    # Hero disc drawn LAST so it overhangs the top card edge, mirroring the
    # bottom-overhang gems that overhang the opposite edge.
...
draw.text((CANVAS_W // 2, HDR_H // 2), "D — bottom-overhang  r2",
          fill=CREAM, font=fnt_hdr, anchor="mm")

# Two panels centred beneath the header band; the fixed 444x412 canvas leaves a
# 12px side/bottom margin around the 200x340 popups with a 20px column gap.
panel_span = 2 * POP_W + GAP
x0 = (CANVAS_W - panel_span) // 2
py = HDR_H + ((CANVAS_H - HDR_H) - POP_H) // 2
...
for ci, (state_lab, affordable) in enumerate(STATES):
    ...
    # state caption above each panel
    draw.text((px + POP_W // 2, py - 6), state_lab,
              fill=(160, 178, 220) if affordable else (150, 154, 168),
              font=fnt_lab, anchor="mb")

    # ID badge — dark pill top-left, "D" in cream with a lavender outline
    bx, by = px + 5, py + 5
    pad = 4
    tw = draw.textlength("D", font=fnt_bdg)
    draw.rounded_rectangle([bx, by, bx + tw + pad * 2, by + 16], radius=4,
                           fill=(24, 22, 38))
    draw.text((bx + pad, by + 8), "D", fill=CREAM, font=fnt_bdg, anchor="lm",
              stroke_width=1, stroke_fill=LAVEND)

OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "rarity-gems",
                   "bottom-overhang", "round_2.png")
...
print("\n=== gem placement check (affordable) — both must be non-bg ===")
im = panel_img(True)
for (x, y) in [(60, 326), (140, 326)]:
    p = im.getpixel((x, y))
    print(f"  ({x:>3},{y}): {p}  {'ok non-bg' if p != BG else 'WARN bg'}")

print("\n=== arc midpoint check (affordable) — must be warm gold (non-bg) ===")
# Arc midpoint at approximately (100, 333): bottom of the ellipse bowed ~7px below cy=326
for (x, y) in [(100, 333)]:
    p = im.getpixel((x, y))
    print(f"  ({x:>3},{y}): {p}  {'ok non-bg' if p != BG else 'WARN bg'}")
