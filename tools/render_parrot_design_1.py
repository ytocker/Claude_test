"""Review sheet for design_1 · STORM MACAW (PARROTS rarity exploration).
...
test that decides whether the lightning crest survives at store-thumbnail size.

Headless: SDL_VIDEODRIVER=dummy python tools/render_parrot_design_1.py
"""
import os
...
def gameplay_panel_phase(source, w, h, phase, *, frame_idx=2, tilt=10.0):
    # A clone of ninja_render.gameplay_panel parameterised on biome phase so we
    # can read the storm bird against both a day and a night scene.
...
# 40px truth-read row: day-bg, night-bg, and the navy store-card bg.
label(sheet, "40px truth read (NEAREST) — must read as a lightning bird:", 16, 372, FONT)
backgrounds = [
    ((150, 200, 235), "day sky"),
    ((20, 26, 52), "night sky"),
    ((28, 26, 46), "store card"),
...
for frame_idx in range(4):
    for bg, _name in backgrounds:
        ...
        big = pygame.transform.scale(tr, (40, 40))
        sheet.blit(big, (x, 396))
        x += 46
    x += 12
label(sheet, "frame 0 | 1 | 2 | 3   (×3 backgrounds each)", 16, 440)

# A second truth row at true 40px (no upscale) so the real thumbnail is visible.
label(sheet, "exact 40px (no zoom):", 16, 466, SMALL)
x = 150
for bg, name in backgrounds:
    tr = truth_read(build, bg, frame_idx=2, tilt=8.0)
    sheet.blit(tr, (x, 460))
    label(sheet, name, x, 502, SMALL, (170, 178, 200))
    x += 90
