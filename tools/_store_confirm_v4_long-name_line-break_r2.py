#!/usr/bin/env python3
"""
line-break  ·  store buy-confirmation popup v4  ·  long-name overflow  ·  round 2
...
R2 refinements (art-director notes):
  · Split stack now reads editorial, not word-wrap — line 1 at font 30, line 2
    at font 27, so the eye lands on the leading word first.
  · Single-line names bump to font 33 so a short name fills the title zone as
    deliberately as a split pair does.
  · Interline centres open to 22 px apart (y=157 / y=179), keeping ≥24 px from
    the lower line's baseline down to the shifted banner.
Names that already fit render as one boosted line; no-space names that can't
split (and lopsided splits) fall back to scale-to-fit so nothing ever clips.
...
def _build_draw_fn(force_tier=None):
    ...
    def _draw(self, surf):
        ...
        # ── Name: boosted single line, editorial 2-line split, or scale-to-fit ─
        safe_w = m(168)
        ...
        if _fits_at_30(name):
            # A short name should own the title zone, so bump it a notch. Font 33
            # is guarded against safe_w for the pathological case a "fits at 30"
            # name is wide enough to overrun at 33.
            single_font = sc.font(33)
            if sc._glyph_base(name, single_font, 0).get_width() > safe_w:
                single_font = name_font
            sc.plain_text(big, name, single_font, (m(CX), m(Y_NAME)), (250, 248, 240),
            ...
            if split_ok:
                ...
                # Editorial hierarchy: lead word larger, tail word a notch down,
                # centres 22 px apart to breathe above the shifted banner.
                LINE1_Y = Y_NAME + 2   # 157
                LINE2_Y = Y_NAME + 24  # 179
                sc.plain_text(big, line1, sc.font(30), (m(CX), m(LINE1_Y)),
                              (250, 248, 240), shadow_a=160, weight=m(0.9),
                              keyline=(6, 6, 16), kw=m(1.0))
                sc.plain_text(big, line2, sc.font(27), (m(CX), m(LINE2_Y)),
...
PANELS = [
    ("0", "MUMMY", None, "(1-line sz33)"),
    ("1", "SUGAR GLIDER", None, "(2-line 30/27)"),
    ("2", "TEMPEST CONDOR", "legendary", "(2-line 30/27)"),
]
...
# Header
h = fhdr.render("LINE-BREAK · Long-name overflow R2", True, GOLD)
...
zl = flbl.render("2× ZOOM · Panel 2 name zone (line1 sz30 / line2 sz27 + shifted banner + chip clearance)",
                 True, GOLD)
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_popup_v4", "long-name", "line-break", "round_2.png")
