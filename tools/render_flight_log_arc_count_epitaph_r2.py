#!/usr/bin/env python3
"""
arc_count / EPITAPH — round 2

Polish pass implementing all 6 critique notes:
  1. Hero "18%" enlarged to 52pt with real 3-layer incised shadow
  2. Rules tapered as horizontal gold gradient (130/90 peak alpha)
  3. Title tracking reduced to 3 to clear veil seam at x≈92
  4. Verse block lifted (TITLE_Y 98→88) and line spacing tightened (LINE_H 16→14)
  5. "FELL AT PILLAR" removed; replaced with bare "FELL HERE" at 40% alpha
  6. DAY COMPLETE: color+tracking fixed; BACK: 11pt, 75% alpha, track=6
"""
import os
import math
import random
...
def _draw_tapered_rule(surf, center_x, y, half_w, peak_alpha):
    """Draw a 1px-tall horizontal gold rule that tapers to 0 alpha at both ends.

    Each column gets alpha = peak_alpha × (1 − |x − center| / half_w),
    producing a soft linear fade so the rule reads as polished gold in the
    centre and dissolves cleanly into the sky at the edges.
    """
    lay = pygame.Surface((surf.get_width(), 1), pygame.SRCALPHA)
    for rx in range(center_x - half_w, center_x + half_w + 1):
        dist = abs(rx - center_x)
        a = int(peak_alpha * max(0.0, 1.0 - dist / half_w))
        if a > 0:
            lay.set_at((rx, 0), (*GOLD, a))
    surf.blit(lay, (0, y))
...
def render_screen():
    ...
    # ── Critique 6: DAY COMPLETE label — muted stone color, track=2, centered beneath terminus ──
    xdc, ydc = pos_u(1.0)
    text(surf, "DAY COMPLETE", 8, center=(int(xdc), int(ydc) + 18),
         color=(150, 146, 138), track=2, shadow=None)

    bx, by, bw = draw_macaw(surf)

    # ── EPITAPH banner: minimal — scrim + FLIGHT LOG header only ──
    ...
    text(surf, "FLIGHT LOG", 12, center=(W // 2, 24), color=GOLD, track=4, shadow=None)
    text(surf, f"DAY {DAY_N}", 9, center=(W // 2, 50), color=(150, 146, 138), track=3, shadow=None)

    # ── VERSE BLOCK — centered in the sky ──
    CX_TEXT = W // 2

    # ── Critique 4: TITLE_Y lifted from 98 to 88 ──
    TITLE_Y = 88

    # ── Critique 3: track=3 pulls left edge of title to ~x=105, clear of veil seam at x≈92 ──
    text(surf, "THE PARROT FLEW", 11, center=(CX_TEXT, TITLE_Y), color=GOLD, track=3,
         shadow=(0, 0, 0, 120))

    # ── Critique 2: Rule 1 — tapered horizontal gradient, peak alpha 130 ──
    RULE1_Y = TITLE_Y + 18
    RULE_W = 120
    RULE_HALF = RULE_W // 2
    _draw_tapered_rule(surf, CX_TEXT, RULE1_Y, RULE_HALF, 130)

    # ── Critique 1: Hero "18%" at 52pt with 3-layer incised shadow ──
    f52 = pygame.font.Font(FONT_PATH, 52)
    hero_str = f"{int(DEATH_PHASE * 100)}%"
    hero_main = f52.render(hero_str, True, CREAM)
    hero_x = CX_TEXT - hero_main.get_width() // 2
    hero_y = RULE1_Y + 10

    # Layer a: near-black (0,0,0) at alpha=200, +1px Y — the cut-stone shadow
    shadow_a = f52.render(hero_str, True, (0, 0, 0))
    shadow_a.set_alpha(200)
    surf.blit(shadow_a, (hero_x, hero_y + 1))

    # Layer b: warm highlight (255,235,200) at -1px Y — rim light from above
    shadow_b = f52.render(hero_str, True, (255, 235, 200))
    shadow_b.set_alpha(180)
    surf.blit(shadow_b, (hero_x, hero_y - 1))

    # Layer c: main CREAM at 0 offset
    surf.blit(hero_main, (hero_x, hero_y))

    # "OF THE DAY FLOWN" below hero number
    OF_Y = hero_y + hero_main.get_height() + 4
    text(surf, "OF THE DAY FLOWN", 8, center=(CX_TEXT, OF_Y), color=(160, 155, 145), track=3,
         shadow=None)

    # ── Critique 4: LINE_H tightened from 16 to 14 ──
    LINE_H = 14
    text(surf, f"PILLAR {DEATH_PILLAR}", 9, center=(CX_TEXT, OF_Y + LINE_H + 4),
         color=(190, 185, 175), shadow=None)
    text(surf, f"0:{TIME_ALIVE:02d}", 9, center=(CX_TEXT, OF_Y + LINE_H * 2 + 4),
         color=(190, 185, 175), shadow=None)

    # ── Critique 2: Rule 2 — tapered horizontal gradient, peak alpha 90 ──
    RULE2_Y = OF_Y + LINE_H * 2 + 20
    _draw_tapered_rule(surf, CX_TEXT, RULE2_Y, RULE_HALF, 90)

    # Coda
    text(surf, "4 ENCOUNTERS AWAIT", 8, center=(CX_TEXT, RULE2_Y + 12),
         color=(110, 108, 122), track=2, shadow=None)

    # ── Critique 5: Remove "FELL AT PILLAR 25"; replace with bare "FELL HERE" ──
    # Down-left of the bird, in the clear sky, at 40% alpha (102).
    fell_s = font(8).render("FELL HERE", True, CREAM)
    fell_s.set_alpha(102)
    surf.blit(fell_s, (60, 300))

    # ── Critique 6: BACK — 11pt, 75% alpha (191), track=6, em-dashes ≥100px ──
    back_text = "— BACK —"
    f_back = font(11)
    track_back = 6
    glyphs_b = [f_back.render(ch, True, CREAM) for ch in back_text]
    tw_b = sum(g.get_width() for g in glyphs_b) + track_back * (len(back_text) - 1)
    th_b = f_back.get_height()
    back_img = pygame.Surface((max(1, tw_b), max(1, th_b)), pygame.SRCALPHA)
    bxi = 0
    for ch, g in zip(back_text, glyphs_b):
        back_img.blit(g, (bxi, 0))
        bxi += g.get_width() + track_back
    back_img.set_alpha(191)
    surf.blit(back_img, ((W - back_img.get_width()) // 2, 592))
