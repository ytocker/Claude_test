#!/usr/bin/env python3
"""
arc_count round 4 — enriched information: encountered strip, best notch, sealed teaser, stats grid.
...
def draw_overlay(ss):
    ...
    for p, rad, fn, col in ring_items:
        x, y = arc_pos(p, rad)
        if p > DEATH_PHASE:
            # Target ring — position known, identity sealed
            rx, ry = int(x * k), int(y * k)
            # Connector from rail
            if rad != R_INNER:
                ax, ay = arc_pos(p, R_INNER)
                pygame.draw.line(ss, (*GOLD, 60), (int(ax * k), int(ay * k)), (rx, ry),
                                 int(1.0 * k))
            # Outer ring
            pygame.draw.circle(ss, (*GOLD, 160), (rx, ry), int(8.0 * k), int(2.0 * k))
            # Inner ring
            pygame.draw.circle(ss, (*GOLD, 110), (rx, ry), int(5.0 * k), int(1.4 * k))
            # Center ink dot
            pygame.draw.circle(ss, (*INK, 220), (rx, ry), int(2.4 * k))
        else:
            if rad != R_INNER:
                ax, ay = arc_pos(p, R_INNER)
...
def render_screen():
    ...
    # Stats grid — 3 chips in the ground band: TIME · PILLAR · BEST
    _stats = [
        ("TIME",   f"0:{TIME_ALIVE:02d}"),
        ("PILLAR", str(DEATH_PILLAR)),
        ("BEST",   f"{BEST_PILLAR} PL"),
    ]
    _stat_centers = [62, 180, 298]
    for (_st_lbl, _st_val), _st_cx in zip(_stats, _stat_centers):
        _cw_s, _ch_s = 88, 30
        _cr_s = pygame.Rect(_st_cx - _cw_s // 2, 456, _cw_s, _ch_s)
        # drop shadow
        _sh_s = pygame.Surface((_cw_s + 2, _ch_s + 2), pygame.SRCALPHA)
        pygame.draw.rect(_sh_s, (*INK, 70), _sh_s.get_rect(), border_radius=6)
        surf.blit(_sh_s, (_cr_s.x - 1, _cr_s.y + 1))
        # background
        _bg_s = pygame.Surface((_cw_s, _ch_s), pygame.SRCALPHA)
        pygame.draw.rect(_bg_s, (8, 8, 20, 200), _bg_s.get_rect(), border_radius=6)
        surf.blit(_bg_s, _cr_s.topleft)
        # border — dimmed gold, full opacity
        pygame.draw.rect(surf, (130, 105, 45), _cr_s, width=1, border_radius=6)
        # label
        text(surf, _st_lbl, 7, center=(_st_cx, _cr_s.y + 9), color=(140, 135, 130), shadow=None)
        # value — slightly dimmer gold for BEST (past achievement)
        _val_col = (255, 206, 92) if _st_lbl != "BEST" else (210, 190, 80)
        text(surf, _st_val, 12, center=(_st_cx, _cr_s.y + 22), color=_val_col, shadow=None)
