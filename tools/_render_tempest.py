"""Scratch round-1 sheet for TEMPEST CONDOR MACAW (design_4).

Hero product-shot + gameplay panel + 4-frame flap filmstrip + a 40px NEAREST
truth-read, on BOTH day and night backgrounds. Exploration only.
"""
import os
...
def _gameplay_panel_phase(source, w, h, phase, frame_idx=2, tilt=10.0):
    """gameplay_panel clone with a selectable biome phase so we can show the
    bird over a navy night scene as well as the bright day one."""
...
def _truth_read(bg, frame_idx=2, tilt=8.0, size=40):
    """A 40px NEAREST downscale of the bird on a flat sky tile — the make-or-
    break in-motion read. NEAREST (not smooth) so we judge the worst case."""
...
# Neutral mid-slate sheet bg so both day + night tiles judge fairly.
sheet.fill((34, 38, 48))

_label(sheet, "TEMPEST CONDOR MACAW — LEGENDARY  ·  round 1", 24, 16, (140, 230, 244))
...
fy = 388
_label(sheet, "FLAP CYCLE — DAY (halo + streamer alive across frames)", 24, fy)
...
# Row 3: 40px NEAREST truth-read, day + night, a couple of frames each.
ty = 580
_label(sheet, "40px TRUTH-READS (NEAREST · the in-motion make-or-break)", 24, ty)
truths = [
    (DAY_SKY, 2, 8.0, "day f2"),
    (DAY_SKY, 0, -6.0, "day f0"),
    (NIGHT_SKY, 2, 8.0, "night f2"),
    (NIGHT_SKY, 3, -6.0, "night f3"),
]
tx = 24
for bg, fi, tl, name in truths:
    t = _truth_read(bg, frame_idx=fi, tilt=tl)
    big = pygame.transform.scale(t, (t.get_width() * 2, t.get_height() * 2))
    sheet.blit(big, (tx, ty + 26))
    _label(sheet, name, tx, ty + 26 + big.get_height(), (210, 220, 228))
    tx += big.get_width() + 28

out = os.path.join(os.path.dirname(__file__), "..", "docs", "store_redesign",
                   "parrot", "wave2", "design_4", "round_1.png")
