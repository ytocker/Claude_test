"""Concept B — lip-bookends (round 2) for the v3 confirm-popup rarity gems.

R2 revisions vs R1:
  cy raised to 237 (was 241) so ~40-50 % of each gem straddles above SHELF_Y=235,
  making the "perched finial" concept legible.
  Shadow-well ring suppression: facet_gem is drawn in two passes with a clip rect so
  the gold shadow-ring cannot reach above the shelf into the clear card sky as a
  floating artifact. Pass A (y≥SHELF_Y) draws the full well + facets below the lip.
  Pass B (gem-body bbox only) restores the above-shelf facets without letting the
  annular shadow ring (y=225-229, outside the bbox) leak through.
  Unaffordable gem dimmed: base (62,64,78) / deep (38,40,52) so the disabled state
  clearly loses contrast against the buyable epic hue.

Output -> docs/store_confirm_shelf_v3/rarity-gems/lip-bookends/round_2.png
"""
import os, sys, math
...
# Lip-bookend gem geometry in virtual (un-scaled) coords.
# cy=237 puts the centre 2 px below SHELF_Y=235, straddle target ~40-50 %
# above the lip.
BOOKEND_CY = 237
BOOKEND_XS = (27, 173)
BOOKEND_R  = 8
...
def _draw_gems(big, affordable):
    # Concept B — lip-bookends. Two rarity gems perch on the shelf's top lip
    # (SHELF_Y=235). cy=237 puts each centre 2 px below the lip so ~half the
    # gem straddles above it, reading as a finial seated on the edge.
    #
    # Aura is drawn first so the halo wells up behind the facet body.
    # facet_gem is then drawn in two passes to keep the above-shelf facets
    # visible while preventing the shadow-well's outer ring from floating as
    # a warm artifact against the clear card sky above the lip:
    #   Pass A  (clip y>=SHELF_Y)  draws shadow+facets in the below-shelf half.
    #   Pass B  (clip to gem-body bbox)  draws shadow+facets for the above-lip
    #           portion, restricting the ring annulus to within the body bbox so
    #           it cannot reach the bare card background above the gem top.
    if affordable:
        base, deep = PAL["gem"], PAL["deep"]
        glow, peak = PAL["glow"], 22
    else:
        # Dimmer slate so the disabled state clearly loses against the epic hue.
        base, deep = (62, 64, 78), (38, 40, 52)
        glow, peak = (90, 92, 110), 11

    shelf_big = m(SHELF_Y)
    w_big     = big.get_width()
    h_big     = big.get_height()
    gem_by    = m(BOOKEND_CY)
    gem_br    = m(BOOKEND_R)

    for gx in BOOKEND_XS:
        gem_bx = m(gx)
        sc._alpha_aura(big, gem_bx, gem_by, m(11), glow, peak=peak, layers=10)

        # Pass A — below-shelf half
        big.set_clip(pygame.Rect(0, shelf_big, w_big, h_big - shelf_big))
        sc.facet_gem(big, gem_bx, gem_by, gem_br, base, deep)
        # Pass B — gem-body bounding box only; above-shelf facets restored
        # without the shadow ring leaking into the card sky above the lip
        big.set_clip(pygame.Rect(gem_bx - gem_br - 1, gem_by - gem_br - 1,
                                  (gem_br + 1) * 2, (gem_br + 1) * 2))
        sc.facet_gem(big, gem_bx, gem_by, gem_br, base, deep)
        big.set_clip(None)
...
def _draw_shelf(big, affordable):
    ...
    # Gem decoration sits on top of the shelf furniture (chip + button row) but
    # before the hero disc/aura/thumb, so the disc never occludes a bookend.
...
# ── 2-panel sheet layout ──────────────────────────────────────────────────────
CANVAS_W, CANVAS_H = 444, 412
MARGIN, GAP, HDR_H = 20, 20, 40
BG = (8, 8, 20)
CREAM = (250, 248, 240)
LAV   = (200, 190, 240)
...
draw.text((CANVAS_W // 2, HDR_H // 2), "B — lip-bookends  (r2)",
          fill=CREAM, font=fnt_hdr, anchor="mm")

STATES = [("AFFORDABLE", True), ("UNAFFORDABLE", False)]
panel_y = HDR_H + MARGIN
# Two 200-wide panels + one 20 gap centred inside the 444 canvas.
content_w = 2 * POP_W + GAP
x0 = (CANVAS_W - content_w) // 2
...
for ci, (state_lab, affordable) in enumerate(STATES):
    ...
    # state caption under the panel
    draw.text((px + POP_W // 2, panel_y + POP_H + 6), state_lab,
              fill=(150, 170, 210) if affordable else (140, 146, 160),
              font=fnt_lab, anchor="ma")

    # ID badge — dark pill top-left, "B" in cream with a lavender outline
    bx, by = px + 4, panel_y + 4
    bw = fnt_bdg.getlength("B") + 8
    draw.rounded_rectangle([bx, by, bx + bw, by + 16], radius=4,
                           fill=(24, 22, 38), outline=LAV)
    draw.text((bx + 4, by + 8), "B", fill=CREAM, font=fnt_bdg, anchor="lm")

OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "rarity-gems",
                   "lip-bookends", "round_2.png")
OUT = os.path.abspath(OUT)
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

# ── PIL verification (affordable panel — both bookend gem centres at cy=237) ──
_aff = render_popup("TEMPEST", True)
_im  = Image.frombytes("RGB", (POP_W, POP_H), pygame.image.tostring(_aff, "RGB"))
print("\n=== lip-bookend gem-centre checks (affordable, cy=237) ===")
for (x, y) in [(27, 237), (173, 237)]:
    p = _im.getpixel((x, y))
    non_bg = p != (0, 0, 0) and p != BG
    print(f"  ({x:>3},{y}): {p}  {'ok non-bg' if non_bg else 'WARN bg'}")

# Confirm no warm speck in the annular shadow-ring zone above the shelf (y=225-229)
print("\n=== stray-pixel check: annular zone above shelf (y=225-229) ===")
BG_DARK_THRESH = 40  # any channel above this in the zone may be a warm artifact
for y in range(225, 230):
    for x in (27, 173):
        p = _im.getpixel((x, y))
        warm = any(c > BG_DARK_THRESH for c in p)
        tag  = "WARM" if warm else "ok"
        print(f"  ({x:>3},{y}): {p}  {tag}")
