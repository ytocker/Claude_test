"""Compose the THE REFEREE round-2 review sheet.

Round 2 judges the enlarged whistle + fatter yellow card, so the hero is a
NEAREST magnify (crisp pixels, no smoothing blur) and the truth read shows a
40px bird on day + night swatches in motion.
...
def _truth_read(box):
    ...
    whistle + cards are judged against a dark night swatch beside the day one."""
