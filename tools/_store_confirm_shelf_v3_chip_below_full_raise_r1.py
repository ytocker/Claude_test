"""Chip-below buttons — option A: full-raise (R1).

Same full-raise shelf as bottom-gems A, but the price chip is repositioned
BELOW the Buy/Cancel buttons: buttons sit near the shelf TOP, chip near the
shelf BOTTOM. Draw order inside the shelf becomes buttons → chip.
  SHELF_Y=210, SHELF_H=79 (shelf bottom 289) — UNCHANGED.
  BTN_CY=231 (buttons near top), CHIP_CY=271 (chip near bottom).
Bottom gems at cy=309 — UNCHANGED.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/full-raise/round_1.png
"""
import os, sys, math
...
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 210, 174, 79
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 231, 8            # buttons near shelf TOP
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD          = 112, 30, 8
CHIP_CY                           = 271               # chip near shelf BOTTOM
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip-below A: full-raise (R1)  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "full-raise", "round_1.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

# PIL verification: chip below buttons, bottom gems intact
img  = Image.open(OUT)
w, h = img.size
...
print(f"size: {w}x{h}")
print(f"chip centre: {chip_px}")
print(f"BUY centre: {btn_px}")
print(f"gem left: {gem_l}")
