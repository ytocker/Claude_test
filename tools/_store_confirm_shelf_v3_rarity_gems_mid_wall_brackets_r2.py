"""Round 2 — concept C "mid-wall-brackets" for the store confirm-popup rarity gems.

Revisions from art-director critique:
- Affordable aura: radius m(19), peak=42 so the halo genuinely washes the
  indigo wall (x≈13) with visible intensity.
- Gem radius: m(9) (was m(8)) to read firmly against the r=15 coin.
- Unaffordable aura: same radius m(19), peak=20 (was 15) — proportionally
  muted despite the larger radius so it stays clearly dimmer than the
  affordable halo, not amplified.

Output -> docs/store_confirm_shelf_v3/rarity-gems/mid-wall-brackets/round_2.png
"""
import os, sys, math
...
GEM_C_R      = 9    # r=9 reads firmly next to the r=15 coin
GEM_C_CY     = 258
GEM_C_L_X    = 28
GEM_C_R_X    = 172
...
def _draw_gems(big, affordable):
    ...
    chip against the indigo shelf walls. Larger radius (m(19)) lets the halo
    wash the wall surface at x≈13 with real intensity; muted peak stays
    proportionally lower so the affordability read is unambiguous."""
    if affordable:
        base, deep, glow, peak = PAL["gem"], PAL["deep"], PAL["glow"], 42
    else:
        base, deep, glow, peak = MUTED_BASE, MUTED_DEEP, MUTED_AURA, 20

    for cx in (GEM_C_L_X, GEM_C_R_X):
        sc._alpha_aura(big, m(cx), m(GEM_C_CY), m(19), glow, peak=peak, layers=14)
...
draw.text((CANVAS_W // 2, HEADER // 2), "C — mid-wall-brackets  r2",
...
print("\n=== x=18 aura edge check (affordable) — should be near-bg, not blasted ===")
p18 = im.getpixel((18, GEM_C_CY))
p_card_edge = im.getpixel((8, GEM_C_CY))
print(f"  (18,{GEM_C_CY}): {p18}  card_edge(8,{GEM_C_CY}): {p_card_edge}")
diff = max(abs(p18[i] - BG[i]) for i in range(3))
print(f"  max channel delta from bg at x=18: {diff}  (low = aura contained; high = wall-wash present)")
