"""Bottom-frame gems — option B: buttons-lift, R2.

Raise buttons only; shelf top stays near original. Chip↔button gap tightens.

R2 changes vs R1:
- Slimmer chip: CHIP_H 34→30, CHIP_CY 248→240 (recover chip↔button gap).
  chip_bottom=240+15=255, btn_top=275-15=260 → 5px clean gap.
- Button lift: BTN_CY 278→275. btn_bottom=275+15=290, gem_top=298 → 8px clearance.
- Shelf top: SHELF_Y 230→226 gives chip more internal room.
- Tamer bottom-gem aura: affordable peak 70→55 (unaffordable stays 35) so gems
  don't compete with button edges.
- NOT ENOUGH text drops to m(292), between btn_bottom (290) and gem_top (298).

Layout: SHELF_Y=226, CHIP_CY=240, CHIP_H=30, BTN_CY=275, BOT_GEM_CY=309.
Gem centers at (33,309)/(167,309); BUY center at (58,275).

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/bottom-gems/buttons-lift/round_2.png
"""
import os, sys, math
...
# Option B: raise buttons only — minimal shelf disruption
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 226, 174, 87
CHIP_CY                            = 240
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 275, 8
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Bottom gems B: buttons-lift R2  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "bottom-gems", "buttons-lift", "round_2.png"))
canvas.save(OUT)
