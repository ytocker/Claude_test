...
Output → docs/store_confirm_shelf_v3/buy-fancy/gold-standard-swap/round_1.png
"""
import os, sys, math
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    ...
    # BUY: struck-gold body matching the bevel rim.
    stops   = [(0.0, (242, 192, 80)), (1.0, (196, 146, 46))]
    lab_col = (52, 36, 12)
    ...
    lab_font = sc.font(font_px)
    # Light emboss keyline, no dark shadow — the amber fill reads bright.
    sc.plain_text(big, label, lab_font, btn_rect.center, lab_col,
                  shadow_a=0, weight=m(0.8), keyline=(220, 180, 60), kw=m(0.9))
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gold-standard-swap (B) r1  |  AFFORDABLE / UNAFFORDABLE",
