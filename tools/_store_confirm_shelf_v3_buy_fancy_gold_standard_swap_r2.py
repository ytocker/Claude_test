"""buy-fancy exploration — concept B: gold-standard-swap.

BUY is cast in the card's own gold accent (warm amber body that echoes the
bevel rim), so the primary action reads as a struck-gold coin. CANCEL keeps
the cool indigo body, making the choice a stark gold-vs-slate contrast. The
dark-espresso BUY label survives on the light amber fill where a cream label
would wash out.

Shelf / chip / locked state: unchanged from the v3 c-orig-bg base.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/buy-fancy/gold-standard-swap/round_2.png
"""
import os, sys, math
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    if locked:
        # Unaffordable BUY keeps the base slate + padlock affordance untouched.
    ...
    if is_cancel:
        # Cool indigo body — deliberately slate so the gold BUY dominates.
    ...
    # BUY: struck-gold body pulled toward the champagne rim family so the two
    # golds read as one accent, not a warm/cool clash.
    stops   = [(0.0, (240, 200, 110)), (1.0, (198, 150, 58))]
    lab_col = (52, 36, 12)
    sc.drop_shadow(big, btn_rect, rad, blur=m(3), alpha=150, dy=m(2))
    big.blit(sc.vgrad_stops(btn_rect.w, btn_rect.h, rad, stops, 255),
             btn_rect.topleft)
    sc.top_sheen(big, btn_rect, rad, m(12), peak=40)
    sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                 (*sc.CARD_RING_BRIGHT, 230), w=max(1, m(2.0)))
    lab_font = sc.font(font_px)
    # Dark amber keyline only — crisp edges without a golden halo washing out.
    sc.plain_text(big, label, lab_font, btn_rect.center, lab_col,
                  shadow_a=0, weight=m(0.8), keyline=(32, 22, 6), kw=m(0.5))
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gold-standard-swap (B) r2  |  AFFORDABLE / UNAFFORDABLE",
...
print(f"BUY center pixel {buy_px}  → {'OK amber' if buy_px[0] >= 200 and buy_px[1] >= 150 and buy_px[2] < 120 else 'WARN'}")
print(f"CANCEL center pixel {can_px}  → {'OK indigo' if can_px[2] > can_px[0] else 'WARN'}")
