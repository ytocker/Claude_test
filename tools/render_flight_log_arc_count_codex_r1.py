#!/usr/bin/env python3
"""
arc_count/codex round 1 — navigator's logbook layout.

CODEX concept: hero "18%" at 64pt CREAM in dedicated banner zone, separated
by a 1px GOLD rule. Below that, a clean 2-column ruled log table (KEY | VALUE).
Death/danger row distinguished by a 2px red left accent rule. Slim BACK button.
"""
import os
import math
import random
...
def render_screen():
    ...
    text(surf, "FLIGHT LOG", 15, midleft=(18, 24), color=GOLD, track=3, shadow=None)
    text(surf, f"DAY {DAY_N}", 15, midright=(W-18, 24), color=CREAM, shadow=None)
    text(surf, f"0:{TIME_ALIVE:02d}", 10, midright=(W-18, 48), color=(160,155,145), shadow=None)

    # ── Hero banner zone (y=78–136) — 18% as large display numeral ──────────
    f64 = pygame.font.Font(FONT_PATH, 64)
    hero_surf = f64.render(f"{int(DEATH_PHASE*100)}%", True, CREAM)
    HERO_X = 20
    surf.blit(hero_surf, (HERO_X, 80))
    # Label to its right
    text(surf, "OF THE DAY", 9, midleft=(HERO_X + hero_surf.get_width() + 10, 90),
         color=(150,145,135), track=2, shadow=None)
    text(surf, "FLOWN", 9, midleft=(HERO_X + hero_surf.get_width() + 10, 104),
         color=(150,145,135), track=2, shadow=None)
    # Gold separator rule
    ...
    for i, (k, v) in enumerate(rows):
        ...
        # Death accent: red left-side rule on PILLAR row
        if k == "PILLAR":
            pygame.draw.rect(surf, (220, 80, 60), (18, ry - 3, 2, ROW_H))
        text(surf, k, 8, midleft=(TABLE_X_KEY, ry + ROW_H//2 - 2), color=(130,126,118), track=1, shadow=None)
        text(surf, v, 9, midleft=(TABLE_X_VAL, ry + ROW_H//2 - 2), color=CREAM, shadow=None)
    # Closing separator
