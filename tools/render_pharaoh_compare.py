"""Final comparison figure for the PHARAOH → full Egyptian-outfit redesign.

Six columns side by side — the ORIGINAL live pharaoh skin plus the five explored
Egyptian archetypes — each Pip mid-flight over the same real gameplay biome
scene, so the user can judge the redesign in context. Pure capture; touches no
production art (the five candidates are scratch builders under
tools/pharaoh_candidates/, the original is the live registered ``skin_pharaoh``).

Run headless from repo root:
``SDL_VIDEODRIVER=dummy python tools/render_pharaoh_compare.py``.
"""
...
# Column order: original first, then the five archetypes in concept-rank order.
COLUMNS = [
    ("ORIGINAL", "CURRENT PHARAOH", "skin_pharaoh"),
    ("DESIGN 1", "GOLDEN PHARAOH", "design_1"),
    ("DESIGN 2", "ANUBIS", "design_2"),
    ("DESIGN 3", "RA", "design_3"),
    ("DESIGN 4", "THE MUMMY", "design_4"),
    ("DESIGN 5", "CLEOPATRA", "design_5"),
]
...
title = _font(32, True).render(
    "PHARAOH REDESIGN — ORIGINAL vs. 5 EGYPTIAN DESIGNS (in gameplay)",
    True, _GOLD_PALE)
...
out = os.path.join("docs", "store_redesign", "costume", "pharaoh", "final_comparison.png")
