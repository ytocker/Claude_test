"""Render design_1 AZUREWING into one labeled review sheet:
hero product-shot + in-gameplay (day + night) + 40px NEAREST truth reads +
the 4-frame flap strip (the wing-beat that carries the iridescent shimmer).
"""
import os, sys
...
from tools.bee_candidates.design_1 import build
...
def _frame_strip(box):
    """The 4 flap poses on a flat panel — read the wing-spread + shimmer beat."""
    ...
    for i in range(4):
        ...
        pygame.draw.rect(cell, (22, 20, 32), cell.get_rect(), border_radius=10)
        fr = build(i, 0.0)
        sw, sh = fr.get_size()
