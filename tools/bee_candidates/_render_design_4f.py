"""Compose the PURPLE EMPEROR (design_4f) review sheet: a hero product shot,
in-gameplay day AND night biome panels, a 40px NEAREST truth read (day | night,
3 poses), and a 4-frame flap strip that shows the violet/blue shimmer wheel.

Scratch exploration; nothing here touches production art.

Headless: SDL_VIDEODRIVER=dummy python tools/bee_candidates/_render_design_4f.py
"""
import os
...
def main():
    ...
    sy = head + box + 28
    _label(sheet, "FLAP STRIP  (frames 0-3 — violet/blue shimmer wheel sweeps the wing each beat)",
