"""Compose the design_4d PAINTED LADY review sheet: a hero product shot, an
in-gameplay day panel, an in-gameplay night panel, a 40px NEAREST truth read
(day | night, 3 poses), and a 4-frame flap strip.
...
TITLE = ("DESIGN 4d — PAINTED LADY  R1  (Vanessa cardui: salmon-tawny field, "
         "cream hindwing zone, black forewing apex patch with white spots)")

NIGHT_PHASE = 0.64375       # NIGHT keyframe — moonlit cool biome for the split
...
def _gameplay_panel_phase(source, w, h, phase):
    ...
    bird can be judged over a night sky as well as day."""
    ...
    frame = ninja_render._frame(source, ninja_render.FRAME_IDX, ninja_render.TILT)
...
def main():
    ...
    widths = [p.get_width() for p in panels]
