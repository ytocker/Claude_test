"""Shelf-redesign: gem-cradle-tab opt-C (most compressed / lowest).

Variation of gem-cradle-chip R2 with design-4's "hanging-price-tab" chip style,
and the buttons + chip + shelf border progressively lowered.

opt-C — most compressed / lowest:
  SHELF_Y=248, SHELF_H=80   # shelf_bottom=328
  BTN_CY=270, BTN_H=28
  CHIP_CY=294, CHIP_H=26    # chip_bottom=307; gems drawn on top
  BOT_GEM_CY=309            # fixed

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-tab/opt-c/round_1.png
"""
import os, sys, math
...
# opt-C — most compressed / lowest.
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 248, 174, 80   # shelf_bottom=328
...
CHIP_CY                            = 294   # chip_bottom=307; gems drawn on top
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-tab opt-C  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-tab", "opt-c", "round_1.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

img   = Image.open(OUT)
btn   = img.getpixel((18 + 58,  54 + 270))
chip  = img.getpixel((18 + 100, 54 + 294))
gem_l = img.getpixel((18 + 33,  54 + 309))
print(f"size:{img.size}  btn:{btn}  chip:{chip}  gem_l:{gem_l}")
