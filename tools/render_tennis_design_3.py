"""Round-1 review sheet for DESIGN 3 — NEON BASELINER (tennis).
...
judged. Touches no production art.
...
def truth_read(source, sky, box=160):
    ...
    honest "what survives the shrink" test on day vs. night backdrops."""
...
cols = 4
sheet_w = PAD * 2 + PANEL_W + GUTTER + HERO + GUTTER + TRUTH + GUTTER + TRUTH
sheet_h = TITLE_H + PANEL_H + 40 + PANEL_H // 1 + PAD
# Two rows: row1 DAY, row2 NIGHT. Each row: gameplay | hero | truth-day | truth-night.
row_h = PANEL_H
...
title = _font(26, True).render(
    "TENNIS — DESIGN 3: NEON BASELINER (round 1, in gameplay)", True, _GOLD_PALE)
...
def place_row(y, phase, sky, row_tag):
    ...
    x += PANEL_W + GUTTER
    hero = nr.hero_panel(SOURCE, HERO, bg=(sky[0] // 3, sky[1] // 3, sky[2] // 2))
    sheet.blit(hero, (x, y))
    sheet.blit(small.render(row_tag + " HERO SHOT", True, (180, 174, 200)), (x + 2, y + HERO + 4))

    x += HERO + GUTTER
    tr = truth_read(SOURCE, sky, TRUTH)
    pygame.draw.rect(sheet, _GOLD_DEEP, pygame.Rect(x - 2, y - 2, TRUTH + 4, TRUTH + 4), 2)
    sheet.blit(tr, (x, y))
    sheet.blit(small.render("40px TRUTH READ", True, (180, 174, 200)), (x + 2, y + TRUTH + 4))


place_row(TITLE_H, 0.0, DAY_SKY, "DAY")
place_row(TITLE_H + row_h + 28, 0.6, NIGHT_SKY, "NIGHT")

out = os.path.join("docs", "store_redesign", "costume", "tennis", "design_3", "round_1.png")
