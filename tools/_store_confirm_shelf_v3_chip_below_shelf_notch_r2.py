"""Chip-below layout — option D: shelf-notch (R2).

Same shelf-notch construction as bottom-gems D (circular bezel holes punched
into the shelf so gems read as stones "set into" the frame), price chip BELOW
the Buy/Cancel buttons.

R2 fixes off the art-director critique:
  - Buttons drawn strictly onto `big` AFTER the shelf blit, with lightened
    fills + stronger rim/shadow so they read as raised keys on the recessed
    shelf (R1 button band blended into the shelf hue and read as flat).
  - Price chip face is now LIGHTER than the shelf interior so the chip reads
    as a distinct panel even without its text.
  - Each punched notch gets a crisp bezel edge: a bright upper-left rim arc +
    a shadow lower-right arc, so the recesses read as actual drilled holes.
  - Both gems drawn/rimmed identically in one loop for guaranteed symmetry.

The BLEND_RGBA_MIN notch punch touches ONLY the local `shelf` surface; nothing
that erases alpha ever runs on `big`.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/shelf-notch/round_2.png
"""
import os, sys, math
...
# Option D: shelf-notch (circular bezel holes in shelf)
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 232, 174, 90  # shelf bottom=322
# Chip BELOW the buttons: buttons ride near the shelf top, the chip drops to
# the shelf bottom just above/over the seated notch gems.
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 255, 8
BUY_CX = CX - (BTN_W + BTN_GAP) // 2  # = 58
CAN_CX = CX + (BTN_W + BTN_GAP) // 2  # = 142
CHIP_W, CHIP_H, CHIP_RAD          = 112, 30, 8
CHIP_CY                           = 289
...
BOT_GEM_LX = 33
BOT_GEM_RX = 167
NOTCH_R    = 13  # bezel-hole radius, slightly larger than gem r=11

# Bezel-hole rim tones: a lit upper-left lip + a shadowed lower-right lip so
# the drilled recess reads as a hole with real depth.
NOTCH_RIM_HI = (180, 182, 220)
NOTCH_RIM_LO = (10, 12, 30)
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    # Fills sit clearly LIGHTER than the shelf interior (~28,30,62) so the keys
    # read as raised on the recessed shelf; CANCEL stays a muted secondary.
    if locked:
        stops   = [(0.0, (58, 60, 74)), (1.0, (40, 42, 54))]
        lab_col = (150, 152, 162)
        sheen   = 10
    elif is_cancel:
        stops   = [(0.0, (34, 36, 76)), (1.0, (20, 22, 52))]
        lab_col = (160, 165, 210)
        sheen   = 16
    else:
        stops   = [(0.0, (52, 56, 116)), (1.0, (30, 33, 78))]
        lab_col = (208, 214, 248)
        sheen   = 26

    # Stronger lift so the key separates from the shelf plane.
    sc.drop_shadow(big, btn_rect, rad, blur=m(4), alpha=125, dy=m(2))
    ...
    if locked:
        ...
        rim_w = m(2.2) if is_cancel else m(2.2)
        sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                     (*sc.CARD_RING_BRIGHT, 235), w=max(1, rim_w))
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    # Chip face is LIGHTER than the shelf interior so the price panel reads as a
    # distinct raised plate even with the text stripped away.
    face_stops = ([(0.0, (52, 54, 98)), (1.0, (42, 44, 86))] if affordable
                  else [(0.0, (48, 50, 74)), (1.0, (38, 40, 62))])
...
def _draw_shelf(big, affordable):
    ...
    # Bottom rounded-corner mask so the shelf tucks under the card frame.
    ...
    # Punch circular bezel holes so gems read as set into the frame. The MIN
    # blend erases the LOCAL shelf surface only — `big` is never touched here,
    # so the button/chip draws that follow are safe.
    notch_mask = pygame.Surface(shelf_rect.size, pygame.SRCALPHA)
    notch_mask.fill((255, 255, 255, 255))
    for gem_cx, gem_cy in [(BOT_GEM_LX, BOT_GEM_CY), (BOT_GEM_RX, BOT_GEM_CY)]:
        local_x = m(gem_cx) - m(SHELF_X)
        local_y = m(gem_cy) - m(SHELF_Y)
        pygame.draw.circle(notch_mask, (0, 0, 0, 0), (local_x, local_y), m(NOTCH_R))
    shelf.blit(notch_mask, (0, 0), special_flags=pygame.BLEND_RGBA_MIN)
    ...
    # --- Shelf plane lands on `big` HERE. Everything below is drawn ON TOP. ---
    ...
    # Buttons directly on `big`, AFTER the shelf blit — never on `shelf`, so the
    # notch punch can't zero them. Then the chip below, layering in front.
...
def _notch_bezel(big, gem_cx, gem_cy):
    """Crisp drilled-hole edge on `big`: a bright upper-left lip arc + a shadow
    lower-right lip arc at the notch radius, so the recess reads as a real hole.
    Drawn after the gem so it hugs the shelf lip just outside the stone."""
    nr = m(NOTCH_R)
    rect = pygame.Rect(gem_cx - nr, gem_cy - nr, nr * 2, nr * 2)
    w = max(1, m(1))
    # pygame arc angles: 0=E, pi/2=N; upper-left spans ~N→W, lower-right ~S→E.
    pygame.draw.arc(big, NOTCH_RIM_HI, rect, math.radians(95),  math.radians(205), w)
    pygame.draw.arc(big, NOTCH_RIM_LO, rect, math.radians(275), math.radians(385), w)


def _draw_bottom_gems(big, affordable):
    # One loop, identical calls per side → guaranteed left/right symmetry.
    for gem_cx in [m(BOT_GEM_LX), m(BOT_GEM_RX)]:
        ...
        _notch_bezel(big, gem_cx, m(BOT_GEM_CY))
...
def render_popup(name, affordable):
    ...
    # Gems after the shelf so they fill the punched bezel holes and read as
    # stones seated into the frame rather than floating above it.
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip below D: shelf-notch  |  AFFORDABLE / UNAFFORDABLE  (R2)",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "shelf-notch", "round_2.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

# Verification against the AFFORDABLE panel (canvas offset 18,54).
img     = canvas
chip_px = img.getpixel((18 + 100, 54 + 289))
btn_px  = img.getpixel((18 + 58,  54 + 255))
gem_l   = img.getpixel((18 + 33,  54 + 309))
gem_r   = img.getpixel((18 + 167, 54 + 309))
notch   = img.getpixel((18 + 19,  54 + 309))  # edge of notch — should be dark
print(f"chip:{chip_px} btn:{btn_px} gem_l:{gem_l} gem_r:{gem_r} notch:{notch}")
