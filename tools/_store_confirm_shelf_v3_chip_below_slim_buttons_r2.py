"""Chip-below — option C: slim-buttons (R2).

R1 pooled a hollow band of empty shelf between the top button row and the
bottom price chip. R2 collapses that void: the shelf is shortened so its
bottom lip lands exactly on the gem centre line, the buttons drop to sit
just under the shelf top, and the chip tucks tight beneath them so the
cascade disc→banner→buttons→chip→gems steps evenly with no dead air.

  - SHELF_H 97→87: shelf_bottom=309 == gem centre → the lip tucks at the gems.
  - BTN_CY 246→254 (btn_top=240, 18px below shelf_top; btn_bottom=268).
  - CHIP_CY=285, CHIP_H=30 (chip_top=270 → 2px tight coupling to buttons;
    chip_bottom=300 → 9px lip to shelf_bottom; gem_top≈298 so the gem aura
    renders just over the chip's lower edge, nestling the gems on the chip).
  - Chip gets a distinctly lighter fill than the shelf interior so it reads
    as a raised label plate even without its text.
  - BUY carries brighter stops than CANCEL to stay the dominant CTA.
  - Buttons drawn before the chip so the chip's shadow reads above the gems.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/slim-buttons/round_2.png
"""
import os, sys, math
...
# Option C R2: slim buttons up top, chip tucked below. SHELF_H shortened so
# the shelf bottom lip lands on the gem centre line (309), killing the R1 void.
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 222, 174, 87
BTN_W, BTN_H, BTN_RAD              = 76, 28, 9
BTN_CY, BTN_GAP                    = 254, 8
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    if locked:
        ...
        # BUY: brighter stops than CANCEL so it stays the dominant CTA.
        stops   = [(0.0, (48, 52, 108)), (1.0, (28, 30, 72))]
        lab_col = (216, 221, 250)
        sheen   = 26
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    # Chip plate reads distinctly lighter than the shelf interior so it looks
    # like a raised label element even before the price glyph lands.
    face_stops = ([(0.0, (40, 44, 86)), (1.0, (26, 30, 62))] if affordable
                  else [(0.0, (42, 44, 66)), (1.0, (28, 28, 48))])
...
def _draw_shelf(big, affordable):
    ...
    # Buttons first (near shelf top), then the chip below so its shadow reads
    # above the corner gems.
...
def _draw_bottom_gems(big, affordable):
    for gem_cx in [m(BOT_GEM_LX), m(BOT_GEM_RX)]:
        if affordable:
            sc._alpha_aura(big, gem_cx, m(BOT_GEM_CY), m(14), PAL["glow"], peak=45, layers=14)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip-below C: slim-buttons R2  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "slim-buttons", "round_2.png"))
...
# Verify chip / button / gem placement against the R2 sheet coordinates.
chip_px = canvas.getpixel((18 + 100, panels_y + 285))
btn_px  = canvas.getpixel((18 + 58,  panels_y + 254))
gem_l   = canvas.getpixel((18 + 33,  panels_y + 309))
