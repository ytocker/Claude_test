"""Shelf-redesign: gem-cradle-tab opt-A (moderate lower).
...
opt-A — moderate lower:
  SHELF_Y=228, SHELF_H=100  # shelf_bottom=328
  BTN_CY=252, BTN_H=30
  CHIP_CY=284, CHIP_H=26    # chip_bottom=297; gem_top=298 → ~1px gap
  BOT_GEM_CY=309            # fixed

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-tab/opt-a/round_1.png
"""
import os, sys, math
...
# opt-A — moderate lower.
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 228, 174, 100  # shelf_bottom=328
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 252, 8
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD           = 88, 26, 8
CHIP_CY                            = 284   # chip_bottom=297; gem_top=298 → ~1px gap
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-tab opt-A  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-tab", "opt-a", "round_1.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

img   = Image.open(OUT)
btn   = img.getpixel((18 + 58,  54 + 252))
chip  = img.getpixel((18 + 100, 54 + 284))
