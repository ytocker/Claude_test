"""Round-2 exploration sheet for design_4 · NEST-BABY (baby parrot, rare).

Same layout as round 1 so the iteration reads side-by-side: the 40px TRUTH-READ
on BOTH skies leads (the north star — the twig-BOWL nest + peep-beak must survive
the in-game downsample), then the hero close-up (judges the bowl banding + grass
+ jaw detail the crop shrinks) and the in-gameplay day/night panels + filmstrips.
Pure capture; the builder is never registered.

Run headless from repo root:
``SDL_VIDEODRIVER=dummy python tools/render_baby_parrot_design4_round2.py``.
"""
...
def _filmstrip(bg):
    """All four wing frames so the bowl + beak anchor are judged across the flap."""
    cell = 120
...
title = _font(28, True).render(
    "NEST-BABY · RARE · baby_parrot design_4 · round 2",
    True, _GOLD_PALE)
sheet.blit(title, title.get_rect(midtop=(sheet_w // 2, 18)))
sub = _font(14, False).render(
    "nest rebuilt as a twig-BOWL (5 fat horizontal bars + hard ink underline) · grass stalks poke up · ink-keyed peep-jaw rooted to head · demoted eyes under aviators · hard wing pop",
    True, (208, 190, 160))
...
y = TITLE_H
x = PAD
sheet.blit(lab.render("40px TRUTH-READ FIRST — does it read 'baby bird IN a nest'? (day | night)",
...
sheet.blit(hero, (x, y))
sheet.blit(lab.render("HERO CLOSE-UP (bowl banding + grass + jaw)", True, _GOLD_PALE),
           (x + 4, y + HERO + 4))
