"""Round 1 — concept A "shelf-corners" (rarity-gems, store confirm popup v3).

Two facet gems pinned in the shelf's bottom rounded corners. AFFORDABLE shows the
epic-rarity hue with full aura; UNAFFORDABLE swaps to dead slate + half aura so the
...
Output -> docs/store_confirm_shelf_v3/rarity-gems/shelf-corners/round_1.png
"""
import os, sys, math
...
def _draw_corner_gems(big, affordable):
    # Concept A: two gems pinned in the shelf's bottom rounded corners. Aura wells
    # up first so the halo sits behind the facet body, never washing over it.
    if affordable:
        base, deep       = PAL["gem"], PAL["deep"]
        glow, peak       = PAL["glow"], 28
    else:
        base, deep       = MUTED_BASE, MUTED_DEEP
        glow, peak       = (90, 92, 110), 14

    for cx in (27, 173):
        sc._alpha_aura(big, m(cx), m(314), m(14), glow, peak=peak, layers=12)
        sc.facet_gem(big, m(cx), m(314), m(9), base, deep)
...
CANVAS_H = MARGIN + HDR_H + POP_H + MARGIN         # 420 (task quotes 412; MARGIN-tight)
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   ...
                   "shelf-corners", "round_1.png")
OUT = os.path.abspath(OUT)
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

# ── PIL verification: gem centres must be non-background on the affordable panel ─
BG = (8, 8, 20)
...
for (x, y) in [(27, 314), (173, 314)]:
    p = aff.getpixel((x, y))
    print(f"  affordable gem ({x},{y}): {p}  {'ok non-bg' if p != BG else 'WARN bg'}")
