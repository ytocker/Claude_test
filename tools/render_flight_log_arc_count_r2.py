...
def draw_overlay(ss):
    ...
    for p, rad, fn, col in ring_items:
        x, y = arc_pos(p, rad)
        if p > DEATH_PHASE:
            # Ink silhouette — shape visible, identity hidden. No colored connector.
            ink_sil = (46, 42, 56)
            if rad != R_INNER:
                ax, ay = arc_pos(p, R_INNER)
                pygame.draw.line(ss, (*ink_sil, 80), (ax * k, ay * k), (x * k, y * k),
                                 int(1.0 * k))
            stamp_glyph(ss, fn, x * k, y * k, 5.2 * k, ink_sil, ink_pad=int(2.2 * k))
        else:
            if rad != R_INNER:
                ax, ay = arc_pos(p, R_INNER)
