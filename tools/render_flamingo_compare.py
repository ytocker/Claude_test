...
COLUMNS = [
    ("ORIGINAL", "detached triangle", "skin_flamingo"),
    ("DESIGN 1", "ROUNDED TUFT", "design_1"),
    ("DESIGN 2", "LAYERED FEATHERS", "design_2"),
    ("DESIGN 3", "SLEEK SWEPT", "design_3"),
    ("DESIGN 4", "UPSWEPT WISPS", "design_4"),
    ("DESIGN 5", "FLUFFY POMPOM", "design_5"),
]
...
def _source(spec):
    if spec == "skin_flamingo":
        return "skin_flamingo"
    return importlib.import_module(f"tools.flamingo_candidates.{spec}").build
...
def main():
    ...
    _label(sheet, "FLAMINGO  ·  tail revision  ·  fixing the detached-triangle tail",
    ...
    for i, (tag, name, spec) in enumerate(COLUMNS):
        ...
        hl = (255, 150, 150) if spec == "skin_flamingo" else (255, 178, 200)
    ...
    out = "docs/store_redesign/animal/flamingo/final_comparison.png"
