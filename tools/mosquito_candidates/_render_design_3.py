"""Render design_3 NEON SKEETER into one labeled review sheet:
hero product-shot + in-gameplay (day + night) + 40px NEAREST truth reads +
the 4-frame flap strip (the wing-beat that carries the neon shimmer).
"""
import os
import sys
...
from tools.mosquito_candidates.design_3 import build
from game import biome
from game.draw import get_sky_surface_biome, draw_mountains, draw_ground, draw_cloud
from game.entities import Pipe
from game.config import W as GW, H as GH, GROUND_Y

OUT = "docs/store_redesign/animal/mosquito/design_3/round_1.png"
TITLE = "DESIGN 3 — NEON SKEETER  R1"
...
def _frame_strip(box):
    """The 4 flap poses on a flat panel — read the wing-spread + neon beat."""
    strip = pygame.Surface((box * 4, box), pygame.SRCALPHA)
    labels = ("down", "mid-dn", "mid-up", "up")
    small = pygame.font.SysFont("Arial", 12)
    for i in range(4):
        cell = pygame.Surface((box, box), pygame.SRCALPHA)
        pygame.draw.rect(cell, (18, 16, 26), cell.get_rect(), border_radius=10)
        fr = build(i, 0.0)
        sw, sh = fr.get_size()
        sc = (box * 0.86) / max(sw, sh)
        fr = pygame.transform.smoothscale(
            fr, (max(1, int(sw * sc)), max(1, int(sh * sc))))
        cell.blit(fr, fr.get_rect(center=(box // 2, box // 2 + 8)))
        cell.blit(small.render(labels[i], True, (180, 186, 200)), (8, 6))
        strip.blit(cell, (i * box, 0))
    return strip
...
def main():
    ...
    hero = hero_panel(build, 300, bg=(16, 14, 24))
    ...
    sheet_h = top + label_h + body_h + pad + strip_box + label_h + pad

    sheet = pygame.Surface((sheet_w, sheet_h))
    sheet.fill((24, 22, 32))
    sheet.blit(font.render(TITLE, True, (240, 242, 248)), (pad, 16))
    ...
    sy = top + label_h + body_h + pad
    sheet.blit(small.render("4-frame flap strip", True, (180, 186, 200)), (pad, sy))
    sheet.blit(strip, (pad, sy + label_h))
