"""Round 1 — shelf concept 'twilight-vault' (store_confirm_shelf_v3 series).
...
Output → docs/store_confirm_shelf_v3/twilight-vault/round_1.png
"""
import os, sys, math
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    ...
    # CANCEL recedes: a duller teal + lower sheen so BUY stays the anchor.
    ...
    if locked:
        ...
        stops   = [(0.0, (23, 98, 96)), (1.0, (12, 65, 65))]
        lab_col = (157, 174, 172)
        sheen   = 14
    else:
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    if affordable:
        face_stops = [(0.0, (8, 30, 34)), (1.0, (5, 22, 26))]
    else:
    ...
    # digit width. Drawn on an alpha overlay so the 55% gold blends onto the
    # dark teal face instead of punching a hole in the base surface.
    if affordable:
        num_h = num_font.size(txt)[1]
        baseline = (num_cy - num_h // 2) + num_font.get_ascent()
        hair_h = max(1, m(1))
        hair = pygame.Surface((num_w, hair_h), pygame.SRCALPHA)
        hair.fill((206, 172, 96, int(255 * 0.55)))
        big.blit(hair, (num_cx - num_w // 2, baseline + m(3)))
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "twilight-vault r1  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "twilight-vault", "round_1.png")
...
buy_rim = aff_final.getpixel((BUY_CX, BTN_CY - BTN_H // 2 + 1))
print(f"BUY rim @ ({BUY_CX},{BTN_CY - BTN_H//2 + 1}): {buy_rim}  "
      f"→ warm gold R>B: {'OK' if buy_rim[0] > buy_rim[2] else 'FAIL'}")

# Button teal vs chip face luma delta (concept requires 20+).
buy_face  = aff_final.getpixel((BUY_CX, BTN_CY))
chip_face = aff_final.getpixel((CX + 40, CHIP_CY))
def _lum(p): return p[0]*0.299 + p[1]*0.587 + p[2]*0.114
print(f"BUY face {buy_face} lum={_lum(buy_face):.1f} | chip face {chip_face} "
