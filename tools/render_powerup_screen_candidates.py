...
# ── Hi-res variants ─────────────────────────────────────────────────────────
# Render at 3x scale natively (1080x1920) so text, icon edges, and rounded
# corners are crisp — not bilinear-blurry like a smoothscale upscale.

HIRES_S = 3

def _hires_text_outlined(surf, txt, center, size, fill=_GOLD_BRIGHT,
...
def _hires_dark_panel(surf, rect, radius, alpha):
    rounded_rect(surf, rect, radius, _PANEL_DARK, alpha)
    accent = pygame.Surface((rect.width - radius * 2, 2 * HIRES_S),
                            pygame.SRCALPHA)
    accent.fill((*_ORANGE_BORDER, 80))
    surf.blit(accent, (rect.x + radius, rect.y + 3 * HIRES_S))


def _hires_pill_btn(surf, center, text, size=20, alpha=255):
    f = _font(size, True)
    img = f.render(text, True, WHITE)
    pad_x = 44 * HIRES_S
    pw = img.get_width() + pad_x
    ph = img.get_height() + 22 * HIRES_S
    pill = pygame.Surface((pw, ph), pygame.SRCALPHA)
    for y in range(ph):
        t = y / max(1, ph - 1)
        c = lerp_color((200, 64, 24), (126, 28, 2), t)
    ...
    pygame.draw.rect(border, (*_ORANGE_BORDER, 255), (0, 0, pw, ph),
                     width=2 * HIRES_S, border_radius=ph // 2)
    pill.blit(border, (0, 0))
    pygame.draw.line(pill, (255, 255, 255, 40),
                     (ph // 2, 2 * HIRES_S),
                     (pw - ph // 2, 2 * HIRES_S))
...
def _hires_seeded_stars(seed, n, sw, sh, top, bot):
    import random
    rng = random.Random(seed)
    return [
        (rng.randint(8 * HIRES_S, sw - 8 * HIRES_S),
         rng.randint(top, bot),
         rng.choice((1, 1, 1, 2)) * HIRES_S,
         rng.uniform(0, 6.28))
        for _ in range(n)
    ]


def _hires_gradient_bg(surf):
    TOP = (8, 4, 32)
    MID = (16, 8, 50)
    BOT = (24, 14, 70)
    h = surf.get_height()
...
def _hires_powerup_icon(surf, kind, cx, cy, size_px):
    # Render at 4x of the requested size for super-clean edges, then down.
    src = pygame.Surface((256, 256), pygame.SRCALPHA)
    p = PowerUp(128, 128, kind)
    p.pulse = 1.6
    # The powerup `draw` is sized for ~28 px footprints, so we render to a
    # small canvas first and then upscale before final downscale — keeps
    # the procedural sprite pixel-true to the in-game look.
    small = pygame.Surface((64, 64), pygame.SRCALPHA)
    p2 = PowerUp(32, 32, kind)
    p2.pulse = 1.6
    p2.draw(small)
    src = pygame.transform.smoothscale(small, (size_px, size_px))
    surf.blit(src, src.get_rect(center=(cx, cy)))


def _hires_wrap_lines(font_obj, blurb, max_w):
    words = blurb.split()
    cur = ""
    lines = []
    for w in words:
...
def candidate_1_hires():
    s = HIRES_S
    sw, sh = W * s, H * s
    surf = pygame.Surface((sw, sh))
    _hires_gradient_bg(surf)

    stars = _hires_seeded_stars(seed=11, n=55, sw=sw, sh=sh,
                                top=8 * s, bot=sh - 30 * s)
    _draw_overlay_stars(surf, stars, t=0.7)

    # Mountain silhouette — scale the polygon points by s.
    ...
    # Title
    _hires_text_outlined(surf, "POWER-UPS", (sw // 2, 50 * s),
    ...
    for i, (kind, name, blurb) in enumerate(POWERUPS):
        ...
        rect = pygame.Rect(base_x, ry, row_w, row_h)
        _hires_dark_panel(surf, rect, radius=12 * s, alpha=205)
        ...
        for li, line in enumerate(_hires_wrap_lines(f, blurb, max_w)[:2]):
            img = f.render(line, True, UI_CREAM)
...
def candidate_2_hires():
    s = HIRES_S
    sw, sh = W * s, H * s
    surf = pygame.Surface((sw, sh))
    _hires_gradient_bg(surf)

    stars = _hires_seeded_stars(seed=22, n=60, sw=sw, sh=sh,
                                top=8 * s, bot=sh - 30 * s)
    _draw_overlay_stars(surf, stars, t=1.4)

    _hires_text_outlined(surf, "POWER-UPS", (sw // 2, 46 * s),
    ...
    for idx, (kind, name, blurb) in enumerate(six):
        ...
        y = grid_top + row * (card_h + gap)
        card = pygame.Rect(x, y, card_w, card_h)
        _hires_dark_panel(surf, card, radius=14 * s, alpha=215)

        _hires_powerup_icon(surf, kind, card.centerx, card.y + 32 * s, 48 * s)

        nimg = _font(14 * s, True).render(name, True, _GOLD_BRIGHT)
        surf.blit(nimg, nimg.get_rect(center=(card.centerx, card.y + 66 * s)))

        f = _font(10 * s, False)
        for li, line in enumerate(_hires_wrap_lines(f, blurb,
                                                    card_w - 16 * s)[:3]):
            img = f.render(line, True, UI_CREAM)
            surf.blit(img,
    ...
    surprise_card = pygame.Rect(base_x, sy, card_w * 2 + gap, 70 * s)
    _hires_dark_panel(surf, surprise_card, radius=14 * s, alpha=220)
    _hires_powerup_icon(surf, "surprise",
                        surprise_card.x + 40 * s,
                        surprise_card.centery, 50 * s)
    nimg = _font(15 * s, True).render("SURPRISE", True, _GOLD_BRIGHT)
    surf.blit(nimg, (surprise_card.x + 80 * s, surprise_card.y + 12 * s))
    f = _font(11 * s, False)
    blurb = "Gift box rerolls into one of the six above."
    text_left = surprise_card.x + 80 * s
    text_right = surprise_card.right - 12 * s
    for li, line in enumerate(_hires_wrap_lines(f, blurb,
                                                text_right - text_left)[:2]):
        img = f.render(line, True, UI_CREAM)
        surf.blit(img,
