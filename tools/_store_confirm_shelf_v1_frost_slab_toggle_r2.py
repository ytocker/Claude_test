"""Round 2 — shelf concept 'frost-slab-toggle'.

Same single frosted-glass slab as Round 1. Director revisions applied:
- BUY warm wash alpha 30→52, mid gradient stop warmed toward gold,
  inner aura peak 45→62 so BUY reads as frosted-gold hero.
- Mullion replaced by a 2px dark core + 1px top-lit BUY-side highlight
  so it reads as a beveled channel in one continuous slab, not a button gap.
- Single unified sheen across the full slab top; per-pane sheens removed.
- Disabled padlock + label lifted to (140,142,162) for legibility.
- "NOT ENOUGH" bumped to font(9) in a slightly warmer cool-grey.
- CANCEL keyline updated to (18,20,40) / kw=m(1.0) so it reads interactive.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE.  Verify with PIL, never by viewing.
Output → docs/store_confirm_shelf_v1/frost-slab-toggle/round_2.png
"""
import sys, os, math
...
def _draw_shelf(big, affordable):
    ...
    # Base frosted-glass ground — only the price lane shows it directly; the
    # pane fills cover the lower section.  No per-pane sheen here; one unified
    # sheen lands after all fills to sell the single-sheet read.
    slab.blit(sc.vgrad_stops(W, H, 0,
              [(0.0, (46, 48, 72)), (1.0, (30, 32, 54))], 255), (0, 0))

    # ── BUY pane fill (left ~62%) ────────────────────────────────────────────
    buy = pygame.Rect(0, lane_h, m(BUY_W), H - lane_h)
    if affordable:
        # Mid stop warmed from (64,58,90) toward (78,66,82) to tip the neutral
        # frost into a frosted-gold reading without losing the glass darkness.
        buy_stops = [(0.0, (52, 46, 76)), (0.5, (78, 66, 82)), (1.0, (40, 36, 62))]
    else:
        buy_stops = [(0.0, (38, 38, 54)), (1.0, (26, 26, 42))]
    slab.blit(sc.vgrad_stops(buy.w, buy.h, 0, buy_stops, 255), buy.topleft)
    if affordable:
        # Alpha raised 30→52 so the warm-cream wash pushes BUY body luminance
        # and the R-B delta toward the frosted-gold hero target.
        wash = pygame.Surface(buy.size, pygame.SRCALPHA)
        wash.fill((255, 238, 196, 52))
        slab.blit(wash, buy.topleft)
    if affordable:
        # Inner aura peak lifted 45→62; clipped to the pane so warmth stays
        # interior and doesn't bleed into the price lane or mullion.
        slab.set_clip(buy)
        sc._alpha_aura(slab, buy.centerx, buy.centery, m(40),
                       (200, 170, 80), peak=62, layers=8)
        slab.set_clip(None)

    # ── CANCEL pane fill (right ~38%) ───────────────────────────────────────
    ...
    # ── Unified sheen across the full slab top ───────────────────────────────
    # One continuous gloss band spanning both panes so the whole surface reads
    # as a single glass sheet with two temperature zones, not two buttons.
    sc.top_sheen(slab, slab.get_rect(), SLAB_RAD, m(14), peak=42)

    # ── price lane (full slab top, drawn after sheen to sit on glass) ────────
    txt      = f"{PRICE}"
    num_font = sc.font(18)
    coin_r   = m(12)
    gap      = m(4)
    ...
    lane_cy  = m(22)                     # slab-local y-centre of the price lane
    if affordable:
    ...
    # ── label content drawn above mullion and bevel ──────────────────────────
    if affordable:
        sc.plain_text(slab, "BUY", sc.font(14), buy.center, (240, 220, 160),
                      shadow_a=120, weight=m(0.9), keyline=(30, 24, 8), kw=m(0.9))
    else:
        # Padlock and label both lifted to (140,142,162) so they read as
        # interactive candidates rather than dead UI at phone scale.
        _padlock(slab, buy.centerx, buy.centery - m(8), (140, 142, 162))
        sc.plain_text(slab, "BUY", sc.font(14),
                      (buy.centerx, buy.centery + m(8)), (140, 142, 162),
                      shadow_a=100, weight=m(0.9), keyline=(20, 20, 30), kw=m(0.9))

    # Updated keyline (18,20,40) at kw=m(1.0) adds enough cool contrast that
    # CANCEL reads as interactive without stealing the stage from BUY.
    sc.plain_text(slab, "CANCEL", sc.font(13), can.center, (170, 166, 190),
                  shadow_a=110, weight=m(0.8), keyline=(18, 20, 40), kw=m(1.0))

    # ── beveled mullion channel ───────────────────────────────────────────────
    # 2px dark core reads as the channel bottom; 1px top-lit highlight on the
    # BUY side (shifted 1px left) completes the bevel so the seam looks like
    # a machined groove in one continuous slab, not a gap between two buttons.
    pygame.draw.line(slab, (18, 18, 32),
                     (m(BUY_W), lane_h), (m(BUY_W), H), 2)
    pygame.draw.line(slab, (*sc.CARD_RING_BRIGHT[:3], 90),
                     (m(BUY_W) - 1, lane_h), (m(BUY_W) - 1, H), 1)
    ...
    if not affordable:
        # Font bumped 7→9 and hue shifted slightly warmer for legibility at
        # phone scale against the dark shelf ground.
        sc.plain_text(big, "NOT ENOUGH", sc.font(9), (m(CX), m(325)),
                      (150, 160, 175), shadow_a=90, weight=m(0.6))
...
def render_popup(name, base_y, affordable):
    ...
    # disc + aura + thumb (LAST so the hero always crowns the composition)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "FROST-SLAB-TOGGLE  |  AFFORDABLE / UNAFFORDABLE  [R2]",
...
OUT = "docs/store_confirm_shelf_v1/frost-slab-toggle/round_2.png"
