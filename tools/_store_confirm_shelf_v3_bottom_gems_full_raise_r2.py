"""Bottom-frame gems — option A: full-raise (R2).

Shelf and ALL contents raised to give bottom gems maximum clearance.
R2 trims dead strip + re-opens the chip↔button gap:
  SHELF_Y=210, SHELF_H=79 (shelf bottom 289, was 297 — removes 14px dead strip).
  CHIP_CY=234 (chip bottom 251), BTN_CY=272 (btn top 257) → 6px clean gap.
  btn bottom 287, gem top 298 → 11px gem clearance.
Bottom gems at cy=309.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/bottom-gems/full-raise/round_2.png
"""
import os, sys, math
...
# Option A: raise shelf + all contents; R2 trims dead strip + re-opens gap
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 210, 174, 79
CHIP_CY                            = 234
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 272, 8
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Bottom gems A: full-raise (R2)  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "bottom-gems", "full-raise", "round_2.png"))
canvas.save(OUT)
