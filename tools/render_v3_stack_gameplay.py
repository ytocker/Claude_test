"""Render the 5 V3 stack picker variants as full-frame gameplay shots.

Output:
  docs/kfc_pillar_variants/v3_stack/v3_stack1..5.png
  docs/kfc_pillar_variants/v3_stack/compare.png

Run from the repo root:

    PYTHONPATH=. python tools/render_v3_stack_gameplay.py
"""
import math
import os
import random
...
from render_v3_stack_variants import install_variant, V3_STACK_VARIANTS  # noqa: E402
...
def main():
    ...
    out_dir = os.path.join("docs", "kfc_pillar_variants", "v3_stack")
    os.makedirs(out_dir, exist_ok=True)

    full_frames = {}

    for key in V3_STACK_VARIANTS:
        label, _ = V3_STACK_VARIANTS[key]
    ...
    # Comparison strip - centre-crop on the middle pillar
    crop_x, crop_y, crop_w, crop_h = 95,  50, 220, 500
    cell_w, cell_h = crop_w, crop_h
    items = list(V3_STACK_VARIANTS.items())
    n = len(items)
