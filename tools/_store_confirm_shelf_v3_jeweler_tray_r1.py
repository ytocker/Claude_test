"""Round 1 — shelf concept 'jeweler-tray' (store_confirm_shelf_v3 series).

Thesis: a warm amber-black tray with a SAPPHIRE call-to-action, so the gold
coin + price on the obsidian chip is the ONLY pure-warm element on the shelf —
the eye is pulled straight to the value the player is spending.

Departures from v2 gold-standard:
  - Shelf is warm amber-black, not indigo; brass lip.
  - BUY / CANCEL are cool sapphire (BUY saturated, CANCEL muted), each ringed
    in the card's gold bevel so the warm frame still binds the cool CTA.
  - Price chip is matte obsidian with a warm-gold numeral + a hairline gold
    rule under the digits — the sole warm accent, no sheen (reads as a set stone
    tray, not a glossy button).

Sheet: left=AFFORDABLE, right=UNAFFORDABLE. Verify with PIL, never by viewing.
Output → docs/store_confirm_shelf_v3/jeweler-tray/round_1.png
"""
import os, sys
...
# jeweler-tray gold bevel — same warm ring the card frame uses, so the cool
# sapphire buttons still read as part of the gilded set.
BEVEL_DEEP   = (58, 48, 22)
BEVEL_BRIGHT = (236, 202, 116)
BEVEL_ALPHA  = 230
BEVEL_W      = max(1, m(2.0))
# grey inert bevel for the unaffordable state (matches the greyed coin).
GREY_DEEP    = (54, 58, 74, 200)
GREY_BRIGHT  = (140, 138, 160, 180)


def _btn(big, btn_rect, rad, label, font_px, fill_stops, lab_col, affordable):
    # Cool sapphire body + warm gold bevel: the temperature clash is the point —
    # the CTA is unmistakably a button while the gold ties it to the frame.
    ...
    if affordable:
        ...
        # Grey bevel drops the warm ring so the whole shelf reads inert.
...
def _draw_chip(big, cx, cy, price, affordable):
    ...
    # digit width — an engraved-value cue that keeps the price the warm anchor.
    if affordable:
        baseline = num_rect.top + num_font.get_ascent()
        rule_y = baseline + m(3)
        rule = pygame.Surface((num_rect.w, max(1, m(1))), pygame.SRCALPHA)
        rule.fill((236, 202, 116, 140))   # ~55% alpha
...
def _draw_shelf(big, affordable):
    ...
    _btn(big, buy, brad, "BUY", 14,
         [(0.0, (72, 96, 196)), (1.0, (44, 62, 150))], (232, 238, 252), affordable)
    _btn(big, can, brad, "CANCEL", 13,
         [(0.0, (62, 76, 145)), (1.0, (37, 48, 110))], (162, 167, 176), affordable)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "jeweler-tray r1  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "jeweler-tray", "round_1.png")
