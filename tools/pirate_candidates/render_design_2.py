"""Compose the PARROT'S PARROT review sheet (hero + gameplay + 40px truth read).

Headless:
  SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy PYTHONPATH=/home/user/skybit \
    python tools/pirate_candidates/render_design_2.py
"""
import os
...
from tools.pirate_candidates.design_2 import build

OUT = "docs/store_redesign/costume/pirate/design_2/round_2.png"
TITLE = "DESIGN 2 — PARROT'S PARROT  (Shoulder-Companion Buccaneer) — Round 2"
...
def _truth_read(box):
    """The 40px-in-motion test: render the frame, NEAREST-shrink to a ~40px bird
    then magnify 3x so the reviewer judges the real on-screen read on day+night."""
