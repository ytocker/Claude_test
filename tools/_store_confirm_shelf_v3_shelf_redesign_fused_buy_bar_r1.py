"""Fused-buy-bar — shelf redesign concept 2 (R1).

Price and action are fused into ONE dominant compound BUY bar: the left
section holds a circular coin badge + price number, the right section holds
the "BUY" label, split by an inner divider seam. CANCEL is demoted to a slim
ghost text line below the bar. No separate chip widget.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/fused-buy-bar/round_1.png
"""
import os, sys, math
...
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 220, 174, 76   # shelf_bottom=296
# No BTN_* or CHIP_* — replaced by compound bar geometry below
BAR_W   = 164   # SHELF_W - 10, centered in shelf
BAR_H   = 36
BAR_CY  = 242   # bar center y
BAR_RAD = 10
# Divider x (logical): split 40% coin/price | 60% BUY
COIN_SEC_W = 68   # logical width of left (coin+price) section
# CANCEL text-only:
CAN_CY  = 278
BOT_GEM_R  = 11
BOT_GEM_CY = 309
...
def _draw_chip(big, cx, cy, price, affordable):
    global _hair_final
    chip = pygame.Rect(0, 0, m(112), m(30))
    chip.center = (cx, cy)
    crad = m(8)

    sc.drop_shadow(big, chip, crad, blur=m(3), alpha=90, dy=m(2))
    # Chip fill sits ~18% lighter than the shelf interior so it reads as a
    # separate plate rather than a hole in the shelf.
    ...
    # Bright inner sheen (top-left lift) reinforces the raised-plate read.
    sc.top_sheen(big, chip, crad, m(11), peak=34 if affordable else 22)
...
def _draw_shelf(big, affordable):
    ...
    # Compound BUY bar
    bar_rect = pygame.Rect(0, 0, m(BAR_W), m(BAR_H))
    bar_rect.center = (m(CX), m(BAR_CY))
    bar_rad = m(BAR_RAD)

    if not affordable:
        bar_stops = [(0.0, (58, 60, 74)), (1.0, (40, 42, 54))]
    else:
        bar_stops = [(0.0, (48, 52, 104)), (1.0, (28, 30, 68))]

    sc.drop_shadow(big, bar_rect, bar_rad, blur=m(3), alpha=100, dy=m(2))
    big.blit(sc.vgrad_stops(bar_rect.w, bar_rect.h, bar_rad, bar_stops, 255), bar_rect.topleft)
    sc.top_sheen(big, bar_rect, bar_rad, m(12), peak=30 if affordable else 10)
    rim_col = (*sc.CARD_RING_BRIGHT, 230) if affordable else (130, 124, 160, 200)
    sc.bevel_rim(big, bar_rect, bar_rad, sc.CARD_RING_DEEP, rim_col, w=max(1, m(2.0)))

    # Inner seam — vertical divider between coin section and BUY label
    seam_x = bar_rect.left + m(COIN_SEC_W)
    pygame.draw.line(big, (*sc.CARD_RING_DEEP[:3], 160),
                     (seam_x, bar_rect.top + m(4)),
                     (seam_x, bar_rect.bottom - m(4)), max(1, m(1)))
    pygame.draw.line(big, (180, 176, 210, 80),
                     (seam_x + max(1, m(1)), bar_rect.top + m(4)),
                     (seam_x + max(1, m(1)), bar_rect.bottom - m(4)), max(1, m(1)))

    # LEFT section: coin badge + price number
    coin_r  = m(13)
    coin_cx = bar_rect.left + m(16)
    coin_cy = bar_rect.centery
    if affordable:
        sc.coin_glyph(big, coin_cx, coin_cy, coin_r)
        num_col = (236, 240, 232)
    else:
        pygame.draw.circle(big, (150, 152, 162), (coin_cx, coin_cy), coin_r)
        pygame.draw.circle(big, (108, 112, 126), (coin_cx, coin_cy), coin_r, width=max(1, m(1)))
        num_col = (140, 144, 152)
    price_txt  = f"{PRICE:,}"
    num_font   = sc.font(18)
    num_cx     = bar_rect.left + m(16) + coin_r + m(5) + num_font.size(price_txt)[0] // 2
    sc.plain_text(big, price_txt, num_font, (num_cx, bar_rect.centery + m(2)),
                  num_col, shadow_a=0, weight=m(0.7))

    # RIGHT section: BUY label (or LOCKED)
    right_cx = seam_x + (bar_rect.right - seam_x) // 2
    if affordable:
        lab_col  = (214, 220, 250)
        buy_font = sc.font(15)
        sc.plain_text(big, "BUY", buy_font, (right_cx, bar_rect.centery),
                      lab_col, shadow_a=110, weight=m(0.8), keyline=(8, 6, 20), kw=m(0.9))
    else:
        lab_col  = (150, 152, 162)
        lock_h   = m(12)
        _padlock(big, right_cx, bar_rect.centery, lock_h, lab_col)

    # CANCEL: slim ghost text below bar
    can_col  = (130, 135, 180) if affordable else (90, 92, 112)
    can_font = sc.font(11)
    sc.plain_text(big, "CANCEL", can_font, (m(CX), m(CAN_CY)), can_col, shadow_a=0)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "fused-buy-bar  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "fused-buy-bar", "round_1.png"))
...
# PIL verification
img    = Image.open(OUT)
bar_px = img.getpixel((18 + 100, 54 + 242))   # bar centre
can_px = img.getpixel((18 + 100, 54 + 278))   # cancel text row
gem_l  = img.getpixel((18 + 33,  54 + 309))   # left bottom gem
print(f"size:{img.size}  bar:{bar_px}  can:{can_px}  gem_l:{gem_l}")
