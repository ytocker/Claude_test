#!/usr/bin/env python3
"""
sun-arc  ·  flight-log arc v2  ·  arc_count — direction C: unknown silhouettes past death.
...
Direction C changes (arc_count):
  ...
  - "? ? ? ?" mystery count shown above BACK pill.
"""
import os
import math
import random
    ...
    for p, rad, fn, col in ring_items:
        ...
        if rad != R_INNER:
            ...
            pygame.draw.line(ss, (*col, 120), (ax * k, ay * k), (x * k, y * k),
                             int(1.0 * k))
        if p > DEATH_PHASE:
            # Ink silhouette — shape visible, identity hidden.
            ink_sil = (46, 42, 56)
...
def render_screen():
    ...
    # DAY COMPLETE prominent sun-disc glow
    xdc, ydc = pos_u(1.0)
    g_dc = soft_glow(22, (255, 206, 92), peak=38, falloff=1.9)
    surf.blit(g_dc, (int(xdc) - 23, int(ydc) - 23), special_flags=pygame.BLEND_ADD)
    ...
    # Unknown event count — you can see the shapes but not name them
    text(surf, "? ? ? ?", 11, center=(W // 2, 570), color=(140, 140, 160), shadow=None, track=4)
