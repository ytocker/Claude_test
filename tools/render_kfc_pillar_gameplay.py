"""Render full-frame gameplay screenshots of each KFC pillar variant.

For each KFC variant V1..V5:
  - install_variant(n) monkey-patches draw_pillar_pair in BOTH
    game.pillar_variants and game.entities so every pillar in the
    rendered scene is drawn with the chosen design.
  - Three pillars are drawn at fixed seeds + biome phase so frames
    are deterministic and only the pillar art differs between PNGs.

Output:
  docs/kfc_pillar_variants/v1.png ... v5.png  (full 360x640 frames)
  docs/kfc_pillar_variants/compare.png        (5-column comparison strip)
...
from render_kfc_pillar_variants import install_variant, KFC_VARIANTS  # noqa: E402


def draw_bg(surf, scroll=0.0, phase=0.62):
    """Sky + clouds + mountains + ground - matches the live game look so the
    KFC pillars are evaluated in real gameplay context."""
...
def main():
    ...
    # Three pillars across the screen with varied gap heights so the user
    # can judge each design at small/large rect sizes.
    pillars = []
    for x, gap_y, seed in [
        ( 35, 300,  9),
        (150, 360, 13),
    ...
    for idx in sorted(KFC_VARIANTS.keys()):
        label, _ = KFC_VARIANTS[idx]
        with install_variant(idx):
            draw_bg(screen, phase=phase)
            for p in pillars:
                p.draw(screen, palette)

            full_frames[idx] = screen.copy()
            out_path = os.path.join(out_dir, f"v{idx}.png")
            pygame.image.save(screen, out_path)
            print(f"saved {out_path}  ({label})")

    # Comparison strip - centre-crop on the middle pillar, scale up so
    # detail reads at thumbnail size.
    crop_x, crop_y, crop_w, crop_h = 95,  50, 220, 500
    SCALE = 1
    cell_w = crop_w * SCALE
    cell_h = crop_h * SCALE

    items = sorted(KFC_VARIANTS.items())
    n = len(items)
    GAP = 14
    LABEL_H = 30
    PAD = 18
    ...
    for i, (idx, (label, _fn)) in enumerate(items):
        x = PAD + i * (cell_w + GAP)
        y = PAD
        crop = full_frames[idx].subsurface(
