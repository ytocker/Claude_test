"""Shelf-redesign: gem-cradle-tab opt-B (medium lower).
...
opt-B — medium lower:
  ...
  BTN_CY=261, BTN_H=30
  CHIP_CY=290, CHIP_H=26    # chip_bottom=303; gems drawn on top of overlap
  BOT_GEM_CY=309            # fixed

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-tab/opt-b/round_1.png
"""
import os, sys, math
...
# opt-B — medium lower.
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 238, 174, 90   # shelf_bottom=328
BTN_W, BTN_H, BTN_RAD              = 76, 30, 9
BTN_CY, BTN_GAP                    = 261, 8
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD           = 88, 26, 8
CHIP_CY                            = 290   # chip_bottom=303; gems drawn on top of overlap
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-tab opt-B  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-tab", "opt-b", "round_1.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

img   = Image.open(OUT)
btn   = img.getpixel((18 + 58,  54 + 261))
chip  = img.getpixel((18 + 100, 54 + 290))
