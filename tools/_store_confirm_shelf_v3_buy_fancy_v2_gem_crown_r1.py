...
Output → docs/store_confirm_shelf_v3/buy-fancy-v2/gem-crown/round_1.png
"""
import os, sys, math
...
def _crown_gems(big):
    # Three legendary gems crown the BUY top edge — drawn AFTER the button so
    # they seat ON the metal lip rather than behind it.
    gem_r = m(8)
    gem_y = m(BTN_CY) - m(15) - gem_r
    for gx in (m(BUY_CX) - m(16), m(BUY_CX), m(BUY_CX) + m(16)):
        sc.facet_gem(big, gx, gem_y, gem_r,
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-crown (A) r1  |  AFFORDABLE / UNAFFORDABLE",
...
# PIL verification
aff = panels_data[0][2]
gem_px = aff.getpixel((BUY_CX, BTN_CY - 15))
print(f"crown gem ({BUY_CX},{BTN_CY-15}): {gem_px}  → "
