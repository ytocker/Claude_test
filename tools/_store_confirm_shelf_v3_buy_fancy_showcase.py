"""Phase 5 showcase: buy-fancy BUY-button differentiation exploration

Panels (left → right):
  0  BEFORE  — C** round_4 (deep-card base, affordable panel)
  A  raised-pedestal    round_2.png
  B  gold-standard-swap round_2.png
  C  glass-cabochon     round_2.png
  D  coin-medallion-tab round_2.png
  E  solid-vs-ghost     round_2.png

Output: docs/store_confirm_shelf_v3/buy-fancy/showcase.png
"""
import sys, os
...
BASE_BF  = os.path.join(BASE_V3, "buy-fancy")

CROP = (18, 54, 218, 394)   # affordable left panel crop from 444×412 sheet → 200×340

# ── BEFORE panel: C** round_4 (same crop) ─────────────────────────────────────
before_img = Image.open(os.path.join(BASE_V3, "c-orig-bg", "round_4.png")).crop(CROP)

# ── Concept panels ─────────────────────────────────────────────────────────────
CONCEPTS = [
    ("A", "raised-pedestal",    "RAISED PEDESTAL"),
    ("B", "gold-standard-swap", "GOLD-STANDARD SWAP"),
    ("C", "glass-cabochon",     "GLASS CABOCHON"),
    ("D", "coin-medallion-tab", "COIN MEDALLION TAB"),
    ("E", "solid-vs-ghost",     "SOLID VS GHOST"),
]

concept_imgs = []
for badge, slug, _ in CONCEPTS:
    path = os.path.join(BASE_BF, slug, "round_2.png")
...
hx = CANVAS_W // 2
hy = MARGIN + HDR_H // 2
draw.text((hx, hy - 8), "BUY BUTTON DIFFERENTIATION — 5 CONCEPTS",
...
OUT = os.path.join(BASE_BF, "showcase.png")
canvas.save(OUT)
