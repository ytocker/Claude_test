"""Compose the IRONCLAD review sheet (hero + in-gameplay + 40px truth read).
...
OUT = "docs/store_redesign/costume/viking/palette/v1/round_1.png"
TITLE = "VIKING PALETTE v1 — IRONCLAD  (classic brown raider · plain de-frost of FROSTREAVER)"
...
def _truth_read(box):
    """The 40px-in-motion test: render three flap poses, NEAREST shrink each to
    a ~40px bird then magnify 3x so the reviewer judges the real on-screen read.
    Three rows × two phase columns (day | night)."""
    ...
    for row, small in enumerate(smalls):
        ...
        big = pygame.transform.scale(tile, (48 * 2, 48 * 2))           # NEAREST x3-ish magnify
        ry = 28 + row * (box - 40) // 3
