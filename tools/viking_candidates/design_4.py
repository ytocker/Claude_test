...
def _icicle(surf, x, y, length, w=3):
    """A downward ice spike: pale-blue body + a bright white inner glint, sized
    to keep a 2px-min point so it survives the 40px downscale."""
    _poly(surf, ICE_HI, [(x - w, y), (x + w, y), (x, y + length)])
    _poly(surf, ICE_WHITE, [(x - w + 1, y), (x + max(1, w - 1), y),
                            (x, y + length - 1)])
...
def _paint(surf, wing_angle_deg):
    ...
    for i in range(-2, 3):
        pygame.draw.circle(surf, (255, 255, 255), (HX - 1 + i * 5, ruff_y - 1), 1)
    ...
    _icicle(surf, HX + 1, HY + 16, 6, w=2)
    _icicle(surf, HX + 7, HY + 16, 7, w=2)
    _icicle(surf, HX + 4, HY + 13, 5, w=2)
    ...
    for sgn, hx0 in ((-1, HX - 9), (1, HX + 9)):
        ...
        _poly(surf, COLD_SHADE,
              [(hx0 - 4, cy + 2), (hx0 + 4, cy + 2), mid,
               (tipx + sgn * 2, cy - 16)])
        _poly(surf, COLD_WOOD,
              [(hx0 - 3, cy + 1), (hx0 + 3, cy + 1),
               (mid[0], mid[1] + 1), (tipx + sgn * 2, cy - 15)])
        # Rimed white frost tip.
    ...
    # Steel-blue spangenhelm dome.
    pygame.draw.ellipse(surf, COLD_SHADE, (HX - 12, cy - 6, 25, 18))
    pygame.draw.ellipse(surf, COLD_WOOD, (HX - 11, cy - 6, 23, 15))
    ...
    # A few icicle drips off the brim so the helm itself reads as frozen.
    _icicle(surf, HX - 8, cy + 7, 4, w=2)
    _icicle(surf, HX + 9, cy + 6, 4, w=2)
...
build = store_skins._make_skin(_paint, base_fn=_frost_base)
