"""Compose the ODINWING ROUND 3 (FINAL) review sheet (hero + in-gameplay + 40px
truth read + 4-frame shimmer filmstrip) and save it to
docs/store_redesign/costume/viking/design_5/round_3.png.

Round 3 carries only the two cheap optional-polish nudges (wing t3/t4 symmetry
+ a 1px gold separation through the throat cluster) over the SHIP-READY round-2
legendary anchor. Scratch only — renders the unregistered candidate builder via
the shared ninja_render harness (production art untouched). Run headless:
  SDL_VIDEODRIVER=dummy PYTHONPATH=/home/user/skybit python tools/viking_candidates/render_5_round3.py
"""
import os
...
OUT = "/home/user/skybit/docs/store_redesign/costume/viking/design_5/round_3.png"
...
def _truth_read(box):
    ...
    survives. The violet body + gold winged-helm + dome-gap-wing must hold."""
...
def _filmstrip(w, h):
    ...
    wing-beat / wing-tip flare / valknut pulse / breathing cape rim are visible
    across a full beat. Each tile is the bounded hero on a flat panel."""
...
def main():
    ...
    _label(sheet, "Allfather Valkyrie Helm  ·  LEGENDARY  ·  viking redesign / design_5  ·  ROUND 3 (FINAL polish)",
           30, 64, 18, GOLD)
    ...
    _label(sheet, "HERO  ·  product panel  ·  symmetric helm-wings", pad, top + 304, 16, SUB)
    ...
    _label(sheet, "4-FRAME SHIMMER FILMSTRIP  ·  wing-beat + wing-tip flare + valknut pulse + breathing cape rim",
