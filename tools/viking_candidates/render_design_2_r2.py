"""Compose the design_2 (JARL GULLHELM) viking ROUND 2 review sheet: hero shot
+ in-gameplay panel + a NEAREST-neighbour 40px "truth read" magnified 3x.
Headless only — reuses the shared ninja_render harness. Exploration only.
"""
import os
...
from tools.viking_candidates.design_2 import build

OUT = "docs/store_redesign/costume/viking/design_2/round_2.png"
TITLE = "JARL GULLHELM — The Golden King-Jarl  (viking design_2 · ROUND 2)"
...
def _truth_read(build_fn, px=40, mag=3):
    """Shrink the bird to px (NEAREST) then magnify (NEAREST) so the sheet
    shows exactly what the player clocks at 40px in motion."""
...
def main():
    ...
    sheet.fill(BG)
    _label(sheet, TITLE, 22, 16, sz=19, bold=True)
    _label(sheet, "continuous fur cloak over the whole rear (no scarlet) · ruff "
                  "is the widest point · gapped crest · one beard band · bone horn",
           22, 42, sz=12, col=SUB)
    ...
    # In-gameplay panel (portrait so the harness crop stays inside the scene).
    gw, gh = 180, 250
