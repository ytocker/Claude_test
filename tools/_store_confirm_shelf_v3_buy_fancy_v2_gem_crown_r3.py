"""buy-fancy-v2 concept A — gem-crown.

A crown of three legendary facet gems arcs over the BUY button's top edge.
BUY carries the canonical Ramp-A gold fill (the card's own gold accent), so the
confirm's primary action reads as the same jewel material as the card. CANCEL
stays plain indigo so the hierarchy is unambiguous. Everything else (card body,
corner gems, name, ribbon, shelf, chip, disc/aura/thumb) is inherited unchanged
from the locked orig-bg hybrid.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/buy-fancy-v2/gem-crown/round_3.png
"""
import os, sys, math
...
def _buy_locked(big, btn_rect, rad):
    # Unaffordable BUY keeps the slate + padlock treatment from the base hybrid.
...
def _buy_gold(big, btn_rect, rad):
    # Gold-on-gold: the card's canonical Ramp-A fill, a gold bevel pair, and a
    # dark-espresso label so type stays legible on warm metal.
    sc.drop_shadow(big, btn_rect, rad, blur=m(3), alpha=140, dy=m(2))
    big.blit(sc.gold_a_fill(btn_rect.w, btn_rect.h, rad, 255), btn_rect.topleft)
    sc.top_sheen(big, btn_rect, rad, m(12), peak=36)
    sc.bevel_rim(big, btn_rect, rad, sc.GOLD_A_RIM_DARK,
                 (*sc.GOLD_A_RIM_BRIGHT, 240), w=max(1, m(2.0)))
    sc.plain_text(big, "BUY", sc.font(14), btn_rect.center, sc.GOLD_A_NUM,
                  shadow_a=0, weight=m(0.8), keyline=(200, 160, 60), kw=m(0.6))


def _crown_gems(big):
    # A refined peak-taper crown accent — a slightly raised center gem flanked
    # by lower, smaller outer gems — reading as a quiet jewel crest, not a
    # theatrical tiara. Drawn AFTER the button so it seats ON the metal lip.
    LEG = sc.RARITY["legendary"]

    # A soft, restrained aura so the cluster glints without spotlighting itself.
    sc._alpha_aura(big, m(58), m(BTN_CY) - m(18), m(16), LEG["glow"],
                   peak=28, layers=10)

    # (x, y, r): center peak modestly raised, outer pair lower + smaller.
    gems = [
        (m(40), m(BTN_CY) - m(15), m(5)),
        (m(58), m(BTN_CY) - m(20), m(7)),
        (m(76), m(BTN_CY) - m(15), m(5)),
    ]
    for gx, gy, gr in gems:
        sc.facet_gem(big, gx, gy, gr, LEG["gem"], LEG["deep"])
        # Upper-left near-white glint — the specular "juice" of a cut jewel.
        pygame.draw.circle(big, (255, 252, 242), (gx - m(3), gy - m(3)), m(1))

    # A 1px dark line seats the cluster on the button lip instead of floating.
    seat_y = m(BTN_CY) - m(13)
    pygame.draw.line(big, (20, 16, 30),
                     (m(BUY_CX) - m(20), seat_y), (m(BUY_CX) + m(20), seat_y),
                     m(1))


def _cancel_indigo(big, btn_rect, rad):
    # Plain indigo secondary action — quiet next to the gold primary.
    ...
    sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                 (*sc.CARD_RING_BRIGHT, 140), w=max(1, m(1.2)))
    sc.plain_text(big, "CANCEL", sc.font(13), btn_rect.center, (168, 162, 200),
                  shadow_a=100, weight=m(0.8), keyline=(8, 6, 20), kw=m(0.8))
...
def _draw_shelf(big, affordable):
    ...
    # Chip seats before the buttons — it sits higher on the shelf (CHIP_CY=258)
    # than the button row (BTN_CY=302).
    ...
    if affordable:
        _buy_gold(big, buy, brad)
        _crown_gems(big)          # gems last on the BUY side, atop the metal lip
    else:
        _buy_locked(big, buy, brad)
    _cancel_indigo(big, can, brad)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-crown (A) r3  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "buy-fancy-v2", "gem-crown",
                   "round_3.png")
...
# PIL verification
aff = panels_data[0][2]
gem_px = aff.getpixel((BUY_CX, BTN_CY - 20))
print(f"center crown gem ({BUY_CX},{BTN_CY-20}): {gem_px}  → "
      f"{'OK gem' if gem_px != (8,8,20) else 'WARN bg'}")
buy_px = aff.getpixel((BUY_CX, BTN_CY))
print(f"BUY center {buy_px}  → "
      f"{'OK gold R>G>B' if buy_px[0] > buy_px[1] > buy_px[2] else 'WARN'}")
can_px = aff.getpixel((CAN_CX, BTN_CY))
print(f"CANCEL center {can_px}  → "
