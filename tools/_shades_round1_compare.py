"""Scratch BEFORE/AFTER review sheet for the SHADES on-Pip seating fix.

For each of the 11 eyewear styles it shows, at a beak-judging zoom:
  * OLD  — the pre-fix on-Pip head crop (loaded from the committed baseline
           modules so the comparison is honest), and
  * NEW  — the current on-Pip head crop, plus
  * ICON — the current product-shot, which must still read as clean glasses.
A (bare) reference cell shows the target seating (high eye, clear orange beak).

The OLD drawers are loaded from a sibling baseline package path passed via
SK_OLD_SHADES so this harness stays a pure renderer with no vendored copies.
Headless. Writes docs/shades/round_1.png.
"""
import importlib
import os
import sys
...
# Pre-fix baseline modules live beside this harness in a path given by the env
# var; the committed copies under the scratchpad are loaded only for the diff.
...
def main():
    ...
    ZOOM = 6
    cw, ch = 36 * ZOOM, 42 * ZOOM           # head-crop tile size
    prod_w = 150
    CELL_W = cw * 2 + prod_w + 48            # OLD | NEW | ICON
    ...
    sheet.blit(title_f.render(
        "SKYBIT — SHADES on-Pip seating fix  ·  round 1  ·  OLD vs NEW head crop @6x + product ICON",
    ...
    for i, (name, mod) in enumerate(items):
        ...
        if mod is None:
            # Bare reference: the target seating — eye high, beak clear.
            bare = head_crop(None, ZOOM)
            r = pygame.Rect(cell.left + 16, cell.top + 24, cw, ch)
            _checker(sheet, r)
            sheet.blit(bare, r.topleft)
            sheet.blit(lab_f.render("(bare) — target seating", True, (230, 236, 248)),
                       (cell.left + 16, cell.top + 4))
            sheet.blit(sub_f.render("eye sits high; orange beak clear at front-right",
                                    True, (180, 188, 206)),
        ...
        sheet.blit(sub_f.render("OLD", True, (255, 150, 150)),
                   (ox, ny + ch + 2))
        ...
        sheet.blit(sub_f.render("NEW", True, (150, 235, 160)),
                   (nx, ny + ch + 2))
    ...
    out = os.path.join(_ROOT, "docs", "shades", "round_1.png")
