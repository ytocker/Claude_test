"""chunky-brush: very heavy rounded strokes, like a thick felt marker."""
import os, sys
...
def _concept_check(face):
    """Both arms fat (w=16), round caps everywhere — heavy felt-marker look."""
    ink = (28, 20, 16)
    cx, cy = sc._TAG_W // 2, int(sc._TAG_H * 0.52)
    vertex = (cx - 2,  cy + 8)
    l_arm  = (cx - 24, cy - 8)    # slightly extended for more presence
    r_arm  = (cx + 28, cy - 22)
    w = 16
    ...
    stroke((20, 14, 10, 80), dx=1, dy=1, ww=w + 2)   # heavy shadow
    stroke(ink)                                         # main chunky stroke
