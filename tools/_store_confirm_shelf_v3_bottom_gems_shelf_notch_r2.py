"""Bottom-frame gems — option D R2: shelf-notch (RE-ROLL).

The shelf surface has two circular bezel holes punched at the gem corner
positions. Gems sit in these holes and read as jewels "set into" the shelf
frame — like stones in a ring setting — rather than floating on top. Buttons
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Bottom gems D: shelf-notch  |  AFFORDABLE / UNAFFORDABLE",
...
BG  = (8, 8, 20)
aff = panels_data[0][2]
for lbl, gx, gy in [("left gem", BOT_GEM_LX, BOT_GEM_CY), ("right gem", BOT_GEM_RX, BOT_GEM_CY)]:
    p = aff.getpixel((gx, gy))
    print(f"  {lbl} ({gx},{gy}): {p}  {'ok' if p != BG else 'WARN bg'}")
p_edge = aff.getpixel((BOT_GEM_LX - NOTCH_R - 1, BOT_GEM_CY))
print(f"  notch edge (left): {p_edge}")
p_btn = aff.getpixel((BUY_CX, BTN_CY))
print(f"  BUY center ({BUY_CX},{BTN_CY}): {p_btn}  {'ok' if p_btn != BG else 'WARN bg'}")
