"""Round 2 (final) — shelf concept 'royal-velvet' (store_confirm_shelf_v3).

Applies the art-director notes on R1:
  * Gold belongs to BUY alone. BUY keeps the mandatory gold bevel; CANCEL trades
    it for a restrained dim-plum rim that frames without gilding, so BUY's gold
    is the unique precious signal.
  * The BUY/CANCEL fill luminance gap widens to ~35-40%. CANCEL drops ~20 luma
    so the value split survives colourblindness (brightness is the CB-safe cue
    since both faces are violet).
  * Chip, shelf, disc/aura/thumb are untouched from R1.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE. Verify with PIL, never by viewing.
Output -> docs/store_confirm_shelf_v3/royal-velvet/round_2.png
"""
import os, sys, math
...
def _btn(big, btn_rect, rad, label, font_px, locked=False, is_cancel=False):
    # Luminous royal-violet BUY is the anchor and the ONLY gilded control: it
    # keeps the mandatory gold bevel. CANCEL sits on a deeper, cooler violet
    # (widened value gap) and takes a tonal dim-plum rim instead of gold, so it
    # frames without competing for the precious signal. Locked BUY drops to a
    # desaturated slate + grey rim + padlock — the inert family of the chip.
    if locked:
        stops   = [(0.0, (72, 60, 92)), (1.0, (48, 40, 66))]
        lab_col = (120, 116, 132)
        sheen   = 12
    elif is_cancel:
        stops   = [(0.0, (100, 60, 150)), (1.0, (70, 38, 120))]
        lab_col = (167, 160, 175)
        sheen   = 16
    else:
    ...
    if locked:
        ...
        # Restrained dim-plum rim: frames CANCEL without gilding it, leaving
        # gold as BUY's exclusive precious cue.
        sc.bevel_rim(big, btn_rect, rad, (34, 16, 54, 200), (120, 90, 160, 110),
                     w=max(1, m(1.3)))
    else:
        # Gold bevel ties the live BUY back to the card's gilded frame.
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "royal-velvet r2  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "royal-velvet", "round_2.png")
...
print(f"CANCEL rim @ ({CAN_CX},{BTN_CY - BTN_H//2 + 1}): {can_rim}  "
      f"-> plum not gold B>R: {'OK' if can_rim[2] > can_rim[0] else 'check'}")
