#!/usr/bin/env python3
"""
sealed_horizon  ·  flight-log arc  ·  round 1

The arc is a curved band (R=163..173, 10px wide). The flown portion has its
cover torn away — the route is visible inside. Past the death marker an opaque
sealed navy flap covers everything. Four gold wax seals mark hidden events.
The tear is the death marker — an interrupted action.
"""
import os
import math
import random
...
# ── build_skybit_bg (copied from render_flight_log_progress_sun_arc.py) ────────
...
def render_screen() -> pygame.Surface:
    ...
    # ── radial components at death angle (used in tear & tab) ─────────────────
    ux_d = math.cos(death_angle)           # ≈ -0.505
    uy_d = -math.sin(death_angle)          # ≈ -0.863  (screen y-down)
    ...
    pygame.draw.polygon(surf, (20, 26, 58), flown_poly)
    ...
    # GEYSER stamp: gold 4-point diamond at approx (120, 256)
    gx, gy, g_r = 120, 256, 8
    diamond = [
        (gx,       gy - g_r),
        (gx + g_r, gy),
    ...
    # Foot mark at start (angle=180°): inward-turned 2px line
    # At 180°, inward radial is (+1, 0) — foot points right toward arc interior
    bs = bkt_pts[0]
    foot_e = (bs[0] + 6, bs[1])
    pygame.draw.line(bkt_lay, (*bkt_col, 150),
                     (int(bs[0]), int(bs[1])), (int(foot_e[0]), int(foot_e[1])), 2)
    # No closing foot at death end (bracket is OPEN there)

    surf.blit(bkt_lay, (0, 0))

    # NEWBIE label at bracket midpoint, offset inward (smaller R, into dome interior)
    ...
    # ══ 5. Death tear  (drawn on top at boundary) ═══════════════════════════════
    ...
    for i in range(n_tear):
        ...
        bx_ = CX + rt * ux_d
        by_ = CY + rt * uy_d        # CY + rt*(-sin(a)) = CY - rt*sin(a) ✓
    ...
    tear_rgba = (200, 190, 170, int(255 * 0.4))

    if len(tear_pts) > 1:
        pygame.draw.lines(tear_lay, tear_rgba, False,
                          [(int(p[0]), int(p[1])) for p in tear_pts], 3)
    ...
    # ══ 6. Gold wax seals ═══════════════════════════════════════════════════════
    def wax_seal(px: float, py: float, radius: int = 5) -> None:
        ix, iy = int(round(px)), int(round(py))
        pygame.draw.circle(surf, GOLD,          (ix, iy), radius)
        pygame.draw.circle(surf, (184, 140, 58), (ix, iy), radius, 2)
    ...
    # DAY COMPLETE: angle=0°, large seal → (348, 430)
    wax_seal(CX + 168.0, float(CY), radius=8)

    # ══ 7. Chrome ═══════════════════════════════════════════════════════════════

    # Header — "FLIGHT LOG" in GOLD (16px), centered at y=28
    _text(surf, "FLIGHT LOG", 16, center=(W // 2, 28), color=GOLD,
          shadow=(0, 0, 0, 160))

    # Sub-header — "DAY 1  ·  PILLAR 25  ·  0:47" in CREAM (11px) at y=50
    _text(surf, "DAY 1  ·  PILLAR 25  ·  0:47", 11,
          center=(W // 2, 50), color=CREAM, shadow=(0, 0, 0, 120))

    # Death callout pill — placed above arc, clear of the band
    callout = "ENDED · 18.4%"
    f9 = font(9)
    cw = f9.size(callout)[0] + 16
    cr = pygame.Rect(0, 0, cw, 20)
    cr.center = (int(death_x) - 2, 192)
    pill = pygame.Surface((cr.w, cr.h), pygame.SRCALPHA)
    pygame.draw.rect(pill, (10, 8, 20, 220), pill.get_rect(), border_radius=10)
    pygame.draw.rect(pill, (*GOLD, 255), pill.get_rect(), width=2, border_radius=10)
    surf.blit(pill, cr.topleft)
    _text(surf, callout, 9, center=cr.center, color=CREAM)

    # Leader line from callout bottom to death position
    _alpha_line(surf, (*GOLD, 120),
                (cr.centerx, cr.bottom), (int(death_x), int(death_y)), 1)

    # BACK button — 80×28 pill centered at (180, 610)
...
def main() -> None:
    ...
    out = os.path.join(ROOT, "docs", "flight_log_arc", "sealed_horizon", "round_1.png")
