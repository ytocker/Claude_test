"""Shelf-redesign: gem-cradle-tab w4 (large downward shift, Δ35).
...
w4 — Δ35:
  SHELF_Y=263, SHELF_H=65  # shelf_bottom=328 (pinned)
  BTN_CY=281, BTN_H=24
  CHIP_CY=312, CHIP_H=26    # chip_bottom=325 (3px from card bottom)
  BOT_GEM_CY=309            # fixed

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-tab/w4/round_1.png
"""
import os, sys, math
...
# w4 — Δ35 downward shift; shelf bottom pinned at card bottom (328).
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 263, 174, 65  # SHELF_H = 328 - SHELF_Y
BTN_W, BTN_H, BTN_RAD              = 76, 24, 9
BTN_CY, BTN_GAP                    = 281, 8
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD           = 88, 26, 8
CHIP_CY                            = 312   # chip_bottom=325 (3px from card bottom)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-tab w4 | AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-tab", "w4", "round_1.png"))
...
print(f"w4  size:{img.size}  btn:{btn}  chip:{chip}  gem_l:{gem_l}")
