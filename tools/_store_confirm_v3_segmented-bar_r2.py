#!/usr/bin/env python3
"""
segmented-bar  ·  store buy-confirmation popup v3  ·  round 2

Carries forward the single machined-bar concept from R1.
R2 addresses the art-director punch list:
  - Vertical groove BUY-side lip swapped to warm gold so the milled cross
    is symmetric and both lips are visible.
  - Unaffordable BUY darkened below CANCEL so affordance hierarchy is correct.
  - CANCEL face lifted slightly and given a half-strength sheen so it reads
    as a pressable cell distinct from the price display.
  - Horizontal groove contrast differential deepened (brighter top, darker floor).
  - Vertical groove clipped to button row only; price face is now unbroken.
"""
import os
import sys
...
def _hchannel(big, x0, x1, y_top, h):
    """Horizontal chamfered groove: a brighter top lip and a very dark floor so
    the eye reads bright→dark→darker top-to-bottom, giving a convincing recessed
    milled seam between the price face and the button row."""
    lw = max(1, sc.m(1))
    pygame.draw.rect(big, (8, 8, 18), (x0, y_top, x1 - x0, h))
    # brighter top lip for a steeper contrast differential
    pygame.draw.line(big, (70, 68, 90), (x0, y_top), (x1, y_top), lw)
    # deeper floor shadow so the chamfer bottom actually registers
    pygame.draw.line(big, (2, 2, 8), (x0, y_top + h - lw), (x1, y_top + h - lw), lw)


def _vchannel(big, x_center, y0, y1, valley_w):
    """Vertical chamfered groove splitting BUY|CANCEL in the button row only.
    BUY-side lip is a warm gold catch that mirrors the gold cell it borders;
    CANCEL-side lip is cool so both lips are visible and the milled cross is
    symmetric — previously the BUY lip was purple-grey and vanished against the
    gold fill."""
    lw = max(1, sc.m(1))
    vx = x_center - valley_w // 2
    # valley matches the horizontal groove depth
    pygame.draw.rect(big, (8, 8, 16), (vx, y0, valley_w, y1 - y0))
    # BUY-side (left) lip: warm gold catch — readable against the gold cell
    pygame.draw.line(big, (220, 180, 100), (vx, y0), (vx, y1), lw)
    # CANCEL-side (right) lip: cool — readable against the slate cell
    pygame.draw.line(big, (60, 58, 80), (vx + valley_w - lw, y0),
...
def _patched_draw_confirm(self, surf):
    ...
    #    CANCEL fill lifted slightly so it reads as a pressable control distinct
    #    from the price face above it, without competing with gold BUY.
    #    Disabled BUY is darkened below live CANCEL so affordance hierarchy is
    #    preserved: the inert cell must recede, the active one must advance.
    ...
    if affordable:
        ...
        # darker than CANCEL so the disabled state visually recedes
        buy_stops = [(0.0, (22, 20, 32)), (1.0, (14, 12, 24))]
        buy_sheen = 8
    # CANCEL: lifted from (30,28,44)→(20,18,32) so it reads as an actionable cell
    can_stops = [(0.0, (38, 36, 54)), (1.0, (28, 26, 44))]
    # CANCEL sheen: half the BUY sheen — just enough separation from the price face
    _button_row(big, bar, bar_rad, btn_top_local, btn_h_local, split_local,
                buy_stops, can_stops, buy_sheen, 18)

    # 3. horizontal groove between price + buttons — deeper contrast differential
    _hchannel(big, bar.x, bar.right, m(DIV_Y), m(BTN_TOP) - m(DIV_Y))

    # 4. vertical groove clipped to button row only so the price face is unbroken;
    #    only the button row is divided by the split.
    _vchannel(big, m(VCH_X), m(BTN_TOP), bar.bottom, max(2, m(3)))
...
hdr = store_mod._font(16, True).render(
    "segmented-bar R2 · MILLED CROSS · WARM BUY-LIP · CANCEL LIFT",
    True, (200, 196, 224))
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_popup_v3", "segmented-bar", "round_2.png")
...
# Confirm the three key correctness invariants so the art director can trust
# that R2 actually implements the punch list rather than relying on visual
# inspection of a potentially aliased thumbnail.
...
print("\n── R2 pixel verification ──")

# 1. Price face: sample near the top of the bar in the price segment (y=210,
#    horizontally centred) — should be a cool indigo, not warm.
px_price = afford_img.getpixel((CX, BAR_TOP + 10))
print(f"  price face @ ({CX},{BAR_TOP+10}): RGB={px_price[:3]}  "
      f"(expect cool indigo ~(20-30, 18-26, 34-44))")

# 2. Vertical groove BUY-side lip: sample just left of the centre split in the
#    button row — should carry warm gold, not cool purple-grey.
groove_x = VCH_X - 2   # left of centre = BUY-side lip
groove_y = BTN_CY
px_groove = afford_img.getpixel((groove_x, groove_y))
print(f"  v-groove BUY-side lip @ ({groove_x},{groove_y}): RGB={px_groove[:3]}  "
      f"(expect warm gold component, R>>B)")
...
buy_lum = sum(px_buy_inert[:3]) / 3
can_lum = sum(px_cancel[:3]) / 3
print(f"  unafford BUY   @ ({buy_cx},{sample_y}): RGB={px_buy_inert[:3]}  lum={buy_lum:.1f}")
print(f"  unafford CANCEL@ ({can_cx},{sample_y}): RGB={px_cancel[:3]}  lum={can_lum:.1f}")
status = "PASS" if buy_lum < can_lum else "FAIL"
print(f"  BUY darker than CANCEL: {status}  (buy={buy_lum:.1f} < cancel={can_lum:.1f})")
