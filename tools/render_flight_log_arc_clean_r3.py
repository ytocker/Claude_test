#!/usr/bin/env python3
"""
sun-arc  ·  flight-log arc v2  ·  arc_clean — round 3 — thick amber trail
    ...
    for p, rad, fn, col in ring_items:
        ...
        if rad != R_INNER:
            ...
            pygame.draw.line(ss, (*col, 120), (ax * k, ay * k), (x * k, y * k),
                             int(1.0 * k))
        # Fix 2: ink silhouette for mystery — dead-zone glyphs become dark shapes.
        if p > DEATH_PHASE:
            ink_sil = (46, 42, 56)
    ...
    for pass_ink in (True, False):
        for i in range(steps):
            ...
            if pass_ink:
                ...
                col = lerp_color((255, 120, 30), (255, 225, 120), t ** 0.7)
                pygame.draw.line(ss, (*col, 255), PU(u0), PU(u1), int(core * k))
    for i in range(steps):
        u0 = u_death * i / steps
        u1 = u_death * (i + 1) / steps
        t = i / steps
        pygame.draw.line(ss, (255, 240, 180, int(150 + 105 * t)), PU(u0), PU(u1),
                         max(1, int(2.0 * k)))
...
def trail_bloom(surf):
    ...
    for i in range(60):
        ...
        x, y = pos_u(u_death * t)
        rad = int(12 + 22 * t ** 1.2)
        g = soft_glow(rad, lerp_color((255, 130, 40), (255, 220, 130), t),
                      peak=int(28 + 56 * t ** 1.3), falloff=1.9)
...
def render_screen():
    ...
    # DAY COMPLETE — aspirational sun visible through the veil
    xdc, ydc = pos_u(1.0)
