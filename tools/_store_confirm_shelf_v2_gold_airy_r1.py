"""Round 1 — shelf concept 'gold-airy' (store_confirm_shelf_v2).

The lightest touch: whisper-thin golden bevels at a low alpha barely kiss the
button edges, the indigo fill lifts a touch brighter for airiness, and the coin
CHIP grows wider with a larger coin + a prominent gold bevel so it becomes the
dominant element while the two buttons read as elegant thin accents.

Sheet: left=AFFORDABLE, right=UNAFFORDABLE. Verify with PIL, never by viewing.
Output → docs/store_confirm_shelf_v2/gold-airy/round_1.png
"""
import sys, os, math
...
def _btn(big, btn_rect, label, locked=False):
    ...
    indigo fill plus a thin, low-alpha gold bevel that barely kisses the edge —
    so it never competes with the prominent coin chip above it."""
    rad = m(BTN_RAD)
    if locked:
        ...
        # Brighter than the current (84,78,126)/(50,46,82) for extra air.
    ...
    # Thin bevel, low alpha — the gold whispers, not shouts.
    sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                 (*sc.CARD_RING_BRIGHT, bevel_a), w=max(1, m(1.3)))
...
def _draw_chip(big, affordable):
    ...
    if affordable:
        # Prominent gold bevel — the loudest gold on the shelf.
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2), "gold-airy",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v2", "gold-airy", "round_1.png")
