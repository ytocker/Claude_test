...
rows A1-A8, A13 and A15:
  ...
  A3  the SPARK-BURST system — 70-120 ballistic 1px sparks, 3-frame trails,
      (191,142,82) @ alpha 120-200, apex ceiling y=512, ground bounce + skitter,
      ONE 150-luma 2px core pixel at contact, 2.5 s cycle with a dark beat
  A4  the burst RIM-LIGHT pass — 1px warm top-edge on every silhouette in the
      block for 2 frames (the storm's lightning-silhouette flash, inverted)
  ...
  A13 the LANTERN ARCH — two poles + an arc of 6 lanterns spanning ~120 px
...
def _scaffold(surf, cx, night, t, *, state='manned', wet=0.0):
    ...
    if state in ('manned', 'burst') and night > 0.05:
        _warm_glow(surf, cx, L(g - 9), radius=9, peak=44, color=(150, 92, 46))
...
# ════════════════════════════════════════════════════════════════════════════
# A3 — THE SPARK-BURST SYSTEM.  Ballistic 1 px sparks with 3-frame trails, a
# hard apex clamp at y=512, a ground bounce + skitter, and exactly one 2 px
...
G_ACC = 300.0                 # px/s^2 — tuned so the fastest spark tops out at 512
...
def _spark_at(i, cx, cy, age, *, apex_y=SPARK_CEIL, ground_y=FAR_Y):
    ...
    sp = (_h(i, 3) - 0.5) * 2.0
    ang = math.radians(-90 + (sp ** 3) * 82)
    vx = speed * math.cos(ang)
    vy = speed * math.sin(ang)
    vy = max(vy, -math.sqrt(2 * G_ACC * max(1.0, cy - apex_y)))
    fall = ground_y - cy
...
def _burst(surf, cx, cy, age, night, *, count=96, apron=None, dt=1.0 / 60.0):
    ...
    for i in range(count):
        ...
        x, y, alpha = st
        min_y = min(min_y, y)
        # 3-frame fading trail behind the head, so a 1 px spark reads as motion
        for k, mul in ((2, 0.30), (1, 0.55)):
            old = _spark_at(i, cx, cy, age - k * dt)
            if old is None:
                continue
            ox, oy, _oa = old
            _put(lay, ox, oy, alpha * mul)
        _put(lay, x, y, alpha)
...
def _put(lay, x, y, alpha):
    xx, yy = int(x), int(L(y))
    if 0 <= xx < lay.get_width() and 0 <= yy < lay.get_height():
        a = max(0, min(255, int(alpha)))
        if a > 3:
            lay.set_at((xx, yy), (*SPARK_COL, a))
...
def _burst_smoke(surf, cx, night, age):
    """A grey veil rising to y~540 at alpha 30 after each burst — the residue
    that makes four bursts read as one accumulating show."""
    if age < 0.25:
        return
    _wisp(surf, cx - 4, L(FAR_Y - 50), age * 0.9, n=3, rise=34, spread=3.2,
          speed=0.34, peak_a=30, r0=2, sway=3.6, color=(168, 164, 160))
    _wisp(surf, cx + 7, L(FAR_Y - 46), age * 0.9, n=2, rise=28, spread=2.6,
...
# with the same one-blit machinery — every top edge in the block gets a 1 px
# warm highlight for 2 frames. Same cost, opposite meaning.
# ════════════════════════════════════════════════════════════════════════════

def _rim_light(layer, *, boost=18, cap=RIM_CAP, warm=(255, 208, 150)):
    ...
    light from above and behind the rig would."""
    w, h = layer.get_size()
    edges = []
    for x in range(w):
        prev_op = False
    ...
    for (x, y) in edges:
        ...
        lit = _mix((r, g, b), warm, 0.35)
        lit = (_clamp(lit[0] + boost), _clamp(lit[1] + boost * 0.82), _clamp(lit[2] + boost * 0.55))
...
def _pearl_bearer(surf, cx, night, t, *, feet=NEAR_Y, trace=False):
    sh_y, hy = None, None
    robe = (168, 74, 62)
    hy, sh_y = _person(surf, cx, L(feet), night, h=20, coat=robe, arms='up', face=1)
    # figure-8 at 0.8 Hz: x on the fundamental, y on the second harmonic
    ph = t * 0.8 * math.tau
    px = cx + 4 + int(math.sin(ph) * 9)
    py = L(feet) - 48 + int(math.sin(ph * 2) * 6)
    ...
    if trace:
        for k in range(28):
            ...
            ty = L(feet) - 48 + int(math.sin(a2 * 2) * 6)
            surf.set_at((tx, ty), (96, 108, 132))
    if night > 0.05:
        _warm_glow(surf, px, py, radius=8, peak=40, color=(150, 108, 54))
    amber = _cap150((186, 138, 74)) if night > 0.05 else (216, 168, 96)
    pygame.draw.circle(surf, _shade(amber, -34), (px, py), 5)
    pygame.draw.circle(surf, amber, (px, py), 4)
    pygame.draw.circle(surf, _cap_to(_shade(amber, 22), 138), (px - 1, py - 1), 2)
...
def _drum_cart(surf, cx, night, t, *, feet=NEAR_Y):
    ...
    # the seated drummer behind, two sticks in antiphase
    _person(surf, cx + 14, bed_y, night, h=17, coat=(96, 76, 128), arms='down')
    ...
    for sgn, phi in ((-1, 0.0), (1, 1.7)):
        fx = cx + sgn * 30
        hy2, shy = _person(surf, fx, g, night, h=18,
    ...
    _person(surf, cx - 34, g, night, h=18, coat=(120, 92, 74), arms='point')
    pygame.draw.line(surf, _retint((150, 132, 96), night),
                     (cx - 30, g - 12), (cx - 18, bed_y + 1), 1)
...
# lanterns spanning ~120 px. Drawn HERE at an apex of y=560 so it obeys the
# 560-640 cast/prop band; the plan's grandfathered garland height (GROUND_Y-118
# = 477) is shown as a ghosted alternate for sign-off, not assumed.
# ════════════════════════════════════════════════════════════════════════════

def _lantern_arch(surf, cx, night, t, *, span=120, apex=560, feet=FAR_Y, ghost=False):
    g = L(feet)
    ...
    if ghost:
        for k in range(0, span, 6):
            surf.set_at((x0 + k, top + int((k - span / 2) ** 2 * 0.0022)), (110, 120, 145))
        return
    for px in (x0, x1):
...
def _yardsticks(panel, night, *, x=18, coin_x=None):
...
def _panel_burst(w, night, cyc_t, *, apron=True, crowd=True, rim=True, stall=True,
    ...
    # the whole block goes onto its own layer so the rim-light pass can find
    # every top edge in one sweep — the lightning-flash blit, inverted
    blk = pygame.Surface((w, SLICE_H), pygame.SRCALPHA)
    if stall:
        _stall_ref(blk, 44, L(FAR_Y), night, t=cyc_t)
    ph = 0 if cyc_t < 0.05 else (1 if cyc_t < CONTACT_T else (2 if cyc_t < 0.5 else 3))
    _scaffold(blk, cx, night, cyc_t, state='burst' if 0 <= age < 1.4 else 'manned')
    _thrower(blk, cx - 25, night, ph)
    _striker(blk, cx + 23, night, 2 if 0 <= age < 0.12 else (1 if age < 0 else 3))
    if crowd:
        # the spark-watch arc: backs to us, chins up, and a 2-frame head-lift
        # ripple travelling left->right at 0.06 s per figure on each burst
        for k in range(9):
            fx = 26 + k * ((w - 46) // 9)
            lift = 0
            if 0 <= age - k * 0.06 < 0.14:
                lift = 2
            _person(blk, fx, L(NEAR_Y), night, h=19 + (k % 3), back=True, arms='up' if k % 4 == 1 else 'down',
                    chin=2 + lift, coat=((80, 88, 116), (104, 84, 96), (78, 96, 92))[k % 3])
    if rim and 0 <= age < 2 / 60.0:
        _rim_light(blk)
    p.blit(blk, (0, 0))
...
def _audit():
    ...
    for tag, cyc in (("burst.contact", CONTACT_T + 0.01),
                     ...
                     ("burst.dark", 2.10)):
        # audited at the DENSEST burst the spec allows (120), not the average
        p, st = _panel_burst(300, night, cyc, clean=True, count=120)
        scan(p, tag)
        if st:
    ...
    scan(p, "draped cart")
    p = _panel(200, night)
    _lantern_arch(p, 100, night, 0.5, span=120)
    scan(p, "lantern arch")
    p = _panel(200, night)
    for i in range(4):
...
def render():
    sheet = pygame.Surface((WIDTH, 2240))
    sheet.fill((24, 26, 36))
    y = PAD
    _text(sheet, "FIRE-TREE NIGHT — round 1 · THE IRON FLOWER (A1-A5) + DRAGON-PARADE SUPPORT (A6-A8, A13) + RESIDUE (A15)",
          PAD, y, 17, (250, 246, 236), bold=True)
    y += 21
    y = _wrap(sheet, "Every panel below is a LITERAL screen slice: world y 500-647 at 1x, with the far deck (595), the near deck (638), the 560 cast/prop band ceiling (blue dashes) and the y=512 spark ceiling (amber dashes) drawn in. "
                     "So the vertical budget is verifiable by eye. Datiehua is the one fire form that reads through spark COUNT + ARC instead of brightness, which is why it survives the 150-luma cap: the sparks are (191,142,82) at alpha 120-200 over dark paving, and exactly ONE 2px core pixel per burst sits ON the cap for 4 frames. "
                     "The audit at the foot is measured on rendered pixels.",
    ...
    ys.append(_place(sheet, p3, PAD * 3 + pw * 2, row_y, "S3 · MID-BURST — the strike frame",
                     sub="~96 ballistic sparks, the doused apron mirroring them, the 8-deep spark-watch crowd backs-to-us with chins up, and the rim-light pass firing on every top edge in the block."))
    ...
    _text(sheet, "A3 · THE BURST CYCLE — one full 2.5 s cycle in 8 phases (the plan's 2.6 s cadence, with the DARK BEAT that makes the next throw land). 70-120 sparks/burst; here 96. Ballistic with gravity 300 px/s², 3-frame fading trails, apex clamped at LAUNCH so no spark can physically reach above y=512, ground bounce + 0.4 s skitter.",
          PAD, y, 12, (240, 220, 150), bold=True)
    y += 18
    ...
    for cyc, cap in phases:
        p, st = _panel_burst(bw, 0.95, cyc, count=104)
        if st:
    ...
    _wrap(sheet, "Why this survives the cap: the eye reads a fire from the NUMBER of moving points and the SHAPE of their arcs, not from peak luminance. At 96 points, three-frame trails and a 33 px rise, the burst reads unmistakably hot at an effective 90-130 luma — the coin at 230 is still, by a wide margin, the brightest pixel on the screen. "
                 "The dark beat between bursts is doing as much work as the burst: four bursts at 2.6 s with 1.1 s of nothing means the player's eye resets and the next throw lands as an event rather than as ambience. "
                 "The apex clamp is applied to LAUNCH VELOCITY (vy is capped at sqrt(2·g·(cy-512))), not to the drawn pixel, so no spark is ever clipped mid-flight — the parabolas stay honest and simply cannot exceed the ceiling.",
          PAD + z + 16, y + 14, WIDTH - PAD * 2 - z - 30, 10, (200, 198, 208), 12)
    y += 14 + 110 * 3 + 14

    # ── A4 rim light ─────────────────────────────────────────────────────────
    _text(sheet, "A4 · THE BURST RIM-LIGHT PASS — 2 frames, a 1 px warm top edge on every silhouette in the block, capped 140. This is the storm chapter's lightning-silhouette blit INVERTED: the lightning flattens the street to black, the iron flower rims it in warm light. Same machinery, opposite meaning — and it ties the festival to the storm that paid for it.",
          PAD, y, 12, (240, 220, 150), bold=True)
    y += 18
    rw = (WIDTH - PAD * 3) // 2
    ...
    def _rim_demo(w, on):
        ...
        _guides(p, w)
        blk = pygame.Surface((w, SLICE_H), pygame.SRCALPHA)
        _stall_ref(blk, 60, L(FAR_Y), 0.95, t=0.7)
        _person(blk, w // 2 - 24, L(NEAR_Y), 0.95, h=20, back=True, chin=2, coat=(80, 88, 116), arms='up')
        _person(blk, w // 2 + 16, L(NEAR_Y), 0.95, h=19, back=True, chin=2, coat=(104, 84, 96))
        _scaffold(blk, w - 60, 0.95, 1.0, state='burst')
        if on:
            _rim_light(blk)
        p.blit(blk, (0, 0))
        _yardsticks(p, 0.95)
    ...
    off_p = _rim_demo(rw, False)
    on_p = _rim_demo(rw, True)
    ...
    y2 = _place(sheet, on_p, PAD * 2 + rw, y, "RIM ON — the 2 frames after contact",
                sub="Hat brims, shoulders, the awning ribs and the stall's counter edge all catch a 1 px warm line. The crowd stops being a mass and becomes 60 separate people, for two frames.")
    y = max(y1, y2)
    z = _zoom(sheet, on_p, (rw // 2 - 40, L(NEAR_Y) - 26, 76, 30), 4, PAD, y + 14, "4x · rim ON — two backs, chins up")
    z2 = _zoom(sheet, off_p, (rw // 2 - 40, L(NEAR_Y) - 26, 76, 30), 4, PAD + z + 20, y + 14, "4x · rim OFF (same frame)")
    _wrap(sheet, "Detection is 'opaque pixel whose neighbour above is transparent' on the block's own SRCALPHA layer, so it is ONE sweep per burst frame over one layer — the same cost profile as the existing lightning flash. Highlight is mixed 35% toward (255,208,150), +18 value, then hard-capped at 140 luma so a rimmed shoulder can never approach the coin.",
    ...
    _guides(pp, qw)
    for i, tt in enumerate((0.0, 0.42, 0.84)):
        _pearl_bearer(pp, 34 + i * 52, 0.95, tt, trace=(i == 1))
    _yardsticks(pp, 0.95, coin_x=qw - 22)
    y1 = _place(sheet, pp, PAD, y, "A6 · PEARL + BEARER — figure-8 @ 0.8 Hz (path traced on the middle figure)",
                sub="Pole overhead, 10 px amber sphere, capped halo r8, a tassel so it reads as SWUNG rather than floating. The bearer teases it up-LEFT — toward Pip — so the object leading the parade points at the player.")
    dp = _panel(qw, 0.95)
    _guides(dp, qw)
    _drum_cart(dp, qw // 2, 0.95, 0.34)
    _yardsticks(dp, 0.95)
    y2 = _place(sheet, dp, PAD * 2 + qw, y, "A7 · DRUM-AND-CYMBAL CART — ~70 px, drawn to lead",
                sub="Barrel drum at 1.5x the busker's on a 2-wheel chassis (the weekend_kit spoked wheel), a seated drummer striking in antiphase, two flanking cymbal figures whose clash is a 1-frame capped ivory disc, and a PULLER at the front — the platform is dragged, per the source.")
    cp2 = _panel(qw, 0.0)
    _guides(cp2, qw)
    ...
    ap = _panel(qw, 0.95)
    _guides(ap, qw)
    _lantern_arch(ap, qw // 2, 0.95, 0.6, span=120, apex=560)
    _lantern_arch(ap, qw // 2, 0.95, 0.6, span=120, apex=512, ghost=True)
    _person(ap, qw // 2 - 40, L(FAR_Y), 0.95, h=18, coat=(90, 96, 124))
    _person(ap, qw // 2 + 34, L(FAR_Y), 0.95, h=17, coat=(104, 88, 100))
    _yardsticks(ap, 0.95)
    y4 = _place(sheet, ap, PAD * 4 + qw * 3, y, "A13 · LANTERN ARCH — 120 px span, 6 lanterns",
                sub="Drawn with its apex at y=560 so it obeys the 560-640 band. The DOTTED line above is the plan's grandfathered garland height (GROUND_Y-118 = 477) shown for sign-off — not assumed. Poles + catenary + 6 capped shells; a different dressing register from the stall row.")
    y = max(y1, y2, y3, y4) + 4
    ...
    _person(r2, 40, L(NEAR_Y), 0.95, h=19, coat=(78, 86, 110))
    _yardsticks(r2, 0.95)
    r3 = _panel(rw2, 0.95)
    _guides(r3, rw2)
    ...
    y = _wrap(sheet, "SPARK LUMA CALIBRATION — one spark drawn at each sustain alpha over the real night paving (bg %.0f luma), read back off the rendered surface:  %s.  "
                     "Sustain band alpha 120-200 -> effective %.0f-%.0f luma, inside the plan's 90-130 target. Rendered spread including 3-frame trails (the fade tail): %.0f-%.0f."
    ...
    print("AUDIT", {k: (round(v, 1) if isinstance(v, float) else v)
                    for k, v in a.items() if k != "hot_by"})
