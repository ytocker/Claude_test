"""Chip-below gems — option B: buttons-lift, R1.

Buttons rise to the shelf TOP; the price chip drops to the shelf BOTTOM, right
above the bottom-frame gems. Reversing the vertical stack puts the call-to-
action first and reads the price as a footnote to the buttons.

Layout vs bottom-gems/buttons-lift R2:
- Buttons lifted: BTN_CY 275→249, so they seat near the shelf top.
- Chip dropped: CHIP_CY 240→287, near the bottom gems (BOT_GEM_CY=309, still
  inside shelf_bottom=313). CHIP_H stays 30.
- Draw order flips: buttons paint before the chip so the chip is the last shelf
  element and can nest cleanly above the gems.
- NOT ENOUGH text moves to the chip centre m(287).

Layout: SHELF_Y=226, BTN_CY=249, CHIP_CY=287, CHIP_H=30, BOT_GEM_CY=309.
Gem centers at (33,309)/(167,309); BUY center at (58,249); chip center at (100,287).

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/buttons-lift/round_1.png
"""
import os, sys, math
...
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 226, 174, 87
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip-below B: buttons-lift R1  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "buttons-lift", "round_1.png"))
