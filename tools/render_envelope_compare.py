"""ONE comparison figure for the ENVELOPE parcel redesign.

Renders the ORIGINAL "SEALED MAILER" + the 5 final design candidates, each
...
below), side by side and labelled. Pure capture — production art untouched.
    ...
    SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy python tools/render_envelope_compare.py
"""
...
from game.parcel_designs import envelope as original


def _load_build(n):
    p = ROOT / "docs" / "store_redesign" / "parcels" / "envelope" / f"design_{n}" / "build.py"
    spec = importlib.util.spec_from_file_location(f"env_design_{n}", p)
...
NAMES = ["SEALED MAILER", "AIRMAIL", "LOVE LETTER", "ROYAL DECREE",
         "POST OFFICE", "PAPER PLANE"]

# Tight panel around Pip + the parcel hanging below him.
PW, PH = 150, 200
...
def main():
    ...
    sheet.blit(big.render("ENVELOPE parcel — ORIGINAL vs 5 redesigns "
    ...
    out = ROOT / "docs" / "store_redesign" / "parcels" / "envelope" / "final_comparison.png"
