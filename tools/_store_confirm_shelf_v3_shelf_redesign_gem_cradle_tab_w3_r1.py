"""Shelf-redesign: gem-cradle-tab w3 (large downward shift, Δ30).

Derived from opt-A. The action block (shelf top + buttons + chip) is pushed
dramatically lower; buttons slim to keep room while the shelf bottom stays
pinned at the card bottom (328). Buttons keep a ~5px gap to the chip.

w3 — Δ30:
  SHELF_Y=258, SHELF_H=70  # shelf_bottom=328 (pinned)
  BTN_CY=277, BTN_H=24
  CHIP_CY=309, CHIP_H=26    # chip_center exactly at bottom gem center (309); chip_bottom=322
  BOT_GEM_CY=309            # fixed

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/shelf-redesign/gem-cradle-tab/w3/round_1.png
"""
import os, sys, math
...
# w3 — Δ30 downward shift; shelf bottom pinned at card bottom (328).
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 258, 174, 70  # SHELF_H = 328 - SHELF_Y
BTN_W, BTN_H, BTN_RAD              = 76, 24, 9
BTN_CY, BTN_GAP                    = 277, 8
BUY_CX = CX - (BTN_W + BTN_GAP) // 2
CAN_CX = CX + (BTN_W + BTN_GAP) // 2
CHIP_W, CHIP_H, CHIP_RAD           = 88, 26, 8
CHIP_CY                            = 309   # chip_center exactly at bottom gem center (309); chip_bottom=322
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "gem-cradle-tab w3 | AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "shelf-redesign", "gem-cradle-tab", "w3", "round_1.png"))
os.makedirs(os.path.dirname(OUT), exist_ok=True)
canvas.save(OUT)
print(f"Saved {OUT}  ({CANVAS_W}x{CANVAS_H})")

img   = Image.open(OUT)
btn   = img.getpixel((18 + BUY_CX, 54 + BTN_CY))
chip  = img.getpixel((18 + 100,    54 + CHIP_CY))
gem_l = img.getpixel((18 + 33,     54 + 309))
print(f"w3  size:{img.size}  btn:{btn}  chip:{chip}  gem_l:{gem_l}")
