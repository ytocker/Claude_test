#!/usr/bin/env python3
"""
wide-oval  ·  store buy-confirmation popup v3  ·  round 1

Concept: the price sits on a wide, matte stadium (racetrack) oval — a flat
IVORY plaque with only a 1px keyline, deliberately finish-free. Below it,
BUY + CANCEL are two matched GLOSSY pills with full sheen + heavy bevel. The
read is unambiguous by finish alone: the oval is CONTENT (matte, no sheen),
the pills are CONTROLS (glossy, bevelled).
"""
import os
import sys
...
def _patched_draw_confirm(self, surf):
    ...
    ovr = m(OV_RAD)
    sc.drop_shadow(big, ov, ovr, blur=m(4), alpha=110, dy=m(2))
    if affordable:
        # MATTE ivory — no top_sheen, no gloss: the finish alone marks this as
        # CONTENT, the polar opposite of the glossy pills below.
        face = sc.vgrad_stops(ov.w, ov.h, ovr,
                              [(0.0, (235, 220, 175)), (1.0, (205, 190, 145))],
                              255, gamma=1.0)
        big.blit(face, ov.topleft)
        # single lightweight keyline — deliberately no bevel
        ...
        face = sc.vgrad_stops(ov.w, ov.h, ovr,
                              ...
                              255, gamma=1.0)
        big.blit(face, ov.topleft)
        pygame.draw.rect(big, (80, 82, 96), ov, width=max(1, m(1)),
                         border_radius=ovr)
...
hdr = store_mod._font(16, True).render("wide-oval R1 · MATTE IVORY OVAL + GLOSSY PILLS",
                                       True, (220, 190, 100))
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_popup_v3", "wide-oval", "round_1.png")
