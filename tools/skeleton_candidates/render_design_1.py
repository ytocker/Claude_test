"""Render the BONEWHITE (design_1) skeleton candidate review sheet.

ONE combined sheet: Pip mid-flight over a real biome scene on a DAY sky and a
NIGHT sky (the white-on-black read must survive both), a clean hero close-up,
and a 40px NEAREST-neighbour "truth read" thumbnail — the bar the redesign
exists to clear: it must still read as a skeleton shrunk to the gameplay bar.

Headless:  SDL_VIDEODRIVER=dummy python tools/skeleton_candidates/render_design_1.py
"""
import os
...
OUT_DIR = os.path.join(
    ROOT, "docs/store_redesign/costume/skeleton/design_1")
os.makedirs(OUT_DIR, exist_ok=True)

_spec = importlib.util.spec_from_file_location(
    "design_1", os.path.join(ROOT, "tools/skeleton_candidates/design_1.py"))
...
def gameplay_panel_phase(source, w, h, phase):
    """Same composite as ninja_render.gameplay_panel but at an arbitrary
    biome phase so we can prove the day AND night read on one sheet."""
...
def truth_read_panel(source, box):
    ...
    smoothing), centred on a 50/50 light/dark split so it must read against
    both a bright and a dark gameplay backdrop."""
    panel = pygame.Surface((box, box))
    panel.fill((150, 195, 235))                       # day-sky blue, left/top
    pygame.draw.rect(panel, (18, 20, 34),             # night, right/bottom
                     (box // 2, 0, box - box // 2, box))
    frame = source(FRAME_IDX, TILT)
...
def labelled(surf, text, font):
    """Stack a caption strip under a panel."""
...
def main():
    ...
    title = title_font.render(
        "SKELETON redesign — design_1  BONEWHITE  (round 1)",
        True, (255, 255, 255))
