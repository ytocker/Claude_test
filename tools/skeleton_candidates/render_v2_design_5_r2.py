"""Render the v2 AUREX-MACAW (v2_design_5) round-2 review sheet — scratch only.
...
"truth read" (must clock GOLD SKULL + HOOKED BEAK → two violet socket points →
gold crown → long gold tail), and a 4-frame flap filmstrip. Day uses the
near-silent-glow variant, night the blazing one, so the gating is honestly
shown on the corrected PARROT skeleton anatomy.
"""
...
OUT = "docs/store_redesign/costume/skeleton/v2/design_5/round_2.png"
...
def main():
    ...
    sheet.blit(pygame.font.SysFont("dejavusans", 26, bold=True).render(
        "SKELETON v2 — design_5  AUREX-MACAW  (cursed gold-lich)  round 2", True, BONE),
        (pad, 12))
