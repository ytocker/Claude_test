"""Render the v2_design_2 PIRATE-MACAW round-2 review sheet for CAPTAIN.

CAPTAIN (``build_v2``, the art-director's lead take) shown across the four
truth columns — hero close-up, DAY gameplay, NIGHT gameplay, and a 40px NEAREST
truth read — so the C1 fixes (night-rimmed tricorn, bone-on-rib chest X, cockade
glint focal, sharpened cutlass tip, repositioned earring) are judged at both
product and gameplay size. Scratch only.

Headless:
  SDL_VIDEODRIVER=dummy PYTHONPATH=. python tools/skeleton_candidates/render_v2_design_2_r2.py
"""
import os
...
def main():
    ...
    sheet.blit(title_font.render(
        "SKELETON  v2_design_2  PIRATE-MACAW  CAPTAIN  round_2  "
        "(night-rimmed tricorn / bone-on-rib X / cockade glint / sharp cutlass)",
    ...
    sheet.blit(row_font.render(
        "v2 CAPTAIN — black tricorn (bone-rim) + bone crossbones cockade",
