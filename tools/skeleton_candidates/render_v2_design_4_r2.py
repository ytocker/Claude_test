"""Render the WISP-MACAW (v2_design_4) skeleton review sheet — round 2, scratch.

Round 2 folds the C1 critique: the beak notch is carved in green and the new
down-hook top-edge re-lit in core; the tail wins its own bright value channel
while the legs drop to mid; the rib confetti collapses to a 3-rung mid ladder
under the bright spine through-line; and the night flex is pushed (higher aura
peaks + a spine rim-bloom + a brighter flame-pip socket). The whole verdict of
WISP is the DAY read at 40px — hooked beak → spine line → tail line as three
distinct reads — so the gameplay-day panel and the day-blue truth tile are
first-class. Night is where the additive bloom blooms.
...
Headless:
  SDL_VIDEODRIVER=dummy PYTHONPATH=. python tools/skeleton_candidates/render_v2_design_4_r2.py
"""
...
def main():
    ...
    title = FONT.render(
        "SKELETON · v2_design_4 WISP-MACAW — spectral ghost-fire (round 2)",
        True, (180, 255, 220))
    ...
    out = os.path.join(
        ROOT, "docs/store_redesign/costume/skeleton/v2/design_4/round_2.png")
