"""Render the v3_design_1 CLEAN skeleton review sheet (round_1).

CLEAN brief proof: the definitive bright-white-bone-on-dark read built ON the
REAL Pip silhouette — skull dome + ONE hollow eye-socket, a clear vertebral
spine, three bold rib rungs, a bone wing leading-edge that flaps, knob knees,
and a tail-feather bone over the ORIGINAL tail. Panels: gameplay DAY + NIGHT,
hero close-up, 40px NEAREST truth read, plus the LIVE original parrot
(sid "default") as a silhouette-fidelity reference corner.

Headless:
  SDL_VIDEODRIVER=dummy PYTHONPATH=. python tools/skeleton_candidates/render_v3_design_1_r2.py
"""
import os
...
OUT_DIR = os.path.join(ROOT, "docs/store_redesign/costume/skeleton/v3/design_1")
os.makedirs(OUT_DIR, exist_ok=True)

_spec = importlib.util.spec_from_file_location(
    "v3_design_1", os.path.join(ROOT, "tools/skeleton_candidates/v3_design_1.py"))
_mod = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_mod)
build = _mod.build

# The live original Pip silhouette — silhouette-fidelity reference.
ORIGINAL = lambda fi, t: parrot.get_skin_frame("default", fi, t) \
    if hasattr(parrot, "get_skin_frame") else parrot.get_frame(fi, t)
...
def ref_panel(source, box):
    """The live original Pip, split day/night, as a silhouette reference."""
    panel = pygame.Surface((box, box))
    panel.fill((150, 195, 235))
    pygame.draw.rect(panel, (18, 20, 34), (box // 2, 0, box - box // 2, box))
    frame = source(FRAME_IDX, TILT)
...
def main():
    ...
    ref = labelled(ref_panel(ORIGINAL, BAR),
                   "ORIGINAL PIP (silhouette ref)", font)

    pad = 24
    title_h = 44
    top_w = day.get_width() + night.get_width() + pad
    bot_w = (hero.get_width() + bar.get_width() + ref.get_width() + pad * 2)
    sheet_w = max(top_w, bot_w, 540) + pad * 3
    ...
    title = title_font.render(
        "SKELETON v3 — design_1  CLEAN  (round 2)",
        True, (255, 255, 255))
    ...
    sheet.blit(ref, (x + hero.get_width() + bar.get_width() + pad * 2, y))
