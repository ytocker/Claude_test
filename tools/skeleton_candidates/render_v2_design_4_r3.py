"""Render the WISP-MACAW (v2_design_4) skeleton review sheet — round 3 (final).

Round 3 folds the C2 critique (DAY-only; night kept byte-identical): the bright
tail is pulled to a shallower, longer horizontal sweep with a 1px dark keyline
gap parting it from the hips, so "long tail" no longer fuses with "two legs";
the beak notch is re-stamped crisp and the hook top-edge re-laid on the deepened
shared geometry (hook tip ~(55,32)) so the head isn't a rounded snout; the rib
arcs recede to one darker mid value and a fatter keyline walls the bright spine
so the backbone reads as ONE clean line. The whole verdict of WISP is the DAY
read at 40px — hooked beak → spine line → tail line as three distinct reads — so
the gameplay-day panel and the day-blue truth tile are first-class. Night is
where the additive bloom blooms (unchanged from round 2).
...
Headless:
  SDL_VIDEODRIVER=dummy PYTHONPATH=. python tools/skeleton_candidates/render_v2_design_4_r3.py
"""
...
def _assert_green(surf, label):
    ...
    wrong-builder leak before the sheet is committed. Scoped to the central bird
    box so warm scenery (the ground band / lit pillars at night) can't swamp the
    count when the bird itself is a small, quieter spectral cluster after the
    round-3 day fixes thinned its bright pixels."""
    w, h = surf.get_size()
    x0, x1 = int(w * 0.20), int(w * 0.80)
    y0, y1 = int(h * 0.15), int(h * 0.70)
    red = green = 0
...
def main():
    ...
    title = FONT.render(
        "SKELETON · v2_design_4 WISP-MACAW — spectral ghost-fire (round 3 · final)",
        True, (180, 255, 220))
    ...
    _label(sheet, "(40px on dark / on day-blue: beak->spine->tail)", pad, yb + 20 + 122)
    ...
    out = os.path.join(
        ROOT, "docs/store_redesign/costume/skeleton/v2/design_4/round_3.png")
