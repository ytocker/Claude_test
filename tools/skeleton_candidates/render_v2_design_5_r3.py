"""Render the v2 AUREX-MACAW (v2_design_5) round-3 FINAL review sheet — scratch.

Same composite as r2 (day gameplay, night gameplay, hero, 40px NEAREST truth
read, 4-frame filmstrip) so the iteration is judged like-for-like. r3 folds in
the C2 mantle fixes: the tattered cape is shrunk ~25-30% so it no longer bulges
below the spine line, its body value is lifted off the void floor, and a
deep-gold keyline now runs the BOTTOM hem too so the cape reads as cloth behind
the shoulders rather than a dark hole the skeleton sits on — letting the gold
win the first read on the bright day sky.
"""
...
from tools.skeleton_candidates.v2_design_5 import build_day, build_night

OUT = "docs/store_redesign/costume/skeleton/v2/design_5/round_3.png"
...
def _gameplay_panel(source, w, h, phase, *, frame_idx=FRAME_IDX, tilt=TILT):
    ...
    night panel actually renders under a dark sky with the rune-fire lit."""
...
def _truth_read(source, box):
    """The 40px NEAREST downscale — must clock GOLD SKULL + HOOKED BEAK → two
    violet socket points → gold crown → long gold tail, gold the brightest mass."""
...
def _filmstrip(source, cell, n=4):
    """The 4 flap poses side by side — the gold finger-wing sweeping."""
...
def main():
    ...
    sheet.blit(pygame.font.SysFont("dejavusans", 26, bold=True).render(
        "SKELETON v2 — design_5  AUREX-MACAW  (cursed gold-lich)  round 3 (final)", True, BONE),
        (pad, 12))

    y0 = title_h
    day = _gameplay_panel(build_day, gp_w, gp_h, 0.0)
    sheet.blit(day, (pad, y0))
    _label(sheet, small, "gameplay — DAY sky (gold wins first read; mantle a cape, not a hole)",
           pad + 4, y0 + gp_h + 2)
    ...
    _label(sheet, small, "40px truth read — GOLD SKULL+BEAK > violet pts > crown > tail",
           pad + 4, y1 + truth + 2)

    xs = pad * 2 + truth
    sheet.blit(_filmstrip(build_night, cell), (xs, y1))
    _label(sheet, small, "4-frame flap filmstrip — gold finger-wing",
           xs + 4, y1 + cell + 2)
