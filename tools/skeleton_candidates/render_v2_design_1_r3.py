"""Render the v2_design_1 BONEWHITE-MACAW review sheet (round_3).

Round-2 proof of the de-tangled lower-left quadrant: the lifted wing must sit
clearly ABOVE the spine, ≥4 of the 6 spine beads must stay visible, and the two
parrot tells (corrected shared down-hooked beak + long bony tail) must still
dominate at the 40px NEAREST truth read. Same four-panel layout as round_1:
gameplay DAY + NIGHT, hero close-up, 40px truth read.

Headless:
  SDL_VIDEODRIVER=dummy PYTHONPATH=. python tools/skeleton_candidates/render_v2_design_1_r2.py
"""
import os
...
def main():
    ...
    title = title_font.render(
        "SKELETON v2 — design_1  BONEWHITE-MACAW  (round 2)",
        True, (255, 255, 255))
