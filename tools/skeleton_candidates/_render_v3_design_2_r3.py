"""Round-3 (final) review sheet for v3_design_2 · X-RAY.

Headless: SDL_VIDEODRIVER=dummy PYTHONPATH=. python tools/skeleton_candidates/_render_v3_design_2_r3.py
"""
import os
...
def main():
    ...
    y0 = 46
    _label(sheet, font, "v3_design_2 · X-RAY — full skeleton (Round 3 · final)", M, 14)
    ...
    # 4-frame flap filmstrip (verify the single wing strut rotates with the flap).
    fx = M + 2 * (160 + 12) + 14
    for fi in range(4):
        f = XRAY(fi, 0.0)
    ...
    out = "docs/store_redesign/costume/skeleton/v3/design_2/round_3.png"
