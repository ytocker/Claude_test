"""Render the WISP (design_3) skeleton review sheet — round 2, scratch only.
...
Round-1 had a sheet-integrity blocker: some panels rendered design_4's pirate.
This script loads THIS design's build via an isolated spec_from_file_location so
no other candidate module's `build` can leak into any panel, and asserts every
panel is spectral-green (no red) before it writes the sheet.

Headless:  SDL_VIDEODRIVER=dummy python tools/skeleton_candidates/render_design_3_r2.py
"""
...
def main():
    ...
    title = FONT.render(
        "SKELETON · design_3 WISP — spectral ghost-fire (round 2)",
        True, (180, 255, 220))
    ...
    out = os.path.join(
        ROOT, "docs/store_redesign/costume/skeleton/design_3/round_2.png")
