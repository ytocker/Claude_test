"""Render the round_3 review sheet for CELESTIAL PANDA design_5 (final).
...
meant to be judged, and where the open-ring halo / glowing eyes must hold) is at
the night keyframe, not at 1.0.
"""
import os
...
def _night_gameplay_panel(w, h):
    ...
    is lifted to sit the bird against the dark UPPER sky and exclude the lit
    horizon band — the cosmic palette, open-ring halo and glowing eyes are meant
    to be judged on a genuinely dark background, not the dusk-blue glow."""
...
label("CELESTIAL PANDA  ·  design_5  ·  round_3  (legendary) — open halo · belly frame · glowing eyes · fused tail", PAD, 8)
...
ty = TOP + PANEL_H + 30
label("40px NEAREST truth read — 4 wing frames (day sky | night sky)", PAD, ty - 4)
fx = PAD
for fi in range(4):
    frame = source(fi, 0.0)
    ...
    # float and the glowing eyes are judged against both contexts at 40px.
