"""Render the round_3 review sheet for CLASSIC PANDA design_1."""
import os
...
spec = importlib.util.spec_from_file_location(
    "panda_design_1", "/home/user/skybit/tools/panda_candidates/design_1.py")
...
PANEL_W, PANEL_H = 240, 340     # ratio ≤ 0.72 so the gameplay crop stays in-bounds
HERO_BOX = 240
PAD = 18
TOP = 34
...
def _night_gameplay_panel(w, h):
    """A night-biome variant of ninja_render.gameplay_panel (phase ~0.5)."""
...
# Row 1: three columns.
label("CLASSIC PANDA  ·  design_1  ·  round_3", PAD, 8)
...
for name, panel in cols:
    # Centre each panel in the column (hero is square, shorter than PANEL_H).
...
# Row 2: four frames at 40px NEAREST scale (the truth read).
ty = TOP + PANEL_H + 30
...
for fi in range(4):
    ...
    # Fit the bird into a 40px tall box (the in-motion target), NEAREST scale.
    fw, fh = frame.get_size()
    scale = 40 / fh
    small_frame = pygame.transform.scale(
        frame, (max(1, int(fw * scale)), 40))
    # Swatch backdrop so the white reads on the dark sheet.
