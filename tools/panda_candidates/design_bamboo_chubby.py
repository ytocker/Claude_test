"""Panda — BAMBOO MUNCHER body + CHUBBY DUMPLING face.

A hybrid build: the BAMBOO MUNCHER rig (one unbroken diagonal bamboo cane,
black-and-white body, paw grips, crown leaf sprig) carrying the CHUBBY
DUMPLING's softer face — warm-brown round eye patches, sparkling glint-lit
eyes, rosy blush, and the little smile-mouth — proportionally scaled down to
fit the smaller bamboo head. The bamboo silhouette stays the identity; the
chubby face just trades the muncher's sharp teardrop mask for the dumpling's
rounder, friendlier read.
"""
import math
import pygame
...
# Chubby-face additions — warm patch + bright blush so the face reads soft, not
# a hollow mask, even after the 40px downscale.
PATCH    = (42, 26, 10)      # warm dark-brown eye patch (from CHUBBY)
PATCH_H  = (66, 44, 22)      # soft brown rim
PINK     = (255, 143, 160)   # rosy cheek blush (from CHUBBY)
...
def build(wing_angle_deg):
    """Draw one 64x84 frame of the Bamboo Muncher panda with a chubby face."""
    surf = _new()
    ...
    # ================= HEAD =================
    # Bamboo head anchors: HCX=44, HCY=34, CROWN_Y=24
    # Head radius ~12 (vs chubby's ~19), so all offsets scaled by ~0.63

    # Round black ears on the crown (keep bamboo-sized, r=6 each)
    for ex in (HCX - 9, HCX + 9):
        _aaellipse(surf, BLACK, (ex, CROWN_Y + 3), 6, 6)
        _aaellipse(surf, BLACK_HI, (ex - 1, CROWN_Y + 1), 2, 2)

    # Round white face disc (unchanged from bamboo)
    _aaellipse(surf, WHITE_SH, (HCX, HCY + 1), 13, 12)
    _aaellipse(surf, WHITE, (HCX - 1, HCY), 12, 11)

    # CHUBBY-style round eye patches — warm brown, circular, scaled to bamboo head
    # Chubby: side*9, HCY+3, patch 6x7 -> scaled: side*6, HCY+2, patch 4x5
    for side in (-1, 1):
        px = HCX + side * 6
        py = HCY + 2
        _aaellipse(surf, PATCH_H, (px, py), 5, 6)   # warm rim
        _aaellipse(surf, PATCH,   (px, py), 4, 5)   # patch body
        # Eye inside the patch
        eye_x = px - side            # slightly toward nose
        eye_y = py + 1
        pygame.draw.circle(surf, WHITE,  (eye_x, eye_y), 2)   # white
    ...
    # Rosy cheek blushes — chubby's bright PINK scaled to bamboo face
    # Chubby: side*14, HCY+10, 5x4 -> scaled: side*9, HCY+7, 3x3
    for side in (-1, 1):
        cx = HCX + side * 9
        cy = HCY + 7
    ...
    # Chubby-style ellipse nose (replaces bamboo's triangle)
    # Chubby nose at HCY+9, scaled to HCY+6 for smaller head
    nx0, ny0 = HCX, HCY + 6
    _aaellipse(surf, NOSE, (nx0, ny0), 2, 1)
    _aaellipse(surf, BLACK_HI, (nx0, ny0 - 1), 1, 1)   # soft highlight

    # Chubby-style soft w / smile mouth under the nose
