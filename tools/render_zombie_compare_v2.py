"""Final comparison figure for the zombie redesign batch v2.

Six columns side by side — the ORIGINAL live zombie plus the five new
designs (design_6 through design_10) — each Pip mid-flight over the same
real gameplay biome scene.

Run headless from repo root:
``SDL_VIDEODRIVER=dummy python -m tools.render_zombie_compare_v2``.
"""
...
title = _font(32, True).render(
    "ZOMBIE REDESIGN v2 — ORIGINAL vs. 5 NEW DESIGNS (in gameplay)",
    True, _GOLD_PALE)
...
out = os.path.join("docs", "store_redesign", "costume", "zombie", "final_comparison_v2.png")
