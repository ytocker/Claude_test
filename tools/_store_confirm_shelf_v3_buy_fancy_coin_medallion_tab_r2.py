"""BUY-button differentiation: coin-medallion-tab (D) — round 2.

A gold coin emblem is inset into the BUY button's left edge — a wax-seal /
medallion tab that slightly overhangs the button's left flank so BUY reads as
"the one that costs coins". CANCEL stays plain. Locked BUY keeps the base
slate + padlock treatment (no coin), so affordability still reads instantly.

R2 tightens the gold hierarchy: CANCEL's rim is demoted to muted slate so the
bright gold bevel belongs to BUY alone, and the coin tab is nudged right for a
clean 4px clearance off the shelf wall instead of the crowded 1px in R1.

Everything else — card body, gems, name, banner, shelf, chip, disc/aura/thumb
drawn LAST, SS=2 pipeline, RGB tostring, 444×412 sheet — is unchanged from the
c-orig-bg base.

Output → docs/store_confirm_shelf_v3/buy-fancy/coin-medallion-tab/round_2.png
"""
import os, sys, math
...
def _buy_btn(big, btn_rect, rad, locked):
    if locked:
        # Base slate + padlock treatment — no coin so affordability reads first.
    ...
    # Unlocked affordable — mid-indigo fill, gold bevel, right-shifted label,
    # then a coin medallion tab overhanging the left edge (drawn last, on top).
    stops = [(0.0, (68, 64, 104)), (1.0, (44, 40, 74))]
    sc.drop_shadow(big, btn_rect, rad, blur=m(3), alpha=150, dy=m(2))
    big.blit(sc.vgrad_stops(btn_rect.w, btn_rect.h, rad, stops, 255),
             btn_rect.topleft)
    sc.top_sheen(big, btn_rect, rad, m(12), peak=22)
    sc.bevel_rim(big, btn_rect, rad, sc.CARD_RING_DEEP,
                 (*sc.CARD_RING_BRIGHT, 230), w=max(1, m(2.0)))
    sc.plain_text(big, "BUY", sc.font(14),
                  (btn_rect.centerx + m(10), btn_rect.centery), (210, 205, 240),
                  shadow_a=110, weight=m(0.8), keyline=(8, 6, 20), kw=m(0.9))
    # Coin at x=27 (left edge 27-10=17) clears the shelf wall at x=13 by 4px.
    sc.coin_glyph(big, m(27), m(302), m(10))


def _cancel_btn(big, btn_rect, rad):
    stops = [(0.0, (26, 28, 64)), (1.0, (14, 16, 44))]
    sc.drop_shadow(big, btn_rect, rad, blur=m(3), alpha=100, dy=m(2))
    big.blit(sc.vgrad_stops(btn_rect.w, btn_rect.h, rad, stops, 255),
             btn_rect.topleft)
    sc.top_sheen(big, btn_rect, rad, m(12), peak=14)
    # Muted slate rim — gold exclusivity belongs to BUY, so CANCEL steps back.
    sc.bevel_rim(big, btn_rect, rad, (20, 18, 36, 120), (100, 95, 130, 160),
                 w=max(1, m(1.4)))
...
def _draw_shelf(big, affordable):
    ...
    brad = m(BTN_RAD)
    _buy_btn(big, buy, brad, locked=not affordable)
    _cancel_btn(big, can, brad)
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "coin-medallion-tab (D) r2  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_confirm_shelf_v3", "buy-fancy",
                   "coin-medallion-tab", "round_2.png")
...
pip_px  = aff.getpixel((27, BTN_CY))
left_px = aff.getpixel((17, BTN_CY))
print(f"coin pip (27,{BTN_CY}): {pip_px}  → should be gold")
print(f"left of coin (17,{BTN_CY}): {left_px}  → should be shelf/bg (clearance)")
print(f"CANCEL center ({CAN_CX},{BTN_CY}): {can_px}  → dark indigo, no bright gold")
print(f"BUY center ({BUY_CX},{BTN_CY}): {buy_px}  → on-button")
