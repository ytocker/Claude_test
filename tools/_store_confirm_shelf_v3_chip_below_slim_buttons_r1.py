"""Chip-below — option C: slim-buttons (R1).

Flips the shelf stack from the bottom-gems R2 layout: the Buy/Cancel
pill row rides near the shelf TOP and the price chip drops to the shelf
BOTTOM, hugging the corner gems. Reading the CTA first, then the price,
keeps the eye on the action while the cost sits as a footnote by the gem
frame.

  - BTN_CY=246 (buttons near shelf top), BTN_H=28 (slim pill, full CTA weight).
  - CHIP_CY=281, CHIP_H=30 (chip near shelf bottom / gems at cy=309).
  - Buttons drawn before the chip so the chip's shadow reads above the gems.
  - NOT ENOUGH text at m(CHIP_CY).

Sheet: left=AFFORDABLE, right=UNAFFORDABLE, 444×412
Output → docs/store_confirm_shelf_v3/chip-below/slim-buttons/round_1.png
"""
import os, sys, math
...
# Option C: slim buttons up top, price chip dropped to the shelf bottom
SHELF_X, SHELF_Y, SHELF_W, SHELF_H = 13, 222, 174, 97
BTN_W, BTN_H, BTN_RAD              = 76, 28, 9
BTN_CY, BTN_GAP                    = 246, 8
...
draw.text((CANVAS_W // 2, MARGIN + HDR_H // 2),
          "Chip-below C: slim-buttons R1  |  AFFORDABLE / UNAFFORDABLE",
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
      "docs", "store_confirm_shelf_v3", "chip-below", "slim-buttons", "round_1.png"))
...
# Verify chip / button / gem placement against the sheet coordinates.
chip_px = canvas.getpixel((18 + 100, panels_y + 281))
btn_px  = canvas.getpixel((18 + 58,  panels_y + 246))
