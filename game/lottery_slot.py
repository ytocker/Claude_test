...
bird so it never sits in the pillar-approach lane. Compact ~118x86 px
...
Reveal feedback is asymmetric: positive tiers get confetti pouring
out the bottom of the cabinet, negative tiers and NOTHING get only
the cabinet + result strip. Same pattern the in-game float-text
already uses, so the two cues reinforce each other.
"""
...
def draw_reveal(surf, anim):
    ...
    if t >= LOTTERY_REVEAL_TIME:
        ...
        if delta > 0:
            # Confetti spray for wins — no surrounding halo rings
            # (the cabinet itself + result strip carry the celebration;
            # extra halo circles read as visual noise per playtest).
            ...
            _confetti_burst(surf, cabinet_center[0],
                            ...
                            seed=seed_for_tier)
        # Losses: just the cabinet + result strip — no red halo
        # rings, no confetti. The sting is the absence of
        # celebration, not an extra wash.
