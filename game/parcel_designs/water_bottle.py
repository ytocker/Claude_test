...
brighter so the contrast at the water line is sharp. The cap is widened and its
groove hardened to OUTLINE colour so it reads as a screw-on lid, not a painted
red stripe, with a clearly flat top edge. Drawn on a 44px work surface then
smoothscaled to 22 so the shoulder and meniscus antialias cleanly. A baked dark
outline (inflated, drawn first) carries the shape on bright DAY sky; a cool
...
def build(mode="normal") -> pygame.Surface:
    ...
    # Cap: a bold coloured block, slightly wider than the neck. Chunky so it
    # stays the colour anchor even when banked to 90deg, with a flat top edge.
    CW, CH = 14, 8
