...
class HUD:
    ...
    def draw_play(self, surf, world, best: int, paused: bool = False):
        ...
        if active:
            ...
            base_x    = (W - row_w) // 2
            # Default sits just below the live glass-pill score at
            # y=92 (height 56 → bottom ≈ 120). The SKATEBOARD E3
            # score now overlays the banner at y=75 (burst bottom
            # ≈ 117), so the timer row at y=128 still reads cleanly
            # below it — no special case needed anymore.
            top_y = 128
