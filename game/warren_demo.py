...
class WarrenDemo:
    ...
    def _draw_celebration(self, surf):
        ...
        (not painted on the cube): a struck jester medallion (see
        `game.dice_medallion`) with the rolled N as its hero. The reward ramp
        reads on crest-bell density + the banderole label (low/mid → "PAGODAS",
        high → "JACKPOT"); a GHOST roll re-skins it navy with "GHOST!" so its 10
        never looks like a plain 10.
        ...
        from game import dice_medallion
        canvas, D = dice_medallion.render(self.roll, self.ghost_run, ss=3)
